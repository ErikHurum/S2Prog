...
void TReadFileFormat::WriteConvertedTables(wchar_t *fileName)
{
	...
	for ( int i=0; i < NumberOfTanks; i++ ) {
		if ( VolumeEnabled ) {
			MinimizeHeightVol(Tank[i],VolumeLimit);
		}
		if ( SoundingEnabled ) {
			MinimizeSoundingVol(Tank[i],VolumeLimit);
		}
		if ( LCGEnabled ) {
			MinimizeCG(Tank[i],CenterOfGravityLimit);
		}
		if ( IxxEnabled ) {
			MinimizeFSM(Tank[i],FreeSurfaceMomentLimit);
		}
		if ( LCFEnabled ) {
			MinimizeCF(Tank[i],CenterOfFlotationLimit);
		}
	}
	OutFile.setf(ios::fixed);

	for ( int i=0,Cnt=1; i < NumberOfTanks; i++ ) {
		TankData *TPtr = Tank[i];
		AnsiString Ttype;
		...
		OutFile << Ttype << "\t" << endl;
		OutFile << "\tName\t" << TPtr->Name << endl;

		if ( TPtr->IsOnline ) {
			AnsiString idString;
			idString.sprintf("0x%0x", 0x41a0000+Cnt++);
			OutFile <<  "\tProTank " << idString << endl;
		}
		if (!TPtr->FirstFrame.IsEmpty() ) {
			OutFile << "\tFrame  " <<  TPtr->FirstFrame << "\t" << TPtr->LastFrame << endl << endl;
		}
		//OutFile << "\tMAX_TABLE_ULLAGE";
		//OutFile << setw(8) << setprecision(3) << TPtr->Level1[TPtr->Entries1-1] << endl;
		if ( VolumeEnabled ) {
			if (UllageEnabled) {
				OutFile << "\tUllageVolTable" << endl;
			}else {
				OutFile << "\tLevelVolTable" << endl;
			}
			OutFile << "\t\tTableOffset	0.0" << endl;
			OutFile << "\t\tStartTable" << endl;
			...
			OutFile << "\tEndTableObject" << endl << endl;
		}
		if ( SoundingEnabled ) {
...
void TReadFileFormat::Initialize(void){
	...
	InValueList[InValueListEntries++] = Value15;
	InValueList[InValueListEntries++] = Value16;
	InValueList[InValueListEntries++] = Value17;
	InValueList[InValueListEntries++] = Value18;
	InValueList[InValueListEntries++] = Value19;
	InValueList[InValueListEntries++] = Value20;
	InValueList[InValueListEntries++] = Value21;
	InValueList[InValueListEntries++] = Value22;
	InValueList[InValueListEntries++] = Value23;
	InValueList[InValueListEntries++] = Value24;
	InValueList[InValueListEntries++] = Value25;
	...
	OutValueList[OutValueListEntries++] = Value115;
	OutValueList[OutValueListEntries++] = Value116;
	OutValueList[OutValueListEntries++] = Value117;
	OutValueList[OutValueListEntries++] = Value118;
	OutValueList[OutValueListEntries++] = Value119;
	OutValueList[OutValueListEntries++] = Value120;
	OutValueList[OutValueListEntries++] = Value121;
	OutValueList[OutValueListEntries++] = Value122;
	OutValueList[OutValueListEntries++] = Value123;
	OutValueList[OutValueListEntries++] = Value124;
	OutValueList[OutValueListEntries++] = Value125;
	LevelEnabled    = false;
	UllageEnabled   = false;
...
void __fastcall TReadFileFormat::TankComboBoxChange(TObject *Sender){
	...
	if ( TIndex >= 0 ) {
		...
		for ( int i=0; i <Tank[TIndex]->Entries1; i++ ) {
                        float XValue = Tank[TIndex]->Level1[i];
                        float YValue = Tank[TIndex]->Volume[i];
...
//---------------------------------------------------------------------------
void TReadFileFormat::TurnTable(void)
{
	for (int i = 0; i < NumberOfTanks; i++) {
		int tmpNumberOfRows  = NumberOfRowInTank[i];
		// Check if we must turn the tables
		if ( tmpNumberOfRows >= 2  && ( Level[i][NumberOfRowInTank[i]-1] < Level[i][0])) {
			vector<float>vLevel   ((&Level	    [i][0]), &Level   [i][tmpNumberOfRows]);
			vector<float >vUllage  ((&Ullage	[i][0]), &Ullage  [i][tmpNumberOfRows]);
			unsigned LevSize = vLevel.size();
			vector<float >vSounding((&Sounding	[i][0]), &Sounding[i][tmpNumberOfRows]);
			vector<float >vVolume  ((&Volume	[i][0]), &Volume  [i][tmpNumberOfRows]);
			vector<float >vLCG     ((&LCG		[i][0]), &LCG     [i][tmpNumberOfRows]);
			vector<float >vTCG     ((&TCG		[i][0]), &TCG     [i][tmpNumberOfRows]);
			vector<float >vVCG     ((&VCG		[i][0]), &VCG     [i][tmpNumberOfRows]);
			vector<float >vSwl     ((&Swl		[i][0]), &Swl     [i][tmpNumberOfRows]);
			vector<float >vLCF     ((&LCF		[i][0]), &LCF     [i][tmpNumberOfRows]);
			vector<float >vTCF     ((&TCF		[i][0]), &TCF     [i][tmpNumberOfRows]);
			vector<float >vIxx     ((&Ixx		[i][0]), &Ixx     [i][tmpNumberOfRows]);
			vector<float >vIyy     ((&Iyy		[i][0]), &Iyy     [i][tmpNumberOfRows]);
			vector<float >vTrim    ((&Trim		[i][0]), &Trim    [i][tmpNumberOfRows]);
			vector<float >vPressure((&Pressure	[i][0]), &Pressure[i][tmpNumberOfRows]);
			reverse(vLevel   .begin(),vLevel   .end());
			reverse(vUllage  .begin(),vUllage  .end());
			reverse(vSounding.begin(),vSounding.end());
			reverse(vVolume  .begin(),vVolume  .end());
			reverse(vLCG     .begin(),vLCG     .end());
			reverse(vTCG     .begin(),vTCG     .end());
			reverse(vVCG     .begin(),vVCG     .end());
			reverse(vSwl     .begin(),vSwl     .end());
			reverse(vLCF     .begin(),vLCF     .end());
			reverse(vTCF     .begin(),vTCF     .end());
			reverse(vIxx     .begin(),vIxx     .end());
			reverse(vIyy     .begin(),vIyy     .end());
			reverse(vTrim    .begin(),vTrim    .end());
			reverse(vPressure.begin(),vPressure.end());

			for (int j= 0; j<tmpNumberOfRows; j++) {
				Level[i][j]     	= vLevel   [j];
				Ullage[i][j]    	= vUllage  [j];
				Sounding[i][j]  	= vSounding[j];
				Volume[i][j]    	= vVolume  [j];
				LCG[i][j]       	= vLCG     [j];
				TCG[i][j]       	= vTCG     [j];
				VCG[i][j]       	= vVCG     [j];
				Swl[i][j]       	= vSwl     [j];
