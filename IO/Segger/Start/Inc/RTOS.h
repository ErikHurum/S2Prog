...
/*********************************************************************
*
*     Specifics for Atmel AVR cores and IAR compiler
*
**********************************************************************
*/
...
#define OS_PTR_OP_IS_ATOMIC       (0)
#define OS_U32_OP_IS_ATOMIC       (0)
#define OS_GetTime()              (OS__Gettime())
#define OS_SIZEOF_INT             (2)                        // Size of integer in bytes
#define OS_BIG_ENDIAN             (0)
#define OS_SUPPORT_SYSSTACK_INFO  (0)                        // Option to supply more info  to viewer
...
#define OS_SP_ADJUST              (-1)
#define OS_CONST_PTR                                         // Data pointer are default pointer, not const

#define OS_EI_ON_LEAVE()          OS_EnableInterrupts()

/*****  Core specific configuration *********************************/

#define OS_SWITCH_FROM_INT_MODIFIES_STACK   (0)
#define OS_INTERRUPTS_ARE_NESTABLE_ON_ENTRY (1)
#define OS_SUPPORT_INT_PRIORITY             (0)              // No selectable interrupt priorities with AVR
#define OS_SUPPORT_ENTER_INTERRUPT          (0)
...
#else
  #define OS_DI()                 asm("cli")
  #define OS_EI()                 asm("sei")
#endif

#define OS_SADDR                                             // For ATmega 103: tiny could be set
#define OS_STACKPTR             __near                       // SP is 16 bits
#define OS_STACK_ADR            OS_U16                       // Stack resides in lower 64KB
#define OS_COM_OUT_BUFFER_SIZE  (100)
#define OS_DUMMY_STATEMENT()    OS_Dummy++
#define OS_DUMMY_TYPE           OS_U8
#define OS_SUB_TIME(a, b)       OS_SubTime(a, b)
#define OS_TIME                 int
...
#if (defined(OS_LIBMODE_D) || defined(OS_LIBMODE_DP) || defined(OS_LIBMODE_DT))
  #define OS_CallISR            OS_CallISR_Debug
  #define OS_CallNestableISR    OS_CallNestableISR_Debug
#endif
...
typedef struct {
  ...
  char r26;
  char r27;
  int  r28_29;                          /* R28,R29 used as stack pointer                 */
  char PCH;                             /* Task return address                           */
#if (__CORE__ == 5) || (__CORE__ == 6)  /* -v5/6 cores have 24bit address range for code */
  char PCM;                             /* Task return address                           */
#endif
  char PCL;
} OS_REGS_BASE;
...
#define OS_VERSION_GENERIC (38607)       /*   Generic version 3.86g */
...
#define OS_TRACE_ID_WAITMAIL_TIMED  (59)
...
__version_3 void OS_CallNestableISR(void (*pRoutine)(void));
  ...
  #define OS_SWITCH_FROM_INT_IF_REQUIRED()                        \
    if ((OS_RegionCnt == (OS_U8) 1u) && (OS_Pending.All != 0u)) { \
      OS_SwitchFromInt();                                         \
    }
...
#else
  #define OS_SET_TIME(t)    OS_Time = (t);
#endif
...
#define OS_TICK_HANDLE() {                                                                   \
  ...
  Time = OS_Time +1;       /* Compute new OS_Time */                                         \
  OS_SET_TIME(Time);       /* Update the global time variable as atomic operation */         \
  if ( OS_SUB_TIME((OS_TIME)Time, OS_TimeDex) >= 0) {  /* Check if the delay is expired */   \
    OS_Pending.Flag.Preemption = 1;                                                          \
  }                                                                                          \
  OS_HANDLE_RR();                                                                            \
  OS_HANDLE_TICK_HOOK();                                                                     \
}
