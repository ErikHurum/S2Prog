...
----------------------------------------------------------------------
File    : RTOSInit_ATmega64.c for ATmega64 CPU and IAR compiler
...
*/
void OS_InitHW(void) {
  TCCR1A = 0x00;              // No output, no PWM
  TCCR1B = 0x09;              // Disable noise canceler, capture edge, Reset on compare match, clock source CK/1
  OCR1A  = (OS_TIMER_RELOAD); // Generate 1 msec interrupts
  TIMSK |= (1uL << 4);        // Output CompareA Match Interrupt Enable
