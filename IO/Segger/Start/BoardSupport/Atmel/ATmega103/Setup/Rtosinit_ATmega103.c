...
----------------------------------------------------------------------
File    : RTOSInit.c   (for ATmega103 or ATMEL AT90)
...
#pragma vector = TIMER1_COMPA_vect
__interrupt void OS_ISR_Tick (void) {
  OS_CallISR(OS_ISR_Tick_Hanlder);
}
...
void OS_COM_Send1(unsigned char c) {
  UDR =c;
  UCR |=  (1<<5);      // Generate interrupt on transmit buffer emtpy
}
...
void OS_ISR_rx_Handler(void) {
  UCR &=  ~(1uL << 7);   // Avoid nesting itself !!!
  OS_OnRx(UDR);
...
void OS_ISR_tx_Handler(void) {
  UCR &=  ~(1<<5);   // Do not generate any more interrupts
  if (OS_OnTx() == 0) {
    OS_DI();         // Disable before allowing Tx interrupts
    UCR |= (1 << 5); // Re-Enable Tx interrupts
  }
}
