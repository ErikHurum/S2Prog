...
void ExtRS485Ctl(void) {
    ...
    for (char i = 0; i < 8; i++) {
        SetBitsOr(PORTC, (TData.RS4.PortUsed & (0x01 << i)));      // Switch on power
        OS_Delay(100);
    }
    ...
    while (1) {
        for (PortPair = 0; PortPair < 4; PortPair++) {        // loop through all ports
            ...
            if ((TData.RS4.PortUsed >> PortPair) & 0x01) {              // sensor on port 0?
                ...
                char SnsType = (TData.RS4.SensorType[0] >> (2 * PortPair));
                if (SnsType == 0x01) {    //Water ingress??
                    switch (SeqCnt) {
                    case 0:
            ...
            if ((TData.RS4.PortUsed >> (PortPair + 4)) & 0x01) {              // sensor on port 1?
                ...
                char SnsType = (TData.RS4.SensorType[1] >> (2 * PortPair));
                if (SnsType == 0x01) {    //Water ingress??
                    switch (SeqCnt) {
                    case 0:
...
void ReceiveWHData(char ch, unsigned short pointer) {
    ...
    for (short pnt = 0; pnt < 4; pnt++) {
        TData.RS4.WTrackCnt[(ch * 4) + PortPair][pnt] = ((unsigned short*)&UART16552[ch].pRxBuffer[pointer])[pnt];  // 4 x Wash track 
    }
    TData.RS4.LevelSwitch[(ch * 4) + PortPair] = UART16552[ch].pRxBuffer[pointer+8]; // Hgh levels in 9th byte
    TData.RS4.WTTime[(ch * 4) + PortPair] = *((unsigned short *)&UART16552[ch].pRxBuffer[pointer+9]);  // WashTrack time in 10th and 11th bytes
