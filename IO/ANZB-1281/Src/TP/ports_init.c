...
// Read DRDY of AD7715m falling edge.
interrupt [PORTC_PORT_vect] void portc_int_isr(void)
{
// Clear the PORTC interrupt flags
// test if Pin4 port C trigger interrupt
// DRDY_INTERRUPT_FLAG = 0b00010000 PINC.4
unsigned char tmp;
tmp=PORTC.INTFLAGS;
PORTC.INTFLAGS=PORTC.INTFLAGS;
    if ( (tmp&DRDY_INTERRUPT_FLAG)==DRDY_INTERRUPT_FLAG)    //
    {
...
// Write your code here
    if (WASHTRAC1 ==1 ^ (washtrac_prev & 0x01))
    {   delay_us (WASHTRAC_DELAY);
        if ( WASHTRAC1==1)
            washtrac[0] ++;
    }
    if (WASHTRAC2 ==1 ^ (washtrac_prev & 0x02))
    {   delay_us (WASHTRAC_DELAY);
        if ( WASHTRAC2==1)
            washtrac[1] ++;
    }
    if (WASHTRAC3 ==1 ^ (washtrac_prev & 0x04))
    {   delay_us (WASHTRAC_DELAY);
        if ( WASHTRAC3==1)
            washtrac[2] ++;
    }
    if (WASHTRAC4 ==1 ^ (washtrac_prev & 0x08))
    {   delay_us (WASHTRAC_DELAY);
        if ( WASHTRAC4==1)
            washtrac[3] ++;
    } 
