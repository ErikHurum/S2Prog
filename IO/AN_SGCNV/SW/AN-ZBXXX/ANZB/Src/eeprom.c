...
char ReadEEPROMByte( unsigned int Address ) {
    ...
    EEAR = Address;
    OS_IncDI();
    EECR |= (1 << EERE) ;       // Don't use SetBitsOr macrod due to timing
    OS_DecRI();
...
void WriteEEPROMByte( unsigned int Address, char Data ) {   
    ...
    EEAR = Address;
    EEDR = Data;
    EECR |= (1 << EEMWE) ;       // Don't use SetBitsOr macrod due to timing
    EECR |= (1 << EEWE) ;        // Don't use SetBitsOr macrod due to timing
    OS_DecRI();
}
...
void WriteEEPROMBuffer( unsigned int Address, char Size, char *pData ) {   
    ...
    EECR |= (1 << EEMWE) ;       // Don't use SetBitsOr macrod due to timing
    EECR |= (1 << EEWE) ;        // Don't use SetBitsOr macrod due to timing
    EECR |= (1 << EERIE) ;       // Don't use SetBitsOr macrod due to timing
    OS_DecRI();
}
