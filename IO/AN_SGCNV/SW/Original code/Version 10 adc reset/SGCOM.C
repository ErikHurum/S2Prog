...
short CheckAction(short pointer)
{
		...
		if (!(sendtail & TX_ERR)){
			...
			if (test == TRUE){
				if (	  (rx_buff[pointer+3] == 'L')
					...
					test = FALSE ;
					if (Eeprom(rx_buff[pointer+2]>>1, LOCK_APP)){	/* LOCK APP REG. */
						for (i = 0; i < 300; i++) {
							if (Eeprom(rx_buff[pointer+2]>>1, READ_STATUS)) {	/* read status register */
								if (!(eep_status & 0x03)) {
									test = TRUE ;
									i = 300 ;									/* exit loop */
								}
	...
	case SEND_SER_NO :								/* Serial no from eeprom */
		...
		if (!(sendtail & TX_ERR)){
			...
			if (Eeprom(rx_buff[pointer+2]>>1, READ_ROM)) {	/* EEP serial number */
				tx_indx += 8 ;
...
void BuildDisplay(void)
{
   ...
   for (i = 1; i< NO_OF_DISP; i++){
	tx_buff[tx_indx++] = display[i];
   }
