...
unsigned short		curr_sens_data ;				/* sensor data from ad conv. */

unsigned short		cal_data[NOIOCH] ;			/* calibration data */
unsigned short		raw_data[NOIOCH] ;			/* raw data from senors */
...
unsigned char     display[4] ;			/* display mode */
unsigned char		test_eep ;	
...
/*SCI1_SMR = SMR_1;  */ 	/* uart1 mode,			not used */
/*SCI1_BRR = BRR_1;  */ 	/* uart1 baudrate,	not used */
...
void SetSupplyChannel(unsigned char channel)
{
	...
	if (channel == SUPPCHOFF) {
		...
		P1DR &= ~0x03 ; 							/* switch off any channel */
		P3DR &= ~0xe0 ;
		ch_tmp = channel ;				/* remember the channel no */
		P3DR |= ((channel & 0x07) << 5) ;
		P1DR |= ((ch_tmp & 0x08) >> 3) | 0x02 ; /* set channel and enable bit */
	}
}
...
void ADConvert(void)
{
	...
	case	START_NEW_POINT :
		...
		case 0x03 :				 /* pressure */
			if (ad_mode & 0x02) {           	/* pressure readings */
				...
				WriteAdByte(0x2c) ;							/* Ad setup data */
	...
	case	CONV_COMPLETE :
		...
		if (ad_mode & 0x40) {					/* cal point ? */
			cal_data[curr_ad_ch] = ReadAdByte()* 256 ;
			cal_data[curr_ad_ch] += ReadAdByte() ;			/*read data */
			ad_mode &= ~0x40 ;
			SetSupplyCurrent(1) ;
			...
			case 0x03 :				   /* pressure */
				...
				if ((cal_data[curr_ad_ch] > FMAX_PRE_CAL)||
					...
					if (ad_mode & 0x80) {							/* test */
						ch_stat[curr_ad_ch] &= 0 ;				/* no sensor */
					}
				} else {
					if (ad_mode & 0x80) {							/* test */
						ch_stat[curr_ad_ch] |= 0x60 ;				/* pr. sensor  OK*/
					}
					ch_stat[curr_ad_ch] &= ~0x01 ;			/* Reading OK */
					raw_data[curr_ad_ch] = (unsigned short) (curr_sens_data * 10000.0
												  / cal_data[curr_ad_ch]) ;
					CalcPress(curr_ad_ch) ; 				/* calculate pressure */
				}
				Led(1, OFF)	;					/* ASt mark test only */
				Led(2, OFF)	;					/* ASt mark test only */
				break;
			case 0x02 :				    /* "other" */
				break;
			case 0x01 :				    /* temp */
				...
				if ((cal_data[curr_ad_ch] > FMAX_TMP_CAL) ||
					...
					if (ad_mode & 0x80) {							/* test */
						ch_stat[curr_ad_ch] |= 0x20 ;				/* tmp sensor  OK*/
					}
					ch_stat[curr_ad_ch] &= ~0x01 ;			/* Reading OK */
					raw_data[curr_ad_ch] = (unsigned short) (((float)curr_sens_data * 1000.0
			...
			curr_sens_data = ReadAdByte()* 256 ;
			curr_sens_data += ReadAdByte() ;				/*les data */
	...
	case	START_CALIBRATION :
		...
		if (tx_complete == TRUE) {
			...
			Led(1, ON) ;								/* ASt mark test led 1 on */
			WriteAdByte(ad_gain | 0x10) ;	  			/* Next write setup reg */
			if (ad_mode & 0x04){						/* 0ther */
				WriteAdByte(0x68) ;					/*  Ad setup data, bipolar */
			} else {
				WriteAdByte(0x6c) ;					/* Ad setup data, unipolar */
			}
		}
		break ;
	case	WAIT_FOR_CAL_INT :
		WriteAdByte(ad_gain | 0x18) ;	  			/* Next read setup reg */
		if (!(ReadAdByte() & 0xc0)){					/* read setup data */
			ad_state++ ;
		}
		break ;
	case	CAL_COMPLETE :
		...
		time_c_cnt = TIME_BETWEEN_CAL ; 			/*reaload timer */
		ad_state = IDLE ;
		Led(1, OFF) ;								/* ASt mark test led 1 off */
		break ;
	case	IDLE :
		break ;
	default :
...
void CalcPress(signed char ch)
{
	...
	if ((short)raw_data[ch] < sens_data[0]) {
		temp_data = CalcPercent((float)(sens_data[1]-sens_data[0]), (float)sens_data[0],
									 (float)raw_data[ch], 0.0) ;

	} else if (((short)raw_data[ch] >= sens_data[0]) && ((short)raw_data[ch] <  sens_data[1])) {
		temp_data = CalcPercent((float)(sens_data[1]-sens_data[0]), (float)sens_data[0],
									 (float)raw_data[ch], (float)sens_nl1) ;
		temp_data *= 40.0 ;				/* caluclate % */

	} else if (((short)raw_data[ch] >= sens_data[1]) && ((short)raw_data[ch] < sens_data[2])) {
		temp_data = CalcPercent((float)(sens_data[2]-sens_data[1]), (float)sens_data[1],
									 (float)raw_data[ch], (float)sens_nl2) ;
		temp_data = temp_data*(60.0 - 40.0)+40.0 ;				/* caluclate % */

	} else if (((short)raw_data[ch] >= sens_data[2]) && ((short)raw_data[ch] < sens_data[3])) {
		temp_data = CalcPercent((float)(sens_data[3]-sens_data[2]), (float)sens_data[2],
									 (float)raw_data[ch], (float)sens_nl1 * 0.5 ) ;
		temp_data = temp_data*(100.0 - 60.0)+60.0 ;				/* caluclate % */

	} else if ((short)raw_data[ch] >= sens_data[3]) {
		temp_data = CalcPercent((float)(sens_data[3]-sens_data[2]), (float)sens_data[2],
									 (float)raw_data[ch], 0.0) ;
		temp_data = temp_data*(100.0 - 60.0)+60.0 ;				/* caluclate % */
	}
...
*
*************************************************************************/
float	CalcPercent(float span, float low_span, float raw_data,
						float nl)
{
	...
	if (nl > 0) {
		sp_h_2 = span * span / 4.0 ;
		return ((nl*span + sp_h_2)/(2.0*nl*span))-
					sqrt(pow(((nl*span+sp_h_2)/(2.0*nl*span)),2.0)+
								(((low_span-raw_data)*sp_h_2)/(pow(span,2.0)*nl))) ;
	} else if (nl < 0){
		sp_h_2 = span* span / 4.0 ;
		return ((nl*span + sp_h_2)/(2.0*nl*span))+
					sqrt(pow(((nl*span+sp_h_2)/(2.0*nl*span)),2.0)+
								(((low_span-raw_data)*sp_h_2)/(pow(span,2.0)*nl))) ;
	} else {									/* non liniarety == 0 */
		return (raw_data-low_span) / span ;
	}
...
/*	temp_data = 3367.85 - sqrt(((100.0 - resist) / 58.0195e-6)+ 11342416.75) ;
*/
	temp_resist = resist - 100.0 ;
	temp_data = (temp_resist / 0.390802) +
					((temp_resist * temp_resist)/1000.0) ;   /* ASt mark */
...
/*	temp_data = (float)raw_data[ch] * 2.5 / 65535.0 ;/*input/16 bit*2.5 volt */
*/
	temp_data = (float)((short)raw_data[ch]) * 2.5 / 32767.0 ;/*input/15 bit*2.5 volt */
...
void UnpackSensData(signed char ch)
{
	...
	float temp_data ;
	char temp_byte ;
	unsigned short temp_us, knst_us ;

	temp_byte = FindTemp(ch) ;						/* find temperature channel */
	...
	temp_data /= 100.0 ;                      /* use 1/100 deg C in calc. */
													...
													/* start calculate data for 0 % FRO */
	if ((eep_type[ch >>1] & 0x01) || (eep_type[ch>>1] & 0x10)) { /* OK data */
		knst =	((short) packed_data[ch >> 1][8] +
							  packed_data[ch >> 1][9] * 256) & 0x1fff ;

		if (knst & 0x1000) {				/* negative */
			knst |= 0xe000 ;
		}
		g1 =  ((short) ((packed_data[ch >> 1][9] >> 5) & 0x07) +
							(packed_data[ch >> 1][10] * 8)	+
							((packed_data[ch >> 1][11] & 0x03) * 2048)) & 0x1fff ;
		if (g1 & 0x1000) {			      /* negative */
			g1 |= 0xe000 ;
		}
		g2 =  ((short) ((packed_data[ch >> 1][11] >> 2) & 0x3f) +
							((packed_data[ch >> 1][12] & 0x7f)* 64 )) & 0x1fff ;
		if (g2 & 0x1000) {			      /* negative */
			g2 |= 0xe000 ;
		}
		g3 =  ((short) ((packed_data[ch >> 1][12] >> 7) & 0x01) +
							(packed_data[ch >> 1][13] * 2)	+
							((packed_data[ch >> 1][14] & 0x0f) * 512)) & 0x1fff ;
		if (g3 & 0x1000) {			      /* negative */
			g3 |= 0xe000 ;
		}
		sens_data[0] = CalcSensData(knst, g1, g2, g3, temp_data) ;
																/* sens value 0%FRO @ temp */

													/* start calculate data for 40 % FRO */
		knst_us =  ((short) ((packed_data[ch >> 1][14] >> 4) & 0x0f) +
									(packed_data[ch >> 1][15] * 16) +
									((packed_data[ch >> 1][16] & 0x01) * 4096))& 0x1fff ;

		g1 =  ((short) ((packed_data[ch >> 1][16] >> 1) & 0x7f) +
							((packed_data[ch >> 1][17] & 0x3f) * 128)) & 0x1fff ;
		if (g1 & 0x1000) {			      /* negative */
			g1 |= 0xe000 ;
		}
		g2 =  ((short) ((packed_data[ch >> 1][17] >> 6) & 0x03) +
							(packed_data[ch >> 1][18] * 4)	+
							((packed_data[ch >> 1][19]& 0x07) * 1024)) & 0x1fff ;
		if (g2 & 0x1000) {			      /* negative */
			g2 |= 0xe000 ;
		}
		g3 =  ((short) ((packed_data[ch >> 1][19] >> 3) & 0x1f) +
							(packed_data[ch >> 1][20] * 32)) & 0x1fff ;
		if (g3 & 0x1000) {			      /* negative */
			g3 |= 0xe000 ;
		}
		knst += (short) knst_us;									/* remember new value */
		sens_data[1] = CalcSensData(knst, g1, g2, g3, temp_data) ;	/* sens value 40%FRO @ temp */

													/* start calculate data for 60 % FRO */
		knst_us =  ((unsigned short) packed_data[ch >> 1][21] +
							  packed_data[ch >> 1][22] * 256) & 0x1fff ;

		g1 =  ((short) ((packed_data[ch >> 1][22] >> 5) & 0x07) +
							(packed_data[ch >> 1][23] * 8)	+
							((packed_data[ch >> 1][24] & 0x03) * 2048)) & 0x1fff ;
		if (g1 & 0x1000) {			      /* negative */
			g1 |= 0xe000 ;
		}
		g2 =  ((short) ((packed_data[ch >> 1][24] >> 2) & 0x3f) +
							((packed_data[ch >> 1][25] & 0x7f)* 64 )) & 0x1fff ;
		if (g2 & 0x1000) {			      /* negative */
			g2 |= 0xe000 ;
		}
		g3 =  ((short) ((packed_data[ch >> 1][25] >> 7) & 0x01) +
							(packed_data[ch >> 1][26] * 2)	+
							((packed_data[ch >> 1][27] & 0x0f) * 512)) & 0x1fff ;
		if (g3 & 0x1000) {			      /* negative */
			g3 |= 0xe000 ;
		}
		knst += (short) knst_us;									/* remember new value */
		sens_data[2] = CalcSensData(knst, g1, g2, g3, temp_data) ;
																/* sens value 60%FRO @ temp */

													/* start calculate data for 100 % FRO */
		knst_us =  ((short) ((packed_data[ch >> 1][27] >> 4) & 0x0f) +
									(packed_data[ch >> 1][28] * 16) +
									((packed_data[ch >> 1][29] & 0x01)) * 4096)& 0x1fff ;

		g1 =  ((short) ((packed_data[ch >> 1][29] >> 1) & 0x7f) +
							((packed_data[ch >> 1][30] & 0x3f) * 128)) & 0x1fff ;
		if (g1 & 0x1000) {			      /* negative */
			g1 |= 0xe000 ;
		}
		g2 =  ((short) ((packed_data[ch >> 1][30] >> 6) & 0x03) +
							(packed_data[ch >> 1][31] * 4)	+
						((packed_data[ch >> 1][32]& 0x07) * 1024)) & 0x1fff ;
		if (g2 & 0x1000) {			      /* negative */
			g2 |= 0xe000 ;
		}
		g3 =  ((short) ((packed_data[ch >> 1][32] >> 3) & 0x1f) +
							(packed_data[ch >> 1][33] * 32)) & 0x1fff ;
		if (g3 & 0x1000) {			      /* negative */
			g3 |= 0xe000 ;
		}
		knst += (short) knst_us;									/* remember new value */
		sens_data[3] = CalcSensData(knst, g1, g2, g3, temp_data) ;
															/* sens value 100%FRO @ temp */

		sens_nl1 = (signed char)((packed_data[ch >> 1][34] >> 3) & 0x0f) ;	/* sensor NL1 */
		if ((packed_data[ch >> 1][34]) & 0x80) {
			sens_nl1 |= 0xf0 ;
		}

		sens_nl2 = (signed char) (packed_data[ch >> 1][34] & 0x07) ;			/* sensor NL2 */
		if ((packed_data[ch >> 1][34]) & 0x80) {
			sens_nl2 |= 0xf8 ;
		}

		offset[ch] =  packed_data[ch >> 1][35] +
						 (packed_data[ch >> 1][36] * 256) ;	 		/*offset*/

		gain[ch] =  1.0 + (float)((packed_data[ch >> 1][37] +
								  packed_data[ch >> 1][38] * 256.0)/100000.0) ;	/*gain*/
	} else {
		sens_data[0] = 0 ;
		sens_data[1] = 0 ;
		sens_data[2] = 0 ;
...
short FindTemp(short ch)
{
	...
	temp_ch = ch + 1 ;
	if ((mes_data[temp_ch] > 150.0) ||	   /* to high or */
...
interrupt [TMR_CMI1A] void TimInt(void)
{
	...
	TMR1_TCSR &= ~(TCSR_CMFB | TCSR_CMFA | TCSR_OVF);  /* Clear match to reenable interrupt */
...
interrupt [SCI_RXI0] void Rx0Int(void)
{
	...
	status = SCI0_SSR;
	bufch = SCI0_RDR;
	ad_mode |= 0x01 ;						/* mark that receiving starts */
