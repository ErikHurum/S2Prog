...
short CheckAction(short pointer)
{
	...
	case CMD_WRITE_EEP :
		...
		test = TRUE ;
		if (!Eeprom(rx_buff[pointer+2], WRITE_SP)){
			test= FALSE ; 								/* write data to scratchpad */
		}
		if (!Eeprom(rx_buff[pointer+2], READ_SP)){
		...
		if (!(sendtail & TX_ERR)){
			...
			if (test == TRUE) {								/* write to eep OK */
				if (Eeprom(rx_buff[pointer+2], COPY_SP_NV)) {	/* copy data to NV-ram */
	...
	case CMD_WRITE_ADR :
		...
		test = TRUE ;
		if (!Eeprom(rx_buff[pointer+2], WRITE_APPREG)){
			test = FALSE ; 						/* write data to adreg */
		}
		if (!Eeprom(rx_buff[pointer+2], READ_APPREG)){
		...
		if (!(sendtail & TX_ERR)){
			...
			if (test == TRUE){
				if (	  (rx_buff[pointer+3] == 'L')
					...
					test = FALSE ;
					if (Eeprom(rx_buff[pointer+2], LOCK_APP)){	/* LOCK APP REG. */
						for (i = 0; i < 300; i++) {
							if (Eeprom(rx_buff[pointer+2], READ_STATUS)) {	/* read status register */
								if (!(eep_status & 0x03)) {
									test = TRUE ;
									i = 1000 ;									/* exit loop */
								}
	...
	case SEND_EEP_DATA :
		...
		break ;
	case CMD_READ_EEP :
		eep_type[rx_buff[pointer+2]] |= 0x20 ;         /* Error before read */
		eep_type[rx_buff[pointer+2]] &= ~0x10 ;
	...
	case SEND_SER_NO :								/* Serial no from eeprom */
		...
		if (!(sendtail & TX_ERR)){
			...
			if (Eeprom(rx_buff[pointer+2], READ_ROM)) {	/* EEP serial number */
				tx_indx += 8 ;
				if (Eeprom(rx_buff[pointer+2], READ_STATUS)) { /* EEP status register */
		...
		break ;
	case REC_DISP_DATA :								/* Serial no from eeprom */
		RecDispData(pointer) ; 						/* new display data */
		break ;
	case SEND_ACK : 									/* No more data, send ack. */
	case NO_MORE_DATA :								/* No more data */
		if (display[0] & 0x80) {					/* send display data */
			if (!sendtail) {							/* make header ? */
				BuildHeader() ; 						/* Build send packet header */
				sendtail |= MAKE ; 				   /* build tail*/
			}
			if ((sendtail & TX_ERR) ||
				...
				BuildDisplay() ;						/* Build display packet. */
			}
			display[0] &= ~0x80 ;					/* send marker off */
		}
		if (sendtail) {
...
void BuildDisplay(void)
{
	...
	tx_buff[tx_indx++] = SEND_DISP_DATA ;			/* action */
	ntna = tx_indx++ ;									/* remember index */
	tx_buff[tx_indx++] = (display[0] & 0x7f ); 	/* remve send flag */
	tx_buff[tx_indx++] = display[1];
	tx_buff[tx_indx++] = display[2];
	tx_buff[tx_indx++] = display[3];
...
void RecOGData(unsigned char pointer)
{
	...
	for (i = 0; i < number; i++) {
	  ...
	  offset[channel] = rx_buff[++pointer] + (rx_buff[++pointer] * 256) ;
	  gain[channel] = 1.0 + (float)((rx_buff[++pointer] +
							  (rx_buff[++pointer] * 256))/100000.0) ;   /*gain*/
	}
}
...
*
*************************************************************************/
void RecDispData(unsigned char pointer)
{

	short i ;

	display[0] = 0x80 ;		/* send answer */
	/* ASt mark     H A N D L E     D A T A   H E R E  ! !  */
}
