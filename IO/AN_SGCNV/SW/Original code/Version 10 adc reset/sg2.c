...
{ 0, 94, 188, 226, 97, 63, 221, 131, 194, 156, 126, 32, 163, 253, 31, 65, 157,
	    195, 33, 127, 252, 162, 64, 30, 95, 1, 227, 189, 62, 96, 130, 220,
	    35, 125, 159, 193, 66, 28, 254, 160, 225, 191, 93, 3, 128, 222, 60,
	    98, 190, 224, 2, 92, 223, 129, 99, 61, 124, 34, 192, 158, 29, 67,
	    161, 255, 70, 24, 250, 164, 39, 121, 155, 197, 132, 218, 56, 102,
	    229, 187, 89, 7, 219, 133, 103, 57, 186, 228, 6, 88, 25, 71, 165,
	    251, 120, 38, 196, 154, 101, 59, 217, 135, 4, 90, 184, 230, 167,
	    249, 27, 69, 198, 152, 122, 36, 248, 166, 68, 26, 153, 199, 37,
	    ...
	    244, 170, 72, 22, 233, 183, 85, 11, 136, 214, 52, 106, 43, 117,
	    151, 201, 74, 20, 246, 168, 116, 42, 200, 150, 21, 75, 169, 247,
...
void CalcPress(signed char ch) 
{
	 ...
	 if ((sens_data[0] > 0) && (raw_data[ch] < sens_data[0])) {
		temp_data =
		    CalcPercent((sens_data[1] - sens_data[0]), sens_data[0],
				raw_data[ch], 0.0);
	 ...
	 temp_data *= ((((float)packed_data[ch >> 1][6] / 10.0) - 
			  (float)packed_data[ch >> 1][5] / 100.0) / 100.0) *
	    10.197162;
...
void UnpackSensData(signed char ch) 
{
	...
	if ((eep_type[c_ch] & 0x01) || (eep_type[c_ch] & 0x10)) {	/* OK data */
		...
		sens_data[0] =
		    CalcSensData(knst * 2, g1 * 2, g2 * 2, g3 * 2, temp_data);
		...
		sens_data[1] =
		    ...
		    d_knst =
		    ((short)packed_data[c_ch][21] +
		     packed_data[c_ch][22] * 256) & 0x1fff;
		 g1 =
		    ((short)((packed_data[c_ch][22] >> 5) & 0x07) +
	...
	if ((eep_type[7] & 0x01) || (eep_type[7] & 0x10)) {	/* ok data */
		cal_zero =
		    (short)packed_data[7][35] + packed_data[7][36] * 256;
...
{
	 return (knst + (g1 * temp) + (g2 * temp * temp) + (g3 * temp * temp * temp));	/* sens value ?%FRO @ temp */
 }
	...
	while (rx_indx > rx_hndx) {	/* more char to check */
		...
		case HEADER:
			if ((rx_hndx - rx_start) >= 3) {	/* rec. header */
				...
				if ((rx_packlen < ((short)RX_SIZE - rx_hndx))
				     && /* chk. header */ 
		...
		case RECEIVE:
			if (++rx_counter >= rx_packlen) {
				if ((rx_buff[rx_packlen + rx_start - 1]) ==
				     ETX) {
					if (CalcRxChecksum()) {
						if (rx_buff[rx_start + 1] <
						     PC_START_ADDRESS) {
						}
						if ((rx_buff[rx_start] == GetMyAddress()) || (rx_buff[rx_start] == 0)) {	/* don't use address */
							ret_val = TRUE;
						}
					} else {
