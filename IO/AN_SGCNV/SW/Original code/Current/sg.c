...
unsigned short ms_cnt;                 /* seconds sounter */
...
unsigned char       highlevel;              // high level result
unsigned short      washtrac[NOWT];          // Whashtrec result
...
/*Bit 7 = test mode
   ...
   Bit 1 = Read Pressure
   Bit 0 =		*/
unsigned short      ms_delay_cnt ;              /* increased each millisecond */
...
void InitRegs(void)
{
    ...
    /* Internal timer 1 ms */
    TMR0_TCR   =  0x4a;       /* int a, clear reg a, 1/64 clock (38600 Hz)*/
    TMR0_TCSR  = 0x00;        /* No invert p44 */
    TMR0_TCORA =  76 ;        /* Set 38600/(76+1) = 2 msec compare */
...
*
*************************************************************************/
void OneMsecDelay(unsigned short time)
{
    ms_delay_cnt = time ;
...
void CalcTemp(signed char ch)
{
    ...
    float temp_data = GetTempFromR(resist);
/*    
    float temp_resist = resist - 100.0 ;
...
/*************************************************************************
*
*      IRQ 2 , external int from connector
*
*************************************************************************/
...
/*************************************************************************
*
*  Timer0 interrupt routine for Washtrac
*
*  This routine is called each msec
*
*************************************************************************/
interrupt [TMR_CMI0A] void Tim0Int(void)
{
    TMR0_TCSR &= ~(TCSR_CMFB | TCSR_CMFA | TCSR_OVF);   // Clear match to reenable interrupt

    highlevel = P2DR;                       // read and store highlevel status


    if ( (P7DR & 0x01) ^ (washtrac_prev & 0x01) ) {   //read input 1
        washtrac[0] ++;
    }
    if ( (P7DR & 0x02) ^ (washtrac_prev & 0x02) ) {   //read input 2
        washtrac[1] ++;
    }
    if ( (P7DR & 0x04) ^ (washtrac_prev & 0x04) ) {   //read input 4
        washtrac[2] ++;
    }
    if ( (P7DR & 0x08) ^ (washtrac_prev & 0x08) ) {   //read input 4
        washtrac[3] ++;
    }
    washtrac_prev = P7DR;                             //remember last state
