	...
	case	CONV_COMPLETE :
		...
		if (ad_mode & 0x40) {							/* cal point ? */
	      ...
	      if (!(ad_mode & 0x01)) {					/* tx during conv? */
				cal_data[curr_ad_ch] = ad_data ;    /* no, use data  */
				...
				case 0x03 :				   /* pressure */
					...
					if ((cal_data[curr_ad_ch] > FMAX_PRE_CAL)||
						...
						raw_data[curr_ad_ch] = (unsigned short) (curr_sens_data * 30000.0
													  / cal_data[curr_ad_ch]) ;
			...
			curr_sens_data = ad_data ;	       /* use ad data */
...
*
*************************************************************************/
float	CalcPercent( unsigned short span, short low_span, unsigned short raw_data,
						float nl)
{
...
void CalcOther(signed char ch)
{
	...
	temp_data = (float)((short)(raw_data[ch]-32768)) * 2.5 / 32767.0 ;
...
void UnpackSensData(signed char ch)
{
	...
	if ((eep_type[c_ch] & 0x01) || (eep_type[c_ch] & 0x10)) { /* OK data */
		...
		sens_data[0] = CalcSensData(knst*3, g1*3, g2*3, g3*3, temp_data) ;
		...
		sens_data[1] = CalcSensData(knst*3, g1*3, g2*3, g3*3, temp_data) ;
		...
		knst += d_knst; 								/* remember new value */
		sens_data[2] = CalcSensData(knst*3, g1*3, g2*3, g3*3, temp_data) ;
		...
		knst += d_knst; 								/* remember new value */
		sens_data[3] = CalcSensData(knst*3, g1*3, g2*3, g3*3, temp_data) ;
