...
char ReadEEPROMByte( unsigned int Address ) {
    ...
    EEAR = Address;
    OS_IncDI();
    EECR |= (1 << EERE) ;       // Don't use My_SetBit macrod due to timing
    OS_DecRI();
...
void WriteEEPROMByte( unsigned int Address, char Data ) {   
    ...
    EEAR = Address;
    EEDR = Data;
    EECR |= (1 << EEMPE) ;       // Don't use My_SetBit macrod due to timing
    EECR |= (1 << EEPE) ;        // Don't use My_SetBit macrod due to timing
    OS_DecRI();
}
...
void WriteEEPROMBuffer( unsigned int Address, char Size, char *pData ) {   
    ...
    EECR |= (1 << EEMPE) ;       // Don't use My_SetBit macrod due to timing
    EECR |= (1 << EEPE) ;        // Don't use My_SetBit macrod due to timing
    EECR |= (1 << EERIE) ;       // Don't use My_SetBit macrod due to timing
    OS_DecRI();
#endif
}
...
void INT_Handler_EEPROM(void) {
    if ( --WriteCount ) {                        // More to write? 
        ...
        EECR &= ~(1 << EERIE) ;       // Don't use My_ClrBit macrod due to timing
        EEPWriteOK = true;
    }
