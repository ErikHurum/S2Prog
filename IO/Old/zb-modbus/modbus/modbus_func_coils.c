    ...
    */
    ...
    ( usCoilCount < MB_PDU_FUNC_READ_COILCNT_MAX ) )
    {
      ...
      *usLen += 1;
      ...
      eRegStatus = modbus_user_coil_cb(ctx, addr, pucFrameCur, usRegAddress, usCoilCount, MB_REG_READ );
...
modbus_func_handler_write_coil(ModbusCTX* ctx, uint8_t addr, uint8_t * pucFrame, uint16_t * usLen)
{
  ...
  if( *usLen == ( MB_PDU_FUNC_WRITE_SIZE + MB_PDU_SIZE_MIN ) )
  {
    ...
    )
    {
      ...
      eRegStatus = modbus_user_coil_cb(ctx, addr, &ucBuf[0], usRegAddress, 1, MB_REG_WRITE );
...
modbus_func_handler_write_multiple_coils(ModbusCTX* ctx, uint8_t addr, uint8_t * pucFrame, uint16_t * usLen)
{
  ...
  if( *usLen > ( MB_PDU_FUNC_WRITE_SIZE + MB_PDU_SIZE_MIN ) )
  {
    ...
    {
      eRegStatus = modbus_user_coil_cb(ctx, addr, &pucFrame[MB_PDU_FUNC_WRITE_MUL_VALUES_OFF], usRegAddress, usCoilCnt, MB_REG_WRITE );
