...
char CheckActionUart(char ch, unsigned short pointer) {
    ...
    switch (command) {                                  // check action EHSMark
    ...
    case CMD_REQ_ANA_DATA :                              // Request ANA data(4-20ma)
        if (UnitID == AN_ZBANA) {
            ...
            BuildMDataHart(ch, pointer + 4);                     // receive
        }
        break ;
...
/*************************************************************************
*
* Build datablock for WHData on ADcard
*
*************************************************************************/
void BuildWHdata(char ch, short pointer) {

    UART[ch].RxSendReply = true;                               // flag for answering

    UART[ch].pTxBuffer[UART[ch].TxFirst++] = CMD_SND_RS4_WHBUF & 0xff;
    UART[ch].pTxBuffer[UART[ch].TxFirst++] = CMD_SND_RS4_WHBUF >> 8;
    short ntna = UART[ch].TxFirst;                          // remember index
    UART[ch].TxFirst += 2;                                  // two byte length

    OS_EnterRegion();
    for (short i = 0; i < 4; i++) {
        UART[ch].pTxBuffer[UART[ch].TxFirst++] = TData.RS4.WTrackCnt[UART[ch].pRxBuffer[pointer]][i] & 0xff; // WashTrackcnt
        UART[ch].pTxBuffer[UART[ch].TxFirst++] = (TData.RS4.WTrackCnt[UART[ch].pRxBuffer[pointer]][i] >> 8) & 0xff; // WashTrackcnt
    }
...
*
*************************************************************************/
void BuildMDataHart(char ch, short pointer) {
    ...
    for (i = 0; i < 12; i++) {
#if 0 // err... man! -hkim-
        UART[ch].pTxBuffer[UART[ch].TxFirst++] = TData.Hart.Status[i];
        *((float *)&UART[ch].pTxBuffer[UART[ch].TxFirst]) = *((float *)&TData.Hart.Result[i]);   // ADresult
        UART[ch].TxFirst += sizeof(float);
#else
      HartChannel*    chnl;

      chnl = hart_channel_get_with_lock((hart_channel_t)i);
      {
        UART[ch].pTxBuffer[UART[ch].TxFirst++] = chnl->status == HartChannelStatus_Responding ? 1 : 0;
        *((float *)&UART[ch].pTxBuffer[UART[ch].TxFirst]) = *((float *)&chnl->distance);   // ADresult
        UART[ch].TxFirst += sizeof(float);
      }
...
*
*************************************************************************/
void GetResetEEPROM(char ch, short pointer) {
...
void GetEEPROMData(char ch, short pointer) {
  ...
  int t0;
    ...
    channel = UART[ch].pRxBuffer[pointer] ;
    if ((channel >= 0x80) && (UnitID == AN_ZBANA)) { // AN-ZBANA
...
void GetRS4Setup(char ch, short pointer) {
    ...
    //SetRS4Port();                                               // Set power on ports hkim
}
