...
void WriteEEPROMU16( unsigned int Address, unsigned int Data)
{   
  //
  // atmega is little endian
  // write low byte first
  //
  WriteEEPROMByte(Address, (Data) & 0xff);
  WriteEEPROMByte(Address + 1, (Data >> 8) & 0xff);
}
