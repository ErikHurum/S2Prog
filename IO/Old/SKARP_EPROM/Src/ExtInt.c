...
__interrupt void IntHandler_INT0( void ){               //Int handler for ext int 0
    ...
    case 0x00:                  // AN-ZB485  
        do {
            UART16552[0].TxStatus = (Read16552(U0_IIR) & 0x0f) ;      // reason for interrupt
            ...
            case 0x02 :             // transmit holding reg emty
                if (UART16552[0].TxCount == 0) {
                    ...
                    for (UART16552[0].TxSeqCnt = 0;( UART16552[0].TxSeqCnt < FIFO_TX_BUFSIZE ) && (UART16552[0].TxCount > 0 ); UART16552[0].TxSeqCnt++, --UART16552[0].TxCount) {
                        Write16552(U0_THR, UART16552[0].pTxBuffer[UART16552[0].TxLast++] );
                    }
                }
                break;
            ...
            case 0x0C :             // or caracter timeout (FIFO)
                while (Read16552(U0_LSR) & DATA_READY) {
                    ...
                    UART16552[0].pRxBuffer[ UART16552[0].RxFirst ] = Read16552(U0_RBR);                               // Read the character 
            ...
            case 0x06 :                 // Receiver line status, OR, PE, FR, Break int.
                Read16552(U0_LSR);      // Dummy read to empty register        
                break;
            case 0x00 :                 // Modem status.
                Read16552(U0_MSR);      // Dummy read to empty register           
                break;
            }
...
__interrupt void IntHandler_INT1( void ){               //Int handler for Ext int 1
    ...
    case 0x00:               // AN-ZB485 
        do {
            UART16552[1].TxStatus = (Read16552(U1_IIR) & 0x0f) ;      // reason for interrupt
            ...
            case 0x02 :             // transmit holding reg emty
                if (UART16552[1].TxCount == 0) {
                    ...
                    for (UART16552[1].TxSeqCnt = 0;( UART16552[1].TxSeqCnt < FIFO_TX_BUFSIZE ) && (UART16552[1].TxCount > 0 ); UART16552[1].TxSeqCnt++, UART16552[1].TxCount--) {
                        Write16552(U1_THR, UART16552[1].pTxBuffer[UART16552[1].TxLast++] );
                    }
                }
                break;
            case 0x04 :             // Receive data available    
            case 0x0C :             // or caracter timeout (FIFO)
                while (Read16552(U1_LSR) & DATA_READY) {
                    ...
                    UART16552[1].pRxBuffer[ UART16552[1].RxFirst ] = Read16552(U1_RBR);                               // Read the character 
                ...
                break;
            case 0x06 :                 // Receiver line status, OR, PE, FR, Break int.
                Read16552(U1_LSR);      // Dummy read to empty register        
                break;
            case 0x00 :                 // Modem status.
                Read16552(U1_MSR);      // Dummy read to empty register           
                break;
            }
