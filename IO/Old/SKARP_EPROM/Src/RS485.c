...
void TimoutUART0(void) {
    ...
    Write16552(U0_MCR, Read16552(U0_MCR) & ~RTS); // set RTS off
}
...
void TimoutUART1(void) {
    ...
    Write16552(U1_MCR, Read16552(U1_MCR) & ~RTS); // set RTS off
}
...
void ExtRS485Ctl(void) {
    ...
    TData.RS4.SensorType = ReadEEPROMByte(EEPROM_PORTSETUP+1); 
    TData.RS4.ToTargerBuffer[0] = false ;
    ...
    while (1) {
        for (PortPair = 0; PortPair < 4; PortPair++) {        // loop through all ports
            ...
            if ((TData.RS4.PortUsed >> PortPair) & 0x01) {              // sensor on port 0?
                ...
                if ((TData.RS4.SensorType >> PortPair) & 0x01) {    //Water ingress??
                    if (SeqCnt == 0) {
                      ...
                      if ((TData.RS4.FromTargetBuffer[0][0] == false) && (TData.RS4.EEPROMPortpair[0] == PortPair)){ // buffer ready
                        ...
                        for (i = 0; i < 4; i++) {
                          ...
                          if ((TData.RS4.PortUsed >> TData.RS4.EEPROMPortpair[0]) & 0x01){  // if used
                            i = 4;                                                          // break out
                          }
                        }
                      } else {
                  ...
                  if ((TData.RS4.ToTargerBuffer[0] == true ) && // Data ready for this AD card
                      (TData.RS4.ToTargerBuffer[1] == PortPair)) {
                      ...
                      if ((TData.RS4.FromTargetBuffer[0][0] == false) && (TData.RS4.EEPROMPortpair[0] == PortPair)){ // buffer ready
                        if ((TData.RS4.EEPROMADChannel[0] += 2) > 14) { //Next channel
                          ...
                          for (i = 0; i < 4; i++) {
                            ...
                            if ((TData.RS4.PortUsed >> TData.RS4.EEPROMPortpair[0]) & 0x01){  // if uses
                              i = 4;                                                          // break out
                            }
                          }
                        }
            ...
            if ((TData.RS4.PortUsed >> (PortPair+4)) & 0x01) {              // sensor on port 1?   
                ...
                if ((TData.RS4.SensorType >> (PortPair+4)) & 0x01) {    //Water ingress??
                    if (SeqCnt == 0) {
                        Uart_BuildReqWISCardStatus(1);          //WIS Card status
                    } else if (SeqCnt == 20) {
                      if ((TData.RS4.FromTargetBuffer[1][0] == false) && (TData.RS4.EEPROMPortpair[1] == PortPair)){ // buffer ready
                        ...
                        for (i = 0; i < 4; i++) {
                          ...
                          if ((TData.RS4.PortUsed >> (TData.RS4.EEPROMPortpair[1]+4)) & 0x01){  // if uses
                            i = 4;                                                          // break out
                          }
                        }
                      } else {
                  ...
                  if ((TData.RS4.ToTargerBuffer[0] == true ) && // Data ready for this AD card
                      (TData.RS4.ToTargerBuffer[1] == (PortPair+4))) {
...
*
*************************************************************************/
char CheckActionUart16552(char ch, unsigned int pointer) {
...
*
*************************************************************************/
void ReceiveWISStatus(char ch, unsigned int pointer) {
...
*
*************************************************************************/
void ReceiveWISXData(char ch, unsigned int pointer) {
...
*
*************************************************************************/
void ReceiveWISData(char ch, unsigned int pointer) {
...
*
*************************************************************************/
void ReceivePTOData(char ch, unsigned int pointer) {

    unsigned int pnt, ADCh;
    int packlen;
...
*
*************************************************************************/
void ReceiveRData(char ch, unsigned int pointer) {
    ...
    if (packlen > 0) {
      for (pnt = 0; pnt < packlen; pnt += 4) {
          ...
          if (ADCh < 14) {
            TData.RS4.TargetSetup1[(ch * 4) + PortPair][ADCh] = UART16552[ch].pRxBuffer[pointer + pnt + 1];
             *((int*)&TData.RS4.Raw[(ch * 4) + PortPair][ADCh]) = *((int*)&UART16552[ch].pRxBuffer[pointer + pnt + 2]) ; 
          }
      }
    }
}
...
*
*************************************************************************/
void ReceiveCData(char ch, unsigned int pointer) {
    ...
    if (packlen > 0) {
      for (pnt = 0; pnt < packlen; pnt += 4) {
          ...
          if (ADCh < 14) {
            TData.RS4.TargetSetup1[(ch * 4) + PortPair][ADCh] = UART16552[ch].pRxBuffer[pointer + pnt + 1];
           *((int*)&TData.RS4.Cal[(ch * 4) + PortPair][ADCh]) = *((int*)&UART16552[ch].pRxBuffer[pointer + pnt + 2]) ; 
         }
      }
    }
}
...
*
*************************************************************************/
void ReceiveCardStatus(char ch, unsigned int pointer) {
...
*
*************************************************************************/
void ReceiveConfigData(char ch, unsigned int pointer) {
...
*
*************************************************************************/
void ReceiveCardEEPROMData(char ch, unsigned int pointer) {
...
void Uart_BuildTail16552(char ch) {
    ...
    UART16552[ch].TxPkCnt++ ;                               // Count sent packages
    ...
    if (ch == 0) {                                           // Turn on TXE for channel
        ...
        Write16552(U0_MCR, Read16552(U0_MCR) | RTS);        // set RTS on
        OS_Delay(20);                                           // Wait (ms) for tx stable
        Write16552(U0_THR, UART16552[ch].pTxBuffer[0] );    // Send 1. byte
    } else if (ch == 1) {
        OS_RetriggerTimer(&TimerUART1);                    // and start timeout timer
        Write16552(U1_MCR, Read16552(U1_MCR) | RTS);        // set RTS on
        OS_Delay(20);                                           // Wait (ms) for tx stable
        Write16552(U1_THR, UART16552[ch].pTxBuffer[0] );      // Send 1. byte
    }
}
...
void Uart_BuildSndData(char ch) {
  ...
  for (i = 3; i < (TData.RS4.ToTargerBuffer[2]) + 3; i++) {
    UART16552[ch].pTxBuffer[UART16552[ch].TxFirst++] = TData.RS4.ToTargerBuffer[i] ;       /*command, send data */
  }
  TData.RS4.ToTargerBuffer[0] = false;            // data sent
}
...
void ReceivePacketUart16552(char ch) {
    ...
    case HEADER :
        if (UART16552[ch].RxLast >= 3) {             // receive header 
            ...
            if ((UART16552[ch].RxPacklen < ((int) RXSIZE_UART_16552-UART16552[ch].RxLast)) &&    /* chk. header */
...
*
*************************************************************************/
char CalcDSTxChecksumUART16552(char ch, unsigned int len) {
...
*
*************************************************************************/
int CalcDSRxChecksum16552(char ch) {
...
/*************************************************************************
*
*  Write data to the PC16552 UART
*
*************************************************************************/
void Write16552(char address, char data) {

    DDRA   =  0xff ;        // Port A data direction (out)
    PORTA  =  address ;     // Set address
    SetBit(PORTG, 0x04);          // ALE line high
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    ClrBit(PORTG, 0x04);         // ALE line low
    PORTA =  data ;         // Set data
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    ClrBit(PORTG, 0x01);         // nWR line low
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    SetBit(PORTG, 0x01);          // nWR line high
    DDRA  =  0x00 ;          // Port A data direction (in)
}

/*************************************************************************
*
*  Read data from the PC16552 UART
*
*************************************************************************/
char Read16552(char address) {
    ...
    asm("nop");
    asm("nop");
    ClrBit(PORTG, 0x02);         // nRD line low
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    asm("nop");
    indata = PINA;          // read the port
    SetBit(PORTG, 0x02);          // nRD line high
    return indata;
}
...
void EmptyRxBuf16552(char uartno) {
  ...
  case 0:
    do {
      Read16552(U0_RBR);
    } while (Read16552(U0_LSR) & DATA_READY);
    break;
  case 1:
    do {
      Read16552(U1_RBR);
    } while (Read16552(U1_LSR) & DATA_READY);
    break;
  }
}
...
void Init16552( char channel, unsigned long baud ) {
    ...
    case 0x00 :          
        ...
        Write16552(U0_LCR, 0x80);          // set DLAB
        Write16552(U0_DLL, (XTAL_CPU / 8 / 16 / baud ) & 0xff);
        Write16552(U0_DLM, ((XTAL_CPU / 8  / 16 / baud )>> 8) & 0xff);
        Write16552(U0_LCR, 0x03);          // reset DLAB and set 8,n,1
        Write16552(U0_FCR, 0x01);          // enable FIFO ..
        Write16552(U0_FCR, 0xC1);          // and set RXLevel to 14
        Write16552(U0_IER, 0x03);          // enable TX and RX int.
        Write16552(U0_MCR, 0x00);          // RTS off 
        break;
    case 0x01 :
        UART16552[channel].RxTimeout = RX_TO_TIME ;   // reset timeout
        UART16552[channel].pTxBuffer = TxBuf16552Ch1;
        UART16552[channel].pRxBuffer = RxBuf16552Ch1;
        Write16552(U1_LCR, 0x80);          // set DLAB
        Write16552(U1_DLL, (XTAL_CPU / 8 / 16 / baud ) & 0xff);
        Write16552(U1_DLM, ((XTAL_CPU / 8  / 16 / baud )>> 8) & 0xff);
        Write16552(U1_LCR, 0x03);          // reset DLAB and set 8,n,1
        Write16552(U1_FCR, 0x01);          // enable FIFO ..
        Write16552(U1_FCR, 0xC1);          // and set RXLevel to 14
        Write16552(U1_IER, 0x03);          // enable TX and RX int.
        Write16552(U1_MCR, 0x00);          // RTS off 
        break;
    default:
        break ;
    }    
