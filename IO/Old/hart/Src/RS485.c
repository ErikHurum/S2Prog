...
void ExtRS485Ctl(void) {
    ...
    while (1) {
        for (PortPair = 0; PortPair < 4; PortPair++) {        // loop through all ports
            ...
            EventStatus = OS_WaitSingleEventTimed(1, 1000);              // Wait for ready to send next
            if (EventStatus & 0x02) {
                if (PowerOnTimer >= POWER_ON_CNT) {
...
void ExtRS485Rec(void) {
    ...
    while (1) {
        ...
        if (portresp & 0x01) {                  // answer on ch 0
            ...
            if ((TData.RS4.PortUsed >> PortPair) & 0x01) {              // sensor on port 0?
                if ((TData.RS4.FailCnt[PortPair][0] & 0x0007) == 0x0004) {   //check if power shall be switched off
...
void ReceiveRadarInfo(char ch, unsigned short pointer) {
    TData.RS4.SerialNumber[(ch * 4) + PortPair] = atol((char*)&UART16552[ch].pRxBuffer[pointer + 0]);
