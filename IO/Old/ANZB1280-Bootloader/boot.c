...
Program type            : Bootloader - Size:4096words
...
// Bootloader callback function called when an
// internal operation is performed by the bootloader.
// Can be used, for example, to toggle the status
// of a LED to signal boot uploading.
// Note: boot_status values are defined in 'boot.h'
#pragma warn- // Disable warning if boot_status is not referenced in your code
...
#ifdef _WARNINGS_ON_
#pragma warn+ // Re-enable warnings
#endif
...
// Check for entering bootloader mode or
// launching the code from the application section.
// Bringing the PORTE Pin 7 to 1 will start the bootloader.
// Otherwise the application code will be executed.

// Prepare to read the state of the input pin.
// Ensure that PORTE Pin 7 is configured as an input.
DDRE&= ~(1<<7);

// Test pin state
if ((PINE & (1<<7))==0)
   {
   // Pin is 0, execute the code from the application section
   #asm("JMP  0x0")
   }
...
while (1)
      ...
      if (bootloader(boot_callback)==BOOT_OK)
         {
         // The bootloader has successfully uploaded the application
