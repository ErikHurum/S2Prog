...
#define MODBUS_RTU_TASK_STACK_SIZE      120
#define MODBUS_RTU_TASK_PRIORITY        70
#define MODBUS_RTU_TASK_ZB485_PRIORITY  70
...
static void mb_rtu_init_usart1(unsigned long baud);
...
static MODBUSTaskStruct         _mb_rtu_task1 = 
{
  .uart = MODBUSTaskUART_USART1,
  .uart_init = mb_rtu_init_usart1,
  .system_reset = 0,
};
...
mb_rtu_enable_rx(ModbusCTX* ctx)
{
  ...
  else if(ctx->uart_num == MODBUSTaskUART_USART1)
  {
    UCSR1B |= _BV(RXEN1)  | _BV(RXCIE1);
    ClrBit(MB_RS485_RTX_CONRTOL_PORT, _BV(MB_RS485_RTX_CONRTOL_USART1));
  }
}
...
mb_rtu_disable_rx(ModbusCTX* ctx, uint8_t uart, uint8_t gpio)
{
  ...
  else if(ctx->uart_num == MODBUSTaskUART_USART1)
  {
    ...
    if(gpio)
    {
      SetBit(MB_RS485_RTX_CONRTOL_PORT, _BV(MB_RS485_RTX_CONRTOL_USART1));
    }
  }
}
...
static void
usart1_rx_irq_handler(void)
{
  MODBUSTaskStruct*     task = &_mb_rtu_task1;
  ModbusCTX*            ctx = &task->mb_rtu_ctx;
  uint8_t               sra,
                        b;
...
__interrupt void USART1_RX_vect_handler(void)
{
  ENTER_IRQ;
  
  usart1_rx_irq_handler();
  
  LEAVE_IRQ;
}

static void
usart1_udre_irq_handler(void)
{
  MODBUSTaskStruct*     task = &_mb_rtu_task1;
  ModbusCTX*            ctx = &task->mb_rtu_ctx;
  uint8_t               b;
...
__interrupt void USART1_UDRE_vect_handler(void)
{
  ENTER_IRQ;
  
  usart1_udre_irq_handler();
  
  LEAVE_IRQ;
}

static void
usart1_txc_irq_handler(void)
{
  MODBUSTaskStruct*     task = &_mb_rtu_task1;
  ModbusCTX*            ctx = &task->mb_rtu_ctx;
...
__interrupt void USART1_TX_vect_handler(void)
{
  ENTER_IRQ;
  
  usart1_txc_irq_handler();
...
static void
mb_rtu_init_usart1(unsigned long baud)
{
  uint16_t baud_setting = (XTAL_CPU / 4 / baud - 1) / 2;
  ...
  UCSR1C      = _BV(UCSZ10) | _BV(UCSZ11);                  // 8 bit data, no parity, 1 stop-bit
...
static inline uint16_t
mb_rtu_get_rsp_delay(uint8_t port)
{
  extern uint16_t adc_task_get_rsp_delay(char port);
  extern uint16_t rs485_task_get_rsp_delay(uint8_t port);

  uint16_t rsp_delay = 0;

  if (UnitID == 0x00)
  {
    rsp_delay = rs485_task_get_rsp_delay(port);
  }
  else
  {
    rsp_delay = adc_task_get_rsp_delay(port);
  }
...
MODBUS_RTU_State_Handling(MODBUSTaskStruct* self)
{
  ...
  {
    uint16_t rsp_delay = mb_rtu_get_rsp_delay(self->uart);
...
MODBUS_RTU_State_Responding(MODBUSTaskStruct* self)
{
  ...
  {
    //
    // we don't have to care about race condition here
    //
...
MODBUS_RTU_Task_Main(void)
{
  ...
  //
  // wait for ADC/RS485 task to finish initialization
  //
  OS_WaitEvent(0x80);
...
{
  uint8_t priority = MODBUS_RTU_TASK_PRIORITY;

  if(UnitID == 0x00)
  {
    // ZB485
    priority = MODBUS_RTU_TASK_ZB485_PRIORITY;
  }

  OS_CREATETASK(&_mb_rtu_task0.tcb, "MBRTU0", MODBUS_RTU_Task_Main, priority + 5, _mb_rtu_task0.stack);
  OS_CREATETASK(&_mb_rtu_task1.tcb, "MBRTU1", MODBUS_RTU_Task_Main, priority, _mb_rtu_task1.stack);
}
...
modbus_rtu_notify_go(void)
{
  ...
  OS_SignalEvent(0x80, &_mb_rtu_task1.tcb);
}
