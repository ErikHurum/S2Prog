...
mb_rtu_enable_rx(ModbusCTX* ctx)
{
  ...
  else if(ctx->uart_num == 1)
  {
    UCSR1B |= _BV(RXEN1)  | _BV(RXCIE1);
    MB_RS485_RTX_CONRTOL_PORT &= ~_BV(MB_RS485_RTX_CONRTOL_USART1);
  }
}
...
{
  ModbusCTX*            ctx = &_mb_rtu_ctx_uart1;
  uint8_t               sra,
                        b;
  ...
  mb_rtu_put_data(ctx, b);
  set_event(EVENTS_FLAG_UART1_RX);
}
...
__interrupt void USART1_TX_vect_handler(void)
{
  ...
  ctx->tx_frames++;
  set_event(EVENTS_FLAG_UART1_TXC);
}
...
modbus_uart_evt(unsigned char evt)
{
  ...
  case EVENTS_FLAG_UART1_RX:
    start_timer(timer_num_1, 4, EVENTS_FLAG_UART1_T35);
    break;

  case EVENTS_FLAG_UART1_T35:
    mb_rtu_disable_rx(&_mb_rtu_ctx_uart1, 1, 0);
    if(mb_rtu_start_handling_rx_frame(&_mb_rtu_ctx_uart1) == TRUE)
    {
      mb_rtu_disable_rx(&_mb_rtu_ctx_uart1, 0, 1);
      mb_rtu_start_tx(&_mb_rtu_ctx_uart1);
    }
    else
    {
      mb_rtu_enable_rx(&_mb_rtu_ctx_uart1);
    }
    break;

  case EVENTS_FLAG_UART1_TXC:
    mb_rtu_enable_rx(&_mb_rtu_ctx_uart1);
    break;
  }
}
...
modbus_init(void)
{
  ...
  register_event_handler(EVENTS_FLAG_UART1_RX, modbus_uart_evt);
  register_event_handler(EVENTS_FLAG_UART1_T35, modbus_uart_evt);
  register_event_handler(EVENTS_FLAG_UART1_TXC, modbus_uart_evt);

  init_modbus_context(&_mb_rtu_ctx_uart0, 0, dev_addr);
  mb_rtu_init_usart0(baud_rate);
  mb_rtu_enable_rx(&_mb_rtu_ctx_uart0);

  init_modbus_context(&_mb_rtu_ctx_uart1, 1, dev_addr);
  mb_rtu_init_usart1(baud_rate);
  mb_rtu_enable_rx(&_mb_rtu_ctx_uart1);
}
