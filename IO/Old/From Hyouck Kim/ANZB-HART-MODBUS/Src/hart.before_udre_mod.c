...
#define HART_TRANSACTION_TIMEOUT        1000
...
#define HART_MUX_ON                     1
#define HART_MUX_OFF                    0
...
static void hart_control_hart_mux(uint8_t mux_num, uint8_t on_off);
...
////////////////////////////////////////////////////////////////////////////////
static void
hart_control_hart_mux(uint8_t mux_num, uint8_t on_off)
{
  if(mux_num == 0)
  {
    if(on_off)
...
hart_irq_tx_next_byte(HartTaskStruct* task)
{
  ...
  if(tndx >= task->tx_len)
  {
    ...
    OS_SignalEvent(HART_EVENT_TX_COMPLETE, &task->task);
    return;
  }
...
hart_start_uart(HartTaskStruct* task)
{
  ...
  case 0:
    ...
    UCSR2C      = _BV(UCSZ20) | _BV(UCSZ21) | USART2_ODD_PARITY | USART2_1_STOP_BIT;      // 8 bit data, no parity, 1 stop-bit
  ...
  case 1:
    ...
    UCSR3C      = _BV(UCSZ30) | _BV(UCSZ31) | USART3_EVEN_PARITY | USART3_1_STOP_BIT;      // 8 bit data, no parity, 1 stop-bit
...
demo1_hart_make_request(HartTaskStruct* task, uint8_t current_chnl)
{
...
#if 1
  task->hart_encoder.tx_buf[ndx++] = 0xff;
  task->hart_encoder.tx_buf[ndx++] = 0xff;
  task->hart_encoder.tx_buf[ndx++] = 0xff;
  task->hart_encoder.tx_buf[ndx++] = 0xff;
  task->hart_encoder.tx_buf[ndx++] = 0xff;

  task->hart_encoder.tx_buf[ndx++] = 0x82;

  task->hart_encoder.tx_buf[ndx++] = 0x80;
  task->hart_encoder.tx_buf[ndx++] = 0x00;
  task->hart_encoder.tx_buf[ndx++] = 0x00;
  task->hart_encoder.tx_buf[ndx++] = 0x00;
  task->hart_encoder.tx_buf[ndx++] = 0x00;

  task->hart_encoder.tx_buf[ndx++] = 0x03;

  task->hart_encoder.tx_buf[ndx++] = 0x00;
  task->hart_encoder.tx_buf[ndx++] = 0x01;
  ...
  task->hart_encoder.tx_buf[ndx++] = 0x02;
#endif

  task->hart_encoder.tx_ndx = ndx;
#endif
}
...
demo1_hart_handle_rx_complete(HartTaskStruct* task, uint8_t current_chnl)
{
  ...
  if(task->hart_parser.status_ptr[0] != 0)
  {
...
do_hart_transaction(HartTaskStruct* task, uint8_t current_chnl, HartTransaction* t)
{
  ...
  OS_WaitEvent(HART_EVENT_TX_COMPLETE);
  OS_Delay(1);
  hart_control_hart_rts(task->modem_no, HART_DISABLE_MODULATOR, HART_RTS_CONTROL_FROM_TASK);
...
hart_hw_init(void)
{
  ...
  DDRH |= 0x0e;
  DDRJ |= 0x0e;
  hart_control_hart_mux(0, HART_MUX_ON);                       // turn on mux0
  hart_control_hart_mux(1, HART_MUX_ON);                       // turn on mux1
  hart_control_hart_rts(0, HART_ENABLE_MODULATOR, HART_RTS_CONTROL_FROM_TASK);       // enable modulator for modem0
  hart_control_hart_rts(1, HART_ENABLE_MODULATOR, HART_RTS_CONTROL_FROM_TASK);       // enable modulator for modem1
