...
void Init_CPU( void )
{
  ...
  // hkim
  // fucking out of mind to enable global interrupt here
...
void Init_IO( void )
{
  ...
  case AN_ZBHART://EHSMarkAN_ZBANA:                  // AN-ZBANA
    ...
    DDRE   =  0x0E ;      // Port E data direction, all out except rxd(bit 0) and bit 4,5,6,7 ( for address dial)
  ...
  case AN_ZBANA://AN_ZBHART:EHSMark
    hart_gpio_init();
    break;
...
void Init_TMR( void ) {
  ...
  case AN_ZBHART://EHSMark AN_ZBANA:                  // AN-ZBANA
    TCCR0A  =  0x99 ;             // Timer control register 0 
    OCR0A    =  7 ;               // Timer output compare register 0, 1MHZ //16MHz xtal
    break;
  
  case AN_ZBANA://EHSMark AN_ZBHART:
    //
