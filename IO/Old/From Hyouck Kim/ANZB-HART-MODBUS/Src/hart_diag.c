...
//
////////////////////////////////////////////////////////////////////////////////
static OS_STACKPTR int    _hart_diag_task_stack[80];
...
hart_diag_set_diag_value(hart_channel_t chnl, uint16_t raw_adc) {
    ...
    float       current = (voltage * 10.0)/1000.0f;       // voltage in unit V, 0V -> 0mA, 2V -> 20mA.
                                                          // calculated current in A unit.
    ...
    if (chnl <= hart_channel_11) {
        ...
        hchannel->ma_feedback = current;  //current_final;
