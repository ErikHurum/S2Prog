...
#define HART_TRANSACTION_TIMEOUT        1500
...
{
#if 0
  uint8_t   addr[5] = {80, 0, 0, 0, 0};
  uint8_t   cmd = 3;                    // no fucking idea what to use
...
hart_hw_init(void)
{
  ...
  DDRK    |= 0x07;
  DDRC    |= 0x01;
  PORTC   &= ~0x01;     // RTS low. enable modulator

  PORTK   &= ~0x07;     // always channel 0
