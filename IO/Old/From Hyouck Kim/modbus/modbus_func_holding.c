...
modbus_func_handler_write_holding_register(ModbusCTX* ctx, uint8_t addr, uint8_t * pucFrame, uint16_t * usLen)
{
  ...
  if( *usLen == ( MB_PDU_FUNC_WRITE_SIZE + MB_PDU_SIZE_MIN ) )
  {
    ...
    eRegStatus = modbus_user_holding_cb(ctx, addr, &pucFrame[MB_PDU_FUNC_WRITE_VALUE_OFF], usRegAddress, 1, MB_REG_WRITE );
...
modbus_func_handler_write_multiple_holding_registers(ModbusCTX* ctx, uint8_t addr,uint8_t * pucFrame, uint16_t * usLen)
{
  ...
  if( *usLen >= ( MB_PDU_FUNC_WRITE_MUL_SIZE_MIN + MB_PDU_SIZE_MIN ) )
  {
    ...
    ( ucRegByteCount == ( uint8_t ) ( 2 * usRegCount ) ) )
    {
      ...
      eRegStatus = modbus_user_holding_cb(ctx, addr, &pucFrame[MB_PDU_FUNC_WRITE_MUL_VALUES_OFF], usRegAddress, usRegCount, MB_REG_WRITE );
    ...
    */
    if( ( usRegCount >= 1 ) && ( usRegCount <= MB_PDU_FUNC_READ_REGCNT_MAX ) )
    {
      ...
      *usLen += 1;
      ...
      eRegStatus = modbus_user_holding_cb(ctx, addr, pucFrameCur, usRegAddress, usRegCount, MB_REG_READ );
      /* If an error occured convert it into a Modbus exception. */
      if( eRegStatus != MB_ENOERR )
      {
        eStatus = prveMBError2Exception( eRegStatus );
...
modbus_func_handler_read_write_multiple_holding_registers(ModbusCTX* ctx, uint8_t addr, uint8_t * pucFrame, uint16_t * usLen)
{
  ...
  if( *usLen >= ( MB_PDU_FUNC_READWRITE_SIZE_MIN + MB_PDU_SIZE_MIN ) )
  {
    ...
    {
      /* Make callback to update the register values. */
      eRegStatus = modbus_user_holding_cb(ctx, addr, &pucFrame[MB_PDU_FUNC_READWRITE_WRITE_VALUES_OFF], usRegWriteAddress, usRegWriteCount, MB_REG_WRITE );
      ...
      if( eRegStatus == MB_ENOERR )
      {
        ...
        *usLen += 1;
        ...
        eRegStatus = modbus_user_holding_cb(ctx, addr, pucFrameCur, usRegReadAddress, usRegReadCount, MB_REG_READ );
        if( eRegStatus == MB_ENOERR )
        {
