...
extern char ProgramData1281ANZB[];
extern int ProgramData1281ANZBSize;
...
extern AlarmCriticalComFailure *CriticalComAlarm;
...
extern float ConvertToSi(AnsiString NewValue, int Unit, int &Error, float Min,float Max,float OldVal=0.0);
...
extern AnsiString KWrdEnableDisable(bool Enable);
...
extern void strncpy(char* Dest, char* Source, int MaxLength);
    #endif
...
extern bool IsInfinite(float A);
extern bool IsNan(float A);
...
extern vector<PRogramObjectBase*> GetReadPermissionVector(vector<PRogramObjectBase*>MyVect);
extern vector<PRogramObjectBase*> GetWritePermissionVector(vector<PRogramObjectBase*>MyVect);
