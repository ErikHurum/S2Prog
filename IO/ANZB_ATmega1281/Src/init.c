...
#if ANZBANA_V8 == 1
    DDRA    =  0x00 ;      // Port B data direction, bit 0, 4-7 out
    PORTA   =  0x3;
...
void Init_IO( void ) {
    ...
    case AN_ZBANA:                  // AN-ZBANA
#if ANZBANA_V8 == 1
        DDRA   =  0x00 ;      // Port A data direction in ID0, ID1
        PORTA  =  0x03 ;      // Port A data
        DDRB   =  0xF7 ;      // Port B data direction, bit 2 in, bit 0-1 and 3-7 out
        PORTB  =  0x08 ;      // Port B data  Pin 3 pull-up 
        DDRC   =  0xFF ;      // Port C data direction, all out
        PORTC  =  0x00 ;      // Port C data
        DDRD   =  0x02 ;      // Port D data direction, all in except output to ADC CLC
        PORTD  =  0xf1 ;      // Port D data pullup on int fro AD and ADC CLC
        DDRE   =  0xFE ;      // Port E data direction, all out except rxd
        PORTE  =  0x00 ;      // Port E data 
        DDRF   =  0x00 ;      // Port F data direction, used for ADC
        PORTF  =  0x00 ;      // Port F data 
        DDRG   =  0x08 ;      // Port G data direction, bit 3 out
        PORTG  =  0x30 ;      // Port G data pullup PG4, PG5 
        SPI_Init();
#else
