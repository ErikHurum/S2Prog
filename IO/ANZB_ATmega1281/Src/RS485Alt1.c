...
#define MAX_TIME_BETWEEN_BYTE	100
...
void ExtRS485Ctl(void) {
    ...
    EmptyRxBuf16552(1);                            // Empty UART channel 0
    ...
    while (1) {
        for (PortPair = 0; PortPair < 4; PortPair++) {        // loop through all ports
            ...
            SetBit(PORTE, 0xf0);
            ClrBit(PORTE, 0x10 << PortPair);           // set mux port

            // Wait for all events
            char EventStatus = OS_WaitEventTimed(0x03, 1000);              // Wait for ready to send next for both channels
            if (EventStatus & 0x03) {
                if (PowerOnTimer >= POWER_ON_CNT) {
...
void SendRequestToExtDevice(char Ch){
    static char SeqCnt = 0;
    static char PowerOnTimer = POWER_ON_CNT;                    // do not make a unwanted power on sequence in the starting phase
    char EventStatus;
    char AskMD[2];
...
char CheckActionUart16552(char ch, unsigned short pointer) {
    ...
    char retval = true;
    char Cmd = UART16552[ch].pRxBuffer[pointer];
    switch (Cmd) {    // check action
...
*
*************************************************************************/
char ANPRO1_UnpackPacket(char ch) {
    unsigned  pointer = 4;
    char Cmd = UART16552[ch].pRxBuffer[pointer];        // check action
    char retval = true;
    switch (Cmd) {    // check action
    case ANP1_REP_MP_DATA :                              // Measured pressure data
    case ANP1_REP_MT_DATA :                              // Measured temperature data
...
char ANPRO1_IO_Receive(int Ch, UARTDataType *UART16552Ptr) {
    char *RxBuf = UART16552Ptr->pRxBuffer;
    const ANPRO1_PacketHeading *pPH =  (ANPRO1_PacketHeading *)RxBuf;
    OS_MAILBOX *RxMailBoxPtr = &UART16552Ptr->RxMailBox;
    char Data;
    ...
    //memset(RxBuf,0,RXSIZE_UART);
    char Echo          = false;
    do {
        do {
            dataAvailable = !OS_GetMailTimed(RxMailBoxPtr, &Data, 200); // Wait for the response from ANSGCNV
            //WriteByte(Data&0xFF);
        }  while (dataAvailable && Data != ANPRO1_SYN);
        if (dataAvailable) {
            // We have received sufficent SYNC
            // A minimum of 2 SYN to accept start
            //Message("First SYN");
            do {
                dataAvailable = !OS_GetMailTimed(RxMailBoxPtr, &Data, MAX_TIME_BETWEEN_BYTE);
                if (dataAvailable && (Data == ANPRO1_SYN)) SyncCnt++;
            } while (dataAvailable && (Data == ANPRO1_SYN));
            if (dataAvailable && SyncCnt>= 2) {
                // We allready have one byte of the heading. Move it to the RxBuffer and
                // receive the rest of the heading
                char *RxPtr = RxBuf;
                RxBuf[0] = Data;
                RxPtr++;
                for (unsigned i = 1; dataAvailable && i < sizeof(ANPRO1_PacketHeading); i++) {
                    dataAvailable = !OS_GetMailTimed(RxMailBoxPtr, &Data, MAX_TIME_BETWEEN_BYTE);
                    if (dataAvailable) {
                        *RxPtr++ = Data;
                    }
                ...
                if (dataAvailable) {
                    ...
                    if (DataSize > 3 && DataSize < RXSIZE_UART) {
                        ...
                        if (dataAvailable) {
                            char EOTByte = *((char *)&RxBuf[DataSize - 1]);
                            if (EOTByte == ANPRO1_EOT) {
                                ...
                                char  PacketCheckSum = *((char *)&RxBuf[DataSize - 2]);
                                if (PacketCheckSum == CSum) {
                                    if (pPH->TxAddress != MY_SGCNV_ADDR) {
                                        Usart16552CheckPackage(Ch);
...
*
*************************************************************************/
void ExtRS485Rec(void *pVoid) {
    char Ch = (int)pVoid;
    UARTDataType *UART16552Ptr = &UART16552[Ch];
    Init16552(Ch, 9600);   // Init UART channel 
    
    SetBit(EIMSK , __BIT_MASK(Ch));            // External interrupt mask register, int on

    OS_Delay(UART_START_DELAY + 800);           // Waiting for target startup and power on sequence

    while (1) {
        char RS485Channel = PortPair + 4 * Ch;
        char EventStatus = OS_WaitEventTimed(0x01, 1000);              // Wait for TxComplete
        //EmptyRxBuf16552(Ch);
        char PacketOk = ANPRO1_IO_Receive(Ch, UART16552Ptr);  // handle incomming package

        if (PacketOk) {                  // answer on ch 0
            TData.RS4.FailCnt[RS485Channel][0] = 0; // Reset counter
            TData.RS4.IOUnitStatus[RS485Channel] &= ~COMFAIL_BIT; // No error
            UART16552Ptr->RxFaultCnt = 0;          // OK package on 16552 channel
        } else {
            UART16552Ptr->RxFaultCnt++;          // Fault in package on 16552 channel
            TData.RS4.FailCnt[RS485Channel][1]++;      // no answer total counter
            if (TData.RS4.FailCnt[RS485Channel][0]++ > FAILCNT_ERROR) {
                TData.RS4.IOUnitStatus[RS485Channel] |= COMFAIL_BIT;      // report fault
            }
            if ((TData.RS4.PortUsed >> RS485Channel) & 0x01) {              // sensor on port 0?
                if ((TData.RS4.FailCnt[RS485Channel][0] & 0x0007) == 0x0004) {   //check if powqer shall be switched off
        ...
        OS_SignalEvent(1 << Ch, &TCB_RS485Ctl);
    }
}
...
void Uart_BuildTail16552(char ch) {
    ...
    if (ch == 0) {                                           // Turn on TXE for channel
        ...
        OS_Delay(2);                                           // Wait (ms) for tx stable
...
void Init16552(char channel, unsigned long baud) {
    // Create mailbox
    OS_CreateMB(&UART16552[channel].RxMailBox, 1, MBUF_SIZE, &UART16552[channel].MailBuf);
    ...
    case 0x00 :
        ...
        U0_FCR = 0xC1;          // and set RXLevel to 8=0x81, 14 = 0xC1
    ...
    case 0x01 :
        ...
        U1_FCR = 0xC1;          // and set RXLevel to 8=0x81, 14 = 0xC1
