...
/////////////////////////////////////////////////////////////////////////
#pragma hdrstop
#include <windows\sdk\Winsock2.h>
...
#define Anpro_Net_Event_Urgent_PreEncoded       3
   ...
   : _tcp_reader(Anpro_Net::anpro_tcp_read_callback, this)
{
   ...
   _server_tcp_addr.sin_port          = htons(16000);
   ...
   //_server_udp_addr.sin_addr.s_addr   = inet_addr("192.168.10.171");
   _server_udp_addr.sin_port          = htons(15534);
}
...
void
Anpro_Net::send_urgent_preencoded(char* msg, int size)
{
   if(!_urgent_Q)
      return;

   _urgent_Q->PostMessage(msg, size);
   notify_anpro_thread(Anpro_Net_Event_Urgent_PreEncoded);
}
...
void
Anpro_Net::start(const char* ip_addr)
{
   memset(&_server_tcp_addr, 0, sizeof(_server_tcp_addr));
   _server_tcp_addr.sin_family        = AF_INET;
   _server_tcp_addr.sin_addr.s_addr   = inet_addr(ip_addr);
   _server_tcp_addr.sin_port          = htons(16000);
...
/*
   ...
   _udp_sock   = init_udp_socket(15534);
   */
}
...
Anpro_Net::OnControlRX(void)
{
   ...
   case Anpro_Net_Event_Urgent_PreEncoded:
      OnUrgentPreEncodedTXReq();
      break;
...
void
Anpro_Net::OnUrgentPreEncodedTXReq(void)
{
   int  size;
   ...
   tx_over_tcp(&_q_msg_buf[ANPRO10_PREAMBLE_SIZE], size - ANPRO10_PREAMBLE_SIZE);
}
...
{
	extern TTXUFlashProg *TXUFlashProg;

	ANPRO10_UnpackPacket((U8*)msg, TXUFlashProg->WinUartPtr);
}
...
{
   Anpro_Net*        apnet = (Anpro_Net*)lpParameter;
   int               max_fd,
                     ret;
   fd_set            rset, wset;
   struct timeval    tv;
