...
public:
   Anpro_Net();
   virtual ~Anpro_Net();
   void send_normal(char* msg, int size);
   void send_urgent(char* msg, int size);
   void send_urgent_preencoded(char* msg, int size);
   void start(void);
   void start(const char* ip_addr);
   void stop(void);
...
protected:
   ...
   virtual void OnUrgentPreEncodedTXReq(void);
