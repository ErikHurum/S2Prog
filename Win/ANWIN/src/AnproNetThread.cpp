...
//
/////////////////////////////////////////////////////////////////////////
AnproNetThread::Anpro_Net()
   : _tcp_reader(AnproNetThread::anpro_tcp_read_callback, this)
{
}
...
void
AnproNetThread::send_normal(char* msg, int size)
{
   if(!_normal_Q)
      return;
...
void
AnproNetThread::send_urgent(char* msg, int size)
{
   if(!_urgent_Q)
      return;
...
AnproNetThread::start(void)
{
   ...
   _anpro_net_thread = ::CreateThread(
         ...
         0,                               // stack size
         AnproNetThread::anpro_net_thread,     // thread routine
         this,                            // lpParameter
         0,                               // creation flags
         NULL);                           // threadId
...
AnproNetThread::stop(void)
{
   ...
   ::WaitForSingleObject(_anpro_net_thread, INFINITE);
...
/////////////////////////////////////////////////////////////////////////
void
AnproNetThread::anpro_thread_init(void)
{
...
void
AnproNetThread::anpro_thread_deinit(void)
{
...
/////////////////////////////////////////////////////////////////////////
void
AnproNetThread::init_anpro_tcp(void)
{
   if(_tcp_sock != INVALID_SOCKET)
   {
      closesocket(_tcp_sock);
...
void
AnproNetThread::init_anpro_udp(void)
{
   if(_udp_sock != INVALID_SOCKET)
   {
      ::closesocket(_udp_sock);
...
void
AnproNetThread::try_connect_to_server(void)
{
...
void
AnproNetThread::notify_anpro_thread(unsigned char event)
{
   ::send(_pipes[1], &event, 1, 0);
}

unsigned char
AnproNetThread::read_notify_event(void)
{
   int   ret;
   unsigned char cmd;
...
void
AnproNetThread::anpro_select_add_set(fd_set* set, int fd, int* maxfd)
{
...
void
AnproNetThread::anpro_thread_pre_select(fd_set* rset, fd_set* wset, int* maxfd)
{
   *maxfd = 0;
...
void
AnproNetThread::anpro_thread_post_select(fd_set* rset, fd_set* wset)
{
...
void
AnproNetThread::anpro_thread_check_timer(void)
{
   if(_exit_flag)
   {
      return;
...
/////////////////////////////////////////////////////////////////////////
int
AnproNetThread::anpro_encode_message(bool is_urgent, unsigned char* msg, int size, unsigned char* buffer)
{
...
/////////////////////////////////////////////////////////////////////////
void
AnproNetThread::tx_over_tcp(char* pkt, int pkt_len)
{
   int            nsent = 0,
                  ret;
...
void
AnproNetThread::tx_over_udp(char* pkt, int pkt_len)
{
   int ret;
...
/////////////////////////////////////////////////////////////////////////
void
AnproNetThread::OnControlRX(void)
{
   //
...
void
AnproNetThread::OnTCPConnected(void)
{
...
void
AnproNetThread::OnTCPDisconnected(void)
{
   closesocket(_tcp_sock);
...
void
AnproNetThread::OnNormalTXReq(void)
{
   int  size;
   int  pkt_len;
...
void
AnproNetThread::OnUrgentTXReq(void)
{
   int  size;
   int  pkt_len;
...
void
AnproNetThread::OnNormalRX(char* msg, int len)
{
   ANPRO10_UDP_Receive(msg);
}

void
AnproNetThread::OnUrgentRX(char* msg, int len)
{
   ANPRO10_UDP_Receive(msg);
}

int
AnproNetThread::anpro_tcp_read_callback(Anpro10_Reader* reader, unsigned char* msg, int len, void* cb_arg)
{
   Anpro_Net* apnet = (Anpro_Net*)cb_arg;

   OnUrgentRX((char*)msg, len);
   return 0;
}
...
DWORD WINAPI
AnproNetThread::anpro_net_thread(LPVOID lpParameter)
{
   ...
   while(!_exit_flag)
   {
	  ...
	  anpro_thread_pre_select(&rset, &wset, &max_fd);

	  ret = ::select(max_fd + 1, &rset, &wset, NULL, &tv);
	  if(ret > 0)
	  {
		 anpro_thread_post_select(&rset, &wset);
	  }

	  anpro_thread_check_timer();
   }
...
//
//      void __fastcall AnproNetThread::UpdateCaption()
//      {
//        Form1->Caption = "Updated in a thread";
//      }
//---------------------------------------------------------------------------

__fastcall AnproNetThread::AnproNetThread(bool CreateSuspended)
	: TThread(CreateSuspended), _tcp_reader(anpro_tcp_read_callback, this)
{
   _anpro_net_thread = NULL;
   _tcp_sock         = INVALID_SOCKET;
   _udp_sock         = INVALID_SOCKET;
...
//---------------------------------------------------------------------------
void __fastcall AnproNetThread::Execute()
{
	NameThreadForDebugging(System::String(L"AnproNet Thread"));
	...
	try {
		...
		while ( !Terminated ) {
			...
			int ret = ::select(max_fd + 1, &rset, &wset, NULL, &tv);
			if(ret > 0)
			{
				anpro_thread_post_select(&rset, &wset);
			}
