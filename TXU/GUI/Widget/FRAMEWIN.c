...
static void _OnTouch(FRAMEWIN_Handle hWin, FRAMEWIN_Obj* pObj, WM_MESSAGE* pMsg) {
  ...
  if (pMsg->Data.p) {  /* Something happened in our area (pressed or released) */
    if (pState->Pressed) {
      ...
      WM_BringToTop(hWin);
      if ((pObj->Flags & FRAMEWIN_SF_MOVEABLE) && (pObj->Flags & FRAMEWIN_SF_DRAGGING)) {
        int LimitTop;
...
*       _Paint  (Frame)
*/
static void _Paint(FRAMEWIN_Handle hWin, FRAMEWIN_Obj * pObj) {
#if 1
...
*       _OnPidStateChange
*/
static void _OnPidStateChange(FRAMEWIN_Handle hObj, FRAMEWIN_Obj * pObj, WM_MESSAGE * pMsg) {
...
/*********************************************************************
*
*        Exported API routines:  FRAMEWIN_OwnerDraw
*
**********************************************************************
*/
/*********************************************************************
...
int FRAMEWIN_OwnerDraw(const WIDGET_ITEM_DRAW_INFO * pDrawItemInfo) {
  ...
  pObj = (FRAMEWIN_Obj *)GUI_ALLOC_h2p(pDrawItemInfo->hWin);
  pPos = (POSITIONS *)pDrawItemInfo->p;
  Index = (pObj->Flags & FRAMEWIN_SF_ACTIVE) ? 1 : 0;
  if (pObj->hText) {
    pText = (const char*) GUI_ALLOC_h2p(pObj->hText);
  } else {
...
void FRAMEWIN_Callback (WM_MESSAGE *pMsg) {
  ...
  case WM_PID_STATE_CHANGED:
    _OnPidStateChange(hWin, pObj, pMsg);
    break;
                                  ...
                                  int WinFlags, int ExFlags, int Id, const char* pTitle, WM_CALLBACK* cb)
{
  ...
  if (hObj) {
    ...
    pObj->pfDrawItem     = FRAMEWIN_OwnerDraw;
    FRAMEWIN__CalcPositions(pObj, &Pos);
