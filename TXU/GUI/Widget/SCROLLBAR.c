...
*       _DeleteTimer
*/
static void _DeleteTimer(SCROLLBAR_Obj * pObj) {
...
*       _CreateTimer
*/
static void _CreateTimer(SCROLLBAR_Handle hObj, SCROLLBAR_Obj * pObj, int Dir, int Period) {
  pObj->hTimer    = WM_CreateTimer(hObj, TIMER_ID, Period, 0);
  pObj->TimerStep = Dir;
}
...
static void _OnTouch(SCROLLBAR_Handle hObj, SCROLLBAR_Obj* pObj, WM_MESSAGE*pMsg) {
  ...
  if (pMsg->Data.p) {  /* Something happened in our area (pressed or released) */
    if (pState->Pressed) {
      ...
      if (TimerStep) {
        if (!pObj->TimerStep) {
          _CreateTimer(hObj, pObj, TimerStep, 400);
        }
      } else {
        if (pObj->TimerStep) {
          _DeleteTimer(pObj);
...
SCROLLBAR_Obj * SCROLLBAR_h2p(SCROLLBAR_Handle h) {
  SCROLLBAR_Obj * p = (SCROLLBAR_Obj *)GUI_ALLOC_h2p(h);
  if (p) {
...
void SCROLLBAR_Callback (WM_MESSAGE *pMsg) {
  ...
  SCROLLBAR_Obj* pObj;
  hObj = pMsg->hWin;
  pObj = (SCROLLBAR_Obj *)GUI_ALLOC_h2p(hObj); /* Don't use use WIDGET_H2P because WIDGET_INIT_ID() has not be called at this point */
