...
*/
  ...
  #define MULTIEDIT_H2P(h) MULTIEDIT_h2p(h)
  #define MULTIEDIT_INIT_ID(p) p->DebugId = MULTIEDIT_ID
#else
  #define MULTIEDIT_H2P(h) (MULTIEDIT_OBJ *)GUI_ALLOC_h2p(h)
...
static void _ManageAutoScrollV(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj) {
  if (pObj->Flags & MULTIEDIT_SF_AUTOSCROLLBAR_V) {
    ...
    if (WM_SetScrollbarV(hObj, IsRequired) != IsRequired) {
      pObj = (MULTIEDIT_OBJ *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required */
...
static int _GetCursorSizeX(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, int * pIsRTL) {
  if (_IsOverwriteAtThisChar(hObj, pObj)) {
      ...
      if (pObj->Flags & MULTIEDIT_SF_PASSWORD) {
        ...
        const char *pText;
        pText  = (const char *)GUI_ALLOC_h2p(pObj->hText);
        Char = GUI__BIDI_GetLogChar(pText, pObj->NumChars, pObj->CursorPosChar);
        *pIsRTL = GUI__BIDI_GetCharDir(pText, pObj->NumChars, pObj->CursorPosChar);
        return GUI_GetCharDistX(Char);
      }
    #else
      const char *pText;
      *pIsRTL = 0;
...
*/
static int _IncrementBuffer(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, unsigned AddBytes) {
  WM_HMEM hNew;
  int NewSize;
...
*/
static int _IsSpaceInBuffer(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, int BytesNeeded) {
  int NumBytes = 0;
  if (pObj->hText) {
  ...
  if (BytesNeeded > 0) {
    if (!_IncrementBuffer(hObj, pObj, BytesNeeded + MULTIEDIT_REALLOC_SIZE)) {
      return 0;
    }
  }
  return 1;
...
static int _InsertChar(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, U16 Char) {
  if (_IsCharsAvailable(pObj, 1)) {
    ...
    if (_IsSpaceInBuffer(hObj, pObj, BytesNeeded)) {
      int CursorOffset;
      char* pText;
      pObj = (MULTIEDIT_OBJ *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required */
      pText = (char*) GUI_ALLOC_h2p(pObj->hText);
...
*       _InvertCursor
*/
static void _InvertCursor(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ * pObj) {
...
static int _AddKey(MULTIEDIT_HANDLE hObj, U16 Key) {
  ...
  int OldScrollPosH;
    ...
    int OldCursorPosX;
    int NoChanges = 0;
    #define SET_NO_CHANGES() NoChanges = 1
  #else
    #define SET_NO_CHANGES()
  #endif
  if (WM__IsEnabled(hObj) == 0) {
    ...
    pScrollFunc  = (Key == GUI_KEY_UP) ? SCROLLBAR_Dec : (Key == GUI_KEY_DOWN) ? SCROLLBAR_Inc : NULL;
    if (pScrollFunc) {
  ...
  case GUI_KEY_BACKSPACE:
    if (!(pObj->Flags & MULTIEDIT_SF_READONLY)) {
      if (pObj->CursorPosChar > pObj->NumCharsPrompt) {
        _SetCursorPos(hObj, pObj, pObj->CursorPosChar - 1);
        _DeleteChar(hObj, pObj); /* Pointer recalculation required, will be done later in this routine */
      }
      r = 1;               /* Key has been consumed */
    }
    break;
...
static void _SetText(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ * pObj, const char * pNew) {
  ...
  if (_IsCharsAvailable(pObj, NumCharsNew - NumCharsOld)) {
    if (_IsSpaceInBuffer(hObj, pObj, NumBytesNew - NumBytesOld)) {
      pObj   = (MULTIEDIT_OBJ *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required */
      pText  = (char*) GUI_ALLOC_h2p(pObj->hText);
      pText += GUI_UC__NumChars2NumBytes(pText, pObj->NumCharsPrompt);
      if (pNew) {
  ...
  if (hObj && s) {
    ...
    if (!pObj->hText) {
      ...
      if (NumCharsNew > 0) {
        ...
        if (_IsSpaceInBuffer(hObj, pObj, NumBytesNew)) {
          pObj  = (MULTIEDIT_OBJ *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required */
          pText = (char *)GUI_ALLOC_h2p(pObj->hText);
