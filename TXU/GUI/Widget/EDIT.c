    ...
    *pCursorWidth = ((pObj->XSizeCursor > 0) ? (pObj->XSizeCursor) : (1));
    ...
    if (pText) {
      ...
      if ((pObj->EditMode != GUI_EDIT_MODE_INSERT) || ((pObj->SelSize) && ManageSelection)) {
        ...
        #if GUI_SUPPORT_BIDI
          IsRTL = GUI__BIDI_GetCharDir(pText, NumChars, pObj->CursorPos);
        #endif
      }
...
*       _Paint
*/
static void _Paint(EDIT_Handle hObj, EDIT_Obj * pObj, int Complete) {
  int CursorWidth;
  const char GUI_UNI_PTR * pText = NULL;
...
*       _InvertCursor
*/
static void _InvertCursor(EDIT_Handle hObj, EDIT_Obj * pObj) {
  _Paint(hObj, pObj, 0);
}
...
*/
static int _IncrementBuffer(EDIT_Handle hObj, EDIT_Obj* pObj, unsigned AddBytes) {
  WM_HMEM hNew;
  int NewSize;
  NewSize = pObj->BufferSize + AddBytes;
  hNew    = GUI_ALLOC_Realloc(pObj->hpText, NewSize);
  pObj = (EDIT_Obj *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required */
  if (hNew) {
    if (!(pObj->hpText)) {
      char* pText;
...
*/
static int _IsSpaceInBuffer(EDIT_Handle hObj, EDIT_Obj* pObj, int BytesNeeded) {
  int NumBytes = 0;
  if (pObj->hpText) {
  ...
  if (BytesNeeded > 0) {
    if (!_IncrementBuffer(hObj, pObj, BytesNeeded + EDIT_REALLOC_SIZE)) {
      return 0;
    }
  }
  return 1;
                          ...
                          int Id, int MaxLen)
{
  ...
  if (hObj) {
    ...
    pObj->BufferSize    = 0;
    pObj->hpText        = 0;
    if (_IncrementBuffer(hObj, pObj, pObj->MaxLen + 1) == 0) {
...
void EDIT_SetMaxLen(EDIT_Handle  hObj, int MaxLen) {
  if (hObj) {
    ...
    if (MaxLen != pObj->MaxLen) {
      ...
      _IncrementBuffer(hObj, pObj, MaxLen - pObj->BufferSize + 1);
      pObj->MaxLen = MaxLen;
      EDIT_Invalidate(hObj);
    }
    WM_UNLOCK();
