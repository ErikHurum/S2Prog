...
static void _ManageScrollBars(GRAPH_OBJ * pObj, GRAPH_Handle hObj) {
  ...
  pObj = (GRAPH_OBJ *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required */
...
void GRAPH_DetachData(GRAPH_Handle hObj, GRAPH_DATA_Handle hData) {
  if (hObj) {
    ...
    WM_LOCK();
    pObj = GRAPH_H2P(hObj);
    for (i = 0; i < pObj->GraphArray.NumItems; i++) {
      GRAPH_DATA_Handle hi;
      hi = GUI_ARRAY_GethItem(&pObj->GraphArray, i);
      if (hi) {
        hi = *(GRAPH_DATA_Handle *)GUI_ALLOC_h2p(hi);
        if (hi == hData) {
          GUI_ARRAY_DeleteItem(&pObj->GraphArray, i);
          break;
        }
      }
    }
...
void GRAPH_DetachScale(GRAPH_Handle hObj, GRAPH_SCALE_Handle hScale) {
  if (hObj) {
    ...
    WM_LOCK();
    pObj = GRAPH_H2P(hObj);
    for (i = 0; i < pObj->ScaleArray.NumItems; i++) {
      GRAPH_SCALE_Handle hi;
      hi = GUI_ARRAY_GethItem(&pObj->ScaleArray, i);
      if (hi) {
        hi = *(GRAPH_SCALE_Handle *)GUI_ALLOC_h2p(hi);
        if (hi == hScale) {
          GUI_ARRAY_DeleteItem(&pObj->ScaleArray, i);
          break;
        }
      }
      /*
