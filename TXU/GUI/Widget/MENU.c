      ...
      if ((pItem->Flags & MENU_IF_DISABLED) == 0) {
        ...
        if (PrevActiveSubmenu == 0) {
          if (_IsTopLevelMenu(hObj, pObj)) {
            ...
            /* Pointer recalculation required after sending a notification message */
        ...
        WM_AttachWindowAt(pItem->hSubmenu, /*WM_HBKWIN*/hParent, x, y);
...
*       _SetSelectionEx
*/
static void _SetSelectionEx(MENU_Handle hObj, MENU_Obj* pObj, int Index, int SuppressNotification) {
  if (Index != pObj->Sel) {
    pObj->Sel = Index;
    ...
    if ((pObj->IsSubmenuActive == 0) && (Index >= 0) && (SuppressNotification == 0)) {
      MENU_ITEM * pItem;
      pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray, Index);
      if ((pItem->Flags & MENU_IF_SEPARATOR) == 0) {
        MENU__SendMenuMessage(hObj, pObj->hOwner, MENU_ON_ITEMACTIVATE, pItem->Id);
      }
    }
  }
}
...
static void _SetSelection(MENU_Handle hObj, MENU_Obj* pObj, int Index) {
  _SetSelectionEx(hObj, pObj, Index, 0);
}
...
static void _DeselectItem(MENU_Handle hObj, MENU_Obj* pObj) {
  if (pObj->IsSubmenuActive == 0) {
    ...
    pObj = (MENU_Obj *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required after sending a notification message */
...
*/
/*
static void _OpenOnPointOver(MENU_Handle hObj, MENU_Obj* pObj, unsigned Index) {
  if ((pObj->Flags & MENU_SF_OPEN_ON_POINTEROVER) == 0) {
    _ActivateMenu(hObj, pObj, Index);
  }
}
*/
  ...
  if (XYInWidget) {
    ...
    if (ItemIndex >= 0) {
      ...
      if (Pressed > 0) {
        if ((PrevState.Pressed == 0) && (Pressed == 1)) {  /* Clicked */
          ...
          pObj  = (MENU_Obj *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required after sending a notification message */
          pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray, ItemIndex);
          if ((pItem->Flags & MENU_IF_SEPARATOR) == 0) {
            MENU__SendMenuMessage(hObj, pObj->hOwner, MENU_ON_ITEMPRESSED, pItem->Id);
          }
        } else {
          _SelectItem(hObj, pObj, ItemIndex);
        }
    ...
    if ((Pressed > 0) && (PrevState.Pressed == 0)) {
      ...
      if (pObj->Widget.State & WIDGET_STATE_FOCUS) {
        ...
        pObj = (MENU_Obj *)GUI_ALLOC_h2p(hObj);                        /* Pointer recalculation required after sending a notification message */
        _GetTopLevelMenu(hObj, pObj, &hObjTopLevel, &pObjTopLevel, 0); /* Pointer recalculation required after sending a notification message */
...
*/
static int _ForwardPIDMsgToOwner(MENU_Handle hObj, MENU_Obj* pObj, WM_MESSAGE* pMsg) {
...
*/
static void _ForwardPIDStateChange(MENU_Handle hObj, MENU_Obj * pObj, WM_MESSAGE * pMsg) {/**/
  WM_HWIN hWin;
  ...
  if (_IsTopLevelMenu(hObj, pObj)) {
    ...
    hWin = pObj->hOwner ? pObj->hOwner : WM_GetParent(hObj);
...
*       _ForwardPIDMsg
*/
static void _ForwardPIDMsg(MENU_Handle hObj, MENU_Obj * pObj, WM_MESSAGE * pMsg) {/**/
  WM_HWIN hWin;
  GUI_PID_STATE * pState;
  int XYInWidget, x, y;
  pState = (GUI_PID_STATE *)pMsg->Data.p;
...
static void _MoveSel(MENU_Handle hObj, int Dir, int SuppressNotification) {
  ...
  if (NewIndex >= 0) {
    _SetSelectionEx(hObj, pObj, Index, SuppressNotification);
  }
}
...
static int _OnKey(MENU_Handle hObj, MENU_Obj * pObj, int Key) {
  ...
  if (Index < 0) {
    if        (Key == KeyPrev) {
      _SetSelection(hObj, pObj, pObj->ItemArray.NumItems - 1);
      pObj = (MENU_Obj *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required after sending a notification message */
  ...
  if (pItem->hSubmenu && ((Key == KeyOpen) || (Key == GUI_KEY_ENTER))) {
    ...
    if (Key == KeyOpen) {
      ...
      _GetTopLevelMenu(hObj, pObj, &hObjTopLevel, &pObjTopLevel, &SubSel); /* Pointer recalculation required after sending a notification message */
    } else {
      _MoveSel(hObjTopLevel, -1, 1);
      _GetTopLevelMenu(hObj, pObj, &hObjTopLevel, &pObjTopLevel, &SubSel); /* Pointer recalculation required after sending a notification message */
    }
    ...
    if (pObj->hOwner) {
      MENU_ITEM * pItem;
      MENU_Obj  * pObjOwner;
      pObjOwner = (MENU_Obj *)GUI_ALLOC_h2p(pObj->hOwner);
      pItem = (MENU_ITEM *)GUI_ARRAY_GetpItem(&pObjOwner->ItemArray, pObjOwner->Sel);
      if ((pItem->Flags & MENU_IF_SEPARATOR) == 0) {
        MENU__SendMenuMessage(hObj, pObj->hOwner, MENU_ON_ITEMACTIVATE, pItem->Id);
      }
    }
  } else if ((hObjTopLevel == hObj) && (Key == GUI_KEY_ESCAPE) && (!(pObj->Flags & MENU_SF_POPUP))) {
    /*
...
void MENU_Callback(WM_MESSAGE* pMsg) {
  ...
  case WM_PID_STATE_CHANGED:
    _ForwardPIDStateChange(hObj, pObj, pMsg);/**/
    break;
  case WM_MENU:
  ...
  case WM_TOUCH:
    ...
    if (_OnTouch(hObj, pObj, pMsg)) {
      /* Pointer recalculation required after sending a notification message */
      pObj = (MENU_Obj *)GUI_ALLOC_h2p(hObj);
      if (_ForwardPIDMsgToOwner(hObj, pObj, pMsg)) {
        /* Pointer recalculation required after sending a notification message */
        pObj = (MENU_Obj *)GUI_ALLOC_h2p(hObj);
        _ForwardPIDMsg(hObj, pObj, pMsg);
      }
    }
    break;
#if (GUI_SUPPORT_MOUSE)
...
char MENU__SetItem(MENU_Handle hObj, MENU_Obj* pObj, unsigned Index, const MENU_ITEM_DATA* pItemData) {
  ...
  if (GUI_ARRAY_SetItem(&pObj->ItemArray, Index, &Item, sizeof(MENU_ITEM) + strlen(pText)) != 0) {
    MENU_ITEM * pItem;
    pObj  = (MENU_Obj *)GUI_ALLOC_h2p(hObj); /* Pointer recalculation required */
    pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray, Index);
...
int MENU_SetSel(MENU_Handle hObj, int Sel) {
  ...
  if (hObj) {
    ...
    if (pObj) {
      OldSel = pObj->Sel;
      if (Sel < pObj->ItemArray.NumItems) {
        pObj->Sel = Sel;
        WM_InvalidateWindow(hObj);
      }
    }
