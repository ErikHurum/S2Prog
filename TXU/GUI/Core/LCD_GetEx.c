...
tLCDDEV_Index2Color * LCD_GetpfIndex2ColorEx(int LayerIndex) {
  ...
  #if GUI_NUM_LAYERS > 5
    if (LayerIndex == 5) {
      return LCD_L0_5_GetpfIndex2Color();
    }
  #endif
  return 0;
}
...
tLCDDEV_Index2Color * GUI_GetpfIndex2ColorEx(int LayerIndex) {
  ...
  if (GUI_Context.hDevData) {
    /* If a memory device is selected, return the color conversion routine of the memory device */
    GUI_MEMDEV * pDevData;
    pDevData = GUI_MEMDEV_H2P(GUI_Context.hDevData);
    return pDevData->pfIndex2Color;
  } else {
  #else
  {
  #endif
    /* If no memory device is selected, return the color conversion routine of the current layer */
    return LCD_GetpfIndex2ColorEx(LayerIndex);
  }
}
...
tLCDDEV_Color2Index * LCD_GetpfColor2IndexEx(int LayerIndex) {
  if (LayerIndex == 0) {
    return LCD_L0_GetpfColor2Index();
  }
  #if GUI_NUM_LAYERS > 1
    if (LayerIndex == 1) {
      return LCD_L0_1_GetpfColor2Index();
    }
  #endif
  #if GUI_NUM_LAYERS > 2
    if (LayerIndex == 2) {
      return LCD_L0_2_GetpfColor2Index();
    }
  #endif
  #if GUI_NUM_LAYERS > 3
    if (LayerIndex == 3) {
      return LCD_L0_3_GetpfColor2Index();
    }
  #endif
  #if GUI_NUM_LAYERS > 4
    if (LayerIndex == 4) {
      return LCD_L0_4_GetpfColor2Index();
    }
  #endif
  #if GUI_NUM_LAYERS > 5
    if (LayerIndex == 5) {
      return LCD_L0_5_GetpfColor2Index();
    }
  #endif
  return 0;
}
...
tLCDDEV_Color2Index * GUI_GetpfColor2IndexEx(int LayerIndex) {
  ...
  if (GUI_Context.hDevData) {
    ...
    GUI_MEMDEV * pDevData;
    pDevData = GUI_MEMDEV_H2P(GUI_Context.hDevData);
    return pDevData->pfColor2Index;
  } else {
  #else
  {
  #endif
    /* If no memory device is selected, return the color conversion routine of the current layer */
    return LCD_GetpfColor2IndexEx(LayerIndex);
  }
}
