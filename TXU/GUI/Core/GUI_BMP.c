...
static void _NextPixel_FlushBuffer(void) {
  LCD_DrawBitmap(_Context.xPos, _Context.yPos, _Context.xSize, 1, 1, 1, _Context.BitsPerPixel, 0, (const U8 *)_Context.pBuffer, NULL);
...
static int _DrawBitmap_RLE(GUI_BMP_CONTEXT * pContext, int x0, int y0, int XSize, int YSize, int NumColors, int Compression) {
  ...
  do {
    ...
    if (Cmd == 0) {
      ...
      case 0:   /* End of line */
        if (x < x0 + XSize - 1) {
          LCD_COLORINDEX = LCD__aConvTable[0];
      ...
      case BI_RLE4:
        if ((Data & 0xf) == (Data >> 4)) {
          LCD_COLORINDEX = LCD__aConvTable[Data & 0xf];
          ...
          for (i = 0; i < Cmd; i++) {
            if ((i & 1) ==0) {
              LCD_COLORINDEX = LCD__aConvTable[Data >> 4];
            } else {
              LCD_COLORINDEX = LCD__aConvTable[Data & 15];
            }
            LCD_DrawPixel(x++, y);
          }
        }
        break;
      case BI_RLE8:
        LCD_COLORINDEX = LCD__aConvTable[Data];
        LCD_DrawHLine(x, y, x + Cmd - 1);
        x += Cmd;
        break;
      }
...
static int _DrawBitmap_16bpp(GUI_BMP_CONTEXT * pContext, int x0, int y0, int XSize, int YSize) {
  ...
  pNextPixel_API->pfStart(x0, y0, x0 + XSize - 1, y0 + YSize - 1); /* Initialize drawing routine of the driver */
  for (; (y < YSize) && (y >= 0); y += Step) {
    /* Read one line of pixel data */
    if (GUI_BMP__ReadData(pContext, BytesPerLine, &pData, 0)) {
      return 1; /* Error */
...
int GUI_BMP__ReadData(GUI_BMP_CONTEXT * pContext, int NumBytes, const U8 ** ppData, unsigned StartOfFile) {
  if (pContext->fpGetData(pContext->pParam, NumBytes, ppData, StartOfFile) != NumBytes) {
    return 1;
  }
  return 0;
}
...
void GUI_BMP_C(void);
void GUI_BMP_C(void){} /* Avoid empty object files */
