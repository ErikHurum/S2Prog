...
int  GUI_GIF_DrawSubScaledEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p, int x0, int y0, int Index, int Num, int Denom) {
  ...
  {
    const U8 * pData;
    GUI_GIF__ReadData(&Context, 10, &pData, 1);
    pData += 6;
  ...
  OldColorIndex = LCD_GetColorIndex();
    ...
    WM_ITERATE_START(&r) {
  #endif
  Result = GUI_GIF__DrawFromFilePointer(&Context, x0, y0, Index, Num, Denom, _DrawFromDataBlockScaled, _ClearUnusedPixelsScaled);
  #if (GUI_WINSUPPORT)
    } WM_ITERATE_END();
  #endif
...
void GUI_GIF_DrawScaled_C(void);
void GUI_GIF_DrawScaled_C(void){} /* Avoid empty object files */
