...
*       _FindChar
*/
static const GUI_FONT_PROP GUI_UNI_PTR * _FindChar(const GUI_FONT_PROP GUI_UNI_PTR* pProp, U16P c) {
...
void GUIPROP_DispChar(U16P c) {
  ...
  const GUI_FONT_PROP GUI_UNI_PTR * pProp = _FindChar(GUI_Context.pAFont->p.pProp, c);
  if (pProp) {
...
char GUIPROP_IsInFont(const GUI_FONT GUI_UNI_PTR * pFont, U16 c) {
  const GUI_FONT_PROP GUI_UNI_PTR * pProp = _FindChar(pFont->p.pProp, c);
