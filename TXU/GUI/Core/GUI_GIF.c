...
static int _GetNextCode(GUI_GIF_CONTEXT * pContext) {
  int i, j, End;
  long Result;
  if (GUI_GIF__LZWContext.ReturnClear) {
    /* The first code should be a clear code. */
    GUI_GIF__LZWContext.ReturnClear = 0;
    return GUI_GIF__LZWContext.ClearCode;
  }
  End = GUI_GIF__LZWContext.CurBit + GUI_GIF__LZWContext.CodeSize;
  if (End >= GUI_GIF__LZWContext.LastBit) {
    int Count;
    if (GUI_GIF__LZWContext.GetDone) {
      return -1; /* Error */
    }
    GUI_GIF__LZWContext.aBuffer[0] = GUI_GIF__LZWContext.aBuffer[GUI_GIF__LZWContext.LastByte - 2];
    GUI_GIF__LZWContext.aBuffer[1] = GUI_GIF__LZWContext.aBuffer[GUI_GIF__LZWContext.LastByte - 1];
    if ((Count = _GetDataBlock(pContext, &GUI_GIF__LZWContext.aBuffer[2])) == 0) {
      GUI_GIF__LZWContext.GetDone = 1;
    }
    if (Count < 0) {
      return -1; /* Error */
    }
    GUI_GIF__LZWContext.LastByte = 2 + Count;
    GUI_GIF__LZWContext.CurBit   = (GUI_GIF__LZWContext.CurBit - GUI_GIF__LZWContext.LastBit) + 16;
    GUI_GIF__LZWContext.LastBit  = (2 + Count) * 8 ;
    End                  = GUI_GIF__LZWContext.CurBit + GUI_GIF__LZWContext.CodeSize;
  }
  j = End >> 3;
  i = GUI_GIF__LZWContext.CurBit >> 3;
  if (i == j) {
    Result = (long)GUI_GIF__LZWContext.aBuffer[i];
  } else if (i + 1 == j) {
    Result = (long)GUI_GIF__LZWContext.aBuffer[i] | ((long)GUI_GIF__LZWContext.aBuffer[i + 1] << 8);
  }  else {
    Result = (long)GUI_GIF__LZWContext.aBuffer[i] | ((long)GUI_GIF__LZWContext.aBuffer[i + 1] << 8) | ((long)GUI_GIF__LZWContext.aBuffer[i + 2] << 16);
  }
  Result = (Result >> (GUI_GIF__LZWContext.CurBit & 0x7)) & _aMaskTbl[GUI_GIF__LZWContext.CodeSize];
  GUI_GIF__LZWContext.CurBit += GUI_GIF__LZWContext.CodeSize;
  return (int)Result;
}
...
static int _DrawFromDataBlock(GUI_GIF_CONTEXT * pContext, IMAGE_DESCRIPTOR * pDescriptor, int x0, int y0, int Transparency, int Disposal, int Num, int Denom) {
  ...
  LogPalette.pPalEntries = GUI_GIF__aColorTable;
  ...
  GUI_GIF__InitLZW(Codesize);                    /* Initialize the LZW stack with the LZW codesize */
  ...
  for (YCnt = 0, YPos = y0, Pass = 0; YCnt < Height; YCnt++) {
    ...
    for (XPos = x0; XPos <= XEnd; XPos++) {
      if (GUI_GIF__LZWContext.sp > GUI_GIF__LZWContext.aDecompBuffer) {
        Index = *--(GUI_GIF__LZWContext.sp);
      } else {
...
static int _ReadColorMap(GUI_GIF_CONTEXT * pContext, int NumColors) {
  ...
  for (i = 0; i < NumColors; i++) {
    ...
    g = *(pData + 1);
    b = *(pData + 2);
    GUI_GIF__aColorTable[i] = r | ((U16)g << 8) | ((U32)b << 16);
  }
  return 0;
}
...
*       _GetValue
*/
static U16 _GetValue(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p, int Offset) {
  GUI_GIF_CONTEXT Context;
  const U8 * pData;
  Context.fpGetData = fpGetData;
  Context.pParam    = p;
  if (GUI_GIF__ReadData(&Context, Offset + 2, &pData, 0)) {
    return 0; /* Error */
  }
  pData += Offset;
  return GUI__Read16(&pData);
}
...
int GUI_GIF__GetNextByte(GUI_GIF_CONTEXT * pContext) {
  ...
  while ((Code = _GetNextCode(pContext)) >= 0) {
    if (Code == GUI_GIF__LZWContext.ClearCode) {
      /* Corrupt GIFs can make this happen */
      if (GUI_GIF__LZWContext.ClearCode >= (1 << MAX_NUM_LWZ_BITS)) {
        return -1; /* Error */
      }
      /* Clear the tables */
      GUI__memset((U8 *)GUI_GIF__LZWContext.aCode, 0, sizeof(GUI_GIF__LZWContext.aCode));
      for (i = 0; i < GUI_GIF__LZWContext.ClearCode; ++i) {
        GUI_GIF__LZWContext.aPrefix[i] = i;
      }
      /* Calculate the 'special codes' in dependence of the initial code size
         and initialize the stack pointer */
      GUI_GIF__LZWContext.CodeSize    = GUI_GIF__LZWContext.SetCodeSize + 1;
      GUI_GIF__LZWContext.MaxCodeSize = GUI_GIF__LZWContext.ClearCode << 1;
      GUI_GIF__LZWContext.MaxCode     = GUI_GIF__LZWContext.ClearCode + 2;
      GUI_GIF__LZWContext.sp          = GUI_GIF__LZWContext.aDecompBuffer;
      /* Read the first code from the stack after clearing and initializing */
      do {
        GUI_GIF__LZWContext.FirstCode = _GetNextCode(pContext);
      } while (GUI_GIF__LZWContext.FirstCode == GUI_GIF__LZWContext.ClearCode);
      GUI_GIF__LZWContext.OldCode = GUI_GIF__LZWContext.FirstCode;
      return GUI_GIF__LZWContext.FirstCode;
    }
    if (Code == GUI_GIF__LZWContext.EndCode) {
      return -2; /* End code */
    }
    Incode = Code;
    if (Code >= GUI_GIF__LZWContext.MaxCode) {
      *(GUI_GIF__LZWContext.sp)++ = GUI_GIF__LZWContext.FirstCode;
      Code = GUI_GIF__LZWContext.OldCode;
    }
    while (Code >= GUI_GIF__LZWContext.ClearCode) {
      *(GUI_GIF__LZWContext.sp)++ = GUI_GIF__LZWContext.aPrefix[Code];
      if (Code == GUI_GIF__LZWContext.aCode[Code]) {
        return Code;
      }
      if ((GUI_GIF__LZWContext.sp - GUI_GIF__LZWContext.aDecompBuffer) >= sizeof(GUI_GIF__LZWContext.aDecompBuffer)) {
        return Code;
      }
      Code = GUI_GIF__LZWContext.aCode[Code];
    }
    *(GUI_GIF__LZWContext.sp)++ = GUI_GIF__LZWContext.FirstCode = GUI_GIF__LZWContext.aPrefix[Code];
    if ((Code = GUI_GIF__LZWContext.MaxCode) < (1 << MAX_NUM_LWZ_BITS)) {
      GUI_GIF__LZWContext.aCode  [Code] = GUI_GIF__LZWContext.OldCode;
      GUI_GIF__LZWContext.aPrefix[Code] = GUI_GIF__LZWContext.FirstCode;
      ++GUI_GIF__LZWContext.MaxCode;
      if ((GUI_GIF__LZWContext.MaxCode >= GUI_GIF__LZWContext.MaxCodeSize) && (GUI_GIF__LZWContext.MaxCodeSize < (1 << MAX_NUM_LWZ_BITS))) {
        GUI_GIF__LZWContext.MaxCodeSize <<= 1;
        ++GUI_GIF__LZWContext.CodeSize;
      }
    }
    GUI_GIF__LZWContext.OldCode = Incode;
    if (GUI_GIF__LZWContext.sp > GUI_GIF__LZWContext.aDecompBuffer) {
      return *--(GUI_GIF__LZWContext.sp);
    }
  }
  return Code;
}
...
void GUI_GIF__InitLZW(int InputCodeSize) {
  GUI__memset((U8 *)&GUI_GIF__LZWContext, 0, sizeof(LZW_CONTEXT));
  GUI_GIF__LZWContext.SetCodeSize  = InputCodeSize;
  GUI_GIF__LZWContext.CodeSize     = InputCodeSize + 1;
  GUI_GIF__LZWContext.ClearCode    = (1 << InputCodeSize);
  GUI_GIF__LZWContext.EndCode      = (1 << InputCodeSize) + 1;
  GUI_GIF__LZWContext.MaxCode      = (1 << InputCodeSize) + 2;
  GUI_GIF__LZWContext.MaxCodeSize  = (1 << InputCodeSize) << 1;
  GUI_GIF__LZWContext.ReturnClear  = 1;
  GUI_GIF__LZWContext.LastByte     = 2;
  GUI_GIF__LZWContext.sp           = GUI_GIF__LZWContext.aDecompBuffer;
}
...
int GUI_GIF__ReadData(GUI_GIF_CONTEXT * pContext, unsigned NumBytesReq, const U8 ** ppData, unsigned StartOfFile) {
  ...
  if (pContext->NumBytesInBuffer < NumBytesReq) {
    ...
    if ((pContext->NumBytesInBuffer += pContext->fpGetData(pContext->pParam, NumBytesRead, ppData, StartOfFile)) < NumBytesReq) {
      return 1;
    }
  ...
  *ppData = pContext->pBuffer;
  ...
  return 0;
#else
  if ((unsigned)pContext->fpGetData(pContext->pParam, NumBytesReq, ppData, StartOfFile) != NumBytesReq) {
    return 1;
  }
  return 0;
#endif
...
int GUI_GIF_GetXSizeEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p) {
  return _GetValue(fpGetData, p, 6);
}
...
int GUI_GIF_GetYSizeEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p) {
  return _GetValue(fpGetData, p, 8);
}
...
*       GUI_GIF_DrawSubEx
*/
int GUI_GIF_DrawSubEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p, int x0, int y0, int Index) {
  int Result, OldColorIndex;
  GUI_GIF_CONTEXT Context = {0};
  #if (GUI_WINSUPPORT)
    GUI_RECT r;
    int Width, Height;
  #endif
  ...
  Context.pParam    = p;
  #if (GUI_WINSUPPORT)
  if (_GetImageDimension(&Context, &Width, &Height)) {
    return 1; /* Error */
  }
  #endif
  OldColorIndex = LCD_GetColorIndex();
    ...
    WM_ITERATE_START(&r) {
  #endif
  Result = GUI_GIF__DrawFromFilePointer(&Context, x0, y0, Index, 0, 0, _DrawFromDataBlock, _ClearUnusedPixels);
  #if (GUI_WINSUPPORT)
    } WM_ITERATE_END();
  #endif
...
*       GUI_GIF_DrawEx
*/
int GUI_GIF_DrawEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p, int x0, int y0) {
  return GUI_GIF_DrawSubEx(fpGetData, p, x0, y0, 0);
}
...
*       GUI_GIF_GetInfoEx
*/
int GUI_GIF_GetInfoEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p, GUI_GIF_INFO * pInfo) {
  GUI_GIF_CONTEXT Context = {0};
  Context.fpGetData = fpGetData;
  Context.pParam    = p;
  return _GetGIFInfo(&Context, pInfo);
}
...
*/
int GUI_GIF_GetImageInfoEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p, GUI_GIF_IMAGE_INFO * pInfo, int Index) {
  GUI_GIF_CONTEXT Context = {0};
  Context.fpGetData = fpGetData;
  Context.pParam    = p;
  return _GetImageInfo(&Context, pInfo, Index);
}
...
*/
int GUI_GIF_GetCommentEx(GUI_GIF_GET_DATA_FUNC * fpGetData, void * p, U8 * pBuffer, int MaxSize, int Index) {
  GUI_GIF_CONTEXT Context = {0};
  Context.fpGetData = fpGetData;
  Context.pParam    = p;
  if (_GetGIFComment(&Context, pBuffer, MaxSize, Index)) {
    return 1;
  }
  return 0;
}
...
void GUI_GIF_C(void);
void GUI_GIF_C(void){} /* Avoid empty object files */
