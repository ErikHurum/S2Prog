...
GUI_MEMDEV_Handle GUI_MEMDEV_CreateEx(int x0, int y0, int xSize, int ySize, int Flags) {
  ...
  if (GUI_Context.hDevData == 0) {
    ...
    pfIndex2Color = LCD_GetpfIndex2ColorEx(GUI_Context.SelLayer);
      ...
      if (XSize == pDev->XSize) {
        ...
        if (BitsPerPixel == 1) {
          ...
          for (; XSize; ) {
            ...
            xOff  = xOff & ~0x7;
            LCD_DrawBitmap(x + xOff, y + yi, XSize, 1, 1, 1, BitsPerPixel, BytesPerLine, pData + (xOff >> 3), NULL);
            xOff += XSize;
            XSize = GUI_USAGE_GetNextDirty(pUsage, &xOff, yi);
          }
        } else {
