...
TDUAppSetupCargo::~TDUAppSetupCargo(void)
{
}
...
const GUI_WIDGET_CREATE_INFO TDUAppSetupCargo::aDialogCreate[] =
{
	...
	{ BUTTON_CreateIndirect,"Zero adjust tank\n pressure sensor for\n all cargo tanks." ,GUI_ID_BUTTON0,  5,  105, 130, 45},
	{ BUTTON_CreateIndirect,"Zero adjust level\n sensor(s) for\n all cargo tanks."      ,GUI_ID_BUTTON1,  144, 105, 130, 45},
};


void TDUAppSetupCargo::ExecDialogBox(WM_HWIN hCurrentWin)
{
	if ( AppSetupCargoIsClosed ) {
		AppSetupCargoIsClosed = false;
...
* Dialog procedure
*/
void TDUAppSetupCargo::GetDialogHandles(void)
{
...
void TDUAppSetupCargo::InitDialog(void)
{
	...
	for ( int i = 0; i < ValTabEntries; i++ ) {
		...
		if ( ValTabPtr[i].Value==CargoWindow->ValueTable[2].ValueId )ind1=i;
		if ( ValTabPtr[i].Value==CargoWindow->ValueTable[3].ValueId )ind2=i;
		if ( ValTabPtr[i].Value==CargoWindow->ValueTable[4].ValueId )ind3=i;
	}
...
void TDUAppSetupCargo::UpdateDialog(WM_HWIN hWin,int Key)
{
	switch ( Key ) {
	...
	case GUI_ID_DROPDOWN3:
		{
			...
			CargoWindow->ValueTable[col].ValueId = ValTabPtr[DROPDOWN_GetSel(hDropd0)].Value;
...
void TDUAppSetupCargo::Update(void)
{
	SetSelected(GUI_ID_DROPDOWN4,PROSystemData::TXUSystemData->GetOpModeState(SVT_TANK_STATE_STR_CGO));
	if ( hTimer ) WM_RestartTimer(hTimer, 1000);
}

void TDUAppSetupCargo::cbTankSetup(WM_MESSAGE *pMsg)
{
	int NCode, Id;
	WM_HWIN hWin = pMsg->hWin;
	switch ( pMsg->MsgId ) {
	case WM_INIT_DIALOG:
		AppSetupCargo->FirstTime = true;
		break;
	case WM_PAINT:
		GUI_DrawRect(   2,  55, 275, 98 );
		GUI_DrawRect(   2,  154, 175, 177 );
		AppSetupCargo->UpdateLanguage();
		break;
	...
	case WM_NOTIFY_PARENT:
		...
		case WM_NOTIFICATION_VALUE_CHANGED:
			...
			case GUI_ID_DROPDOWN2:
			case GUI_ID_DROPDOWN3:
				AppSetupCargo->UpdateDialog(hWin,Id);
				break;
		...
		case WM_NOTIFICATION_SEL_CHANGED:
			...
			case GUI_ID_DROPDOWN3:
				...
				break;
			case GUI_ID_DROPDOWN4: /* OpMode  */
				AppSetupCargo->GetSelected(GUI_ID_DROPDOWN4,SVT_TANK_STATE_STR_CGO,PROSystemData::TXUSystemData);
				break;
			}
			break;
