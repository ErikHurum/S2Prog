...
///////////////////////////////////////////////////////////////////
//
//  Calculate object added to TaskUniquePROSet
//
///////////////////////////////////////////////////////////////////
#ifdef S2TXU
static TSN_STACKPTR int CalcOtherObjectsStack[4 * 1024];
OS_TASK CalcOtherObjectsTask;
int CalcTime = 0;
int StartTime = 0;
void CalcOtherObjects_Handler(void) {
    int Delay   = 1;
    int Cnt1    = PRogramObject::TaskUniquePROSet.size();
    int Cnt2    = 0;
    int Cnt3    = 0;
    while (true) {
        Cnt2    = 0;
        Cnt3    = 0;
        StartTime = OS_Time;
        RecalcProgramObjects(PRogramObject::TaskUniquePROSet, Delay);
        CalcTime = OS_Time - StartTime;
        vector<AnalogInput*>::iterator pBIt;
        for ( pBIt = AnalogInput::AIPtrVector.begin(); pBIt != AnalogInput::AIPtrVector.end(); pBIt++ ) {
            OS_Delay(Delay);
            (*pBIt)->SendData();
            Cnt2++;
        }
        set<PRogramObjectBase*>::iterator pSGCnvIt;
        for ( pSGCnvIt = SignalConverter::ObjectSet.begin(); pSGCnvIt != SignalConverter::ObjectSet.end(); pSGCnvIt++ ) {
            OS_Delay(Delay);
            (*pSGCnvIt)->SendData();
            Cnt3++;
        }
        int TotalTime = OS_Time - StartTime;
    ...
    if (!IOUnitList.empty()) {
        ...
        for (pBIt = IOUnitList.begin(); pBIt != IOUnitList.end(); pBIt++) {
            ...
            for (pBIt = IOElement->IOUniquePROSet.begin(); pBIt != IOElement->IOUniquePROSet.end(); pBIt++) {
                ...
                PRogramObject::TaskUniquePROSet.insert(IOElement);
            }
        }
        NumberOfRequests += ModbusRegisterIn::UniquePROSet.size();
        set<PRogramObject *>::iterator pBIt2;
        for (pBIt2 = ModbusRegisterIn::UniquePROSet.begin(); pBIt2 != ModbusRegisterIn::UniquePROSet.end(); pBIt2++) {
            PRogramObject *IOElement = *pBIt2;
            PRogramObject::TaskUniquePROSet.insert(IOElement);
        }
        if (!PROProjectInfo::SimulateIO && NumberOfRequests) {
            CREATE_TASK(&CalcOtherObjectsTask, "Calcuations", CalcOtherObjects_Handler, ANPRO10_TASK_PRIORITY, CalcOtherObjectsStack);
            // Forever
            float AvgTime = 0.0;
            int   Delay = 10; //2*MIN_IO_DELAY;
            while (true) {
                ...
                for (pBIt = IOUnitSet.begin(); pBIt != IOUnitSet.end(); pBIt++) {
                    ...
                    AnsiString Name = IOElement->Name;
                    AnsiString ObjectId = IOElement->IDNumber;
                    int StartTime = OS_Time;
                    IOElement->HandleIO(Delay);
                    int StopTime = OS_Time;
                    int ReplyTime = StopTime - StartTime;
                    TotalTime += ReplyTime;
                }
                if (PROTanksystemUnit::MySelf) PROTanksystemUnit::MySelf->SetIOLoadDelay(AvgTime);
                AvgTime = FilterVal(AvgTime, OS_Time - StartTime, 10.0);
                if (AvgTime > 2000 && Delay > 2) {
                    Delay--;
                } else if (AvgTime < 2000 && Delay < 2) {
                    Delay++;
                }
                OS_DelayUntil(StartTime + RS485_IO_PERIODE);
            }
