...
extern OS_TASK *ExternAlarmSilence;                     /* Task control blocks */
#if (NETWORK==1)
extern OS_TASK *HandleCS8900;                           /* Task control blocks */
#endif
//#endif
...
void OS_InitHW(void) {
    ...
    case TDU_R2:
#if (NETWORK == 1)
        rEINTMASK  &= ~(BIT_SUB_EINT16); // original EINT4
#endif
...
#if (NETWORK == 1)
        rEINTMASK  &= ~(BIT_SUB_EINT4 | BIT_SUB_EINT16); // original EINT4
#else
        rEINTMASK  &= ~(BIT_SUB_EINT4);
#endif
    }
...
__swi __arm int swi_handler(int a, int b) {
#ifdef S2TXU
    rGPBDAT |= 1;       // OK LED, System not OK
...
volatile int IsDown      = 0;
volatile int TCCnt           = 0;
    ...
    case INTOFF_8_23:
        {
            ...
            for ( U32 BitOffset = 4; BitOffset <= 23; BitOffset++ ) {
                U32 BitMask = (1 << BitOffset) & IntRequest;
                if ( BitMask ) {
                    switch ( BitOffset ) {
                    case INTOFF_SILENCE:
                        // For external silence
                        if ( ExternAlarmSilence ) {
                            OS_SignalEvent(EXTERNAL_ALARM_SILENCE, ExternAlarmSilence);
                        }
                        break;
                    case INTOFF_IC1_UP  :
                    ...
                    case INTOFF_IC1_DOWN:
                        ...
                        rTCON   |= TIMER2_START;    // Start timer 2
                        {
                            IncMessage Msg;
                            Msg.IncNum      = 1;
                            Msg.IncCntUp    = Inc1CntUp;
                            Msg.IncCntDown  = Inc1CntDown;
                            OS_PutMailCond(&IncMailBox, &Msg);
                        }
                        break;
                    case INTOFF_IC2_UP  :
                    ...
                    case INTOFF_IC2_DOWN:
                        ...
                        rTCON   |= TIMER3_START;    // Start timer 3
                        {
                            IncMessage Msg;
                            Msg.IncNum      = 2;
                            Msg.IncCntUp    = Inc2CntUp;
                            Msg.IncCntDown  = Inc2CntDown;
                            OS_PutMailCond(&IncMailBox, &Msg);
                        }
                        break;
                    case INTOFF_CS8900:
#if (NETWORK==1)
                        if ( HandleCS8900 ) {
                            OS_SignalEvent(NETWORK_IRQ_SIGNAL, HandleCS8900);
                        }
#ifdef __NET_USE_ACTIVE_HIGH_INTERRUPT__
                        rEINTMASK |= BIT_SUB_EINT16; // mask
#endif
#endif
                        break;
                    }
        ...
        break;
    case    INTOFF_ADC:            //ADC / Touchscreen
        ClearPending(BIT_ADC);
        {
            ...
            if ( IntSubSource &  BIT_SUB_TC ) {          // TC(Touch screen Control) Interrupt
                ...
                if ( rADCTSC & 0x100 ) {
                    ...
                    TXUADData[7] = (0x3ff & rADCDAT0); // Catch the position
                    TXUADData[5] = (0x3ff & rADCDAT1);
            ...
            if ( IntSubSource &  BIT_SUB_ADC ) {    // AD converter
                ...
                if ( ADVal & 0x4000 ) {
                    TCCnt = 5;
                    TXUADData[7] = (0x3ff & ADVal);    // Catch the possition
                    TXUADData[5] = (0x3ff & rADCDAT1);
                    // Stylus up = 1
                    if ( ADVal & 0x8000 ) {
                        if ( IsDown ) {
                    ...
                    if ( TCCnt ) {
                        ...
                        TXUADData[ADChannel] = (ADVal & 0x3ff);
