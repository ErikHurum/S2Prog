...
//*****************************************************************************
//
// for udp netconn management.
// problem with tftp is there is no way to shut down an existing udp connection
// when clients give up tftp due to timeout.
// In worst case, these stale connections can suck up all the system resources.
//
//*****************************************************************************
static void TFTPClose(tTFTPConnection *psTFTP);

#define TFTP_MAX_TRANSFERS          4
static tTFTPConnection* _tftp_transfers[TFTP_MAX_TRANSFERS];
static int              _tftp_num_transfers = 0;

static void
TFTP_Connection_shift(int from)
{
  int i;

  for(i = from + 1; i < TFTP_MAX_TRANSFERS; i++)
  {
    _tftp_transfers[from] = _tftp_transfers[i];
    from++;
  }
}

static void
TFTP_Connection_uncache(tTFTPConnection* psTFTP)
{
  int   i;

  for(i = 0; i < _tftp_num_transfers; i++)
  {
    if(_tftp_transfers[i] == psTFTP)
    {
      TFTP_Connection_shift(i);
      _tftp_num_transfers--;
      return;
    }
  }
...
static void
TFTP_Connection_cache(tTFTPConnection* psTFTP)
{
  if(_tftp_num_transfers >= TFTP_MAX_TRANSFERS)
  {
    //
    // sacrifice the first one
    //
    TFTPClose(_tftp_transfers[0]);
  }

  _tftp_transfers[_tftp_num_transfers] = psTFTP;
  _tftp_num_transfers++;
}
...
TFTPClose(tTFTPConnection *psTFTP)
{
  ...
  TFTP_Connection_uncache(psTFTP);
...
TFTPModeGet(uint8_t *pui8Request, uint32_t ui32Len)
{
  ...
  //
  if(!strncasecmp("netascii", (char *)&pui8Request[ui32Loop], ui32Max))
  {
    //
  ...
  // Binary transfer?
  //
  if(!strncasecmp("octet", (char *)&pui8Request[ui32Loop], ui32Max))
  {
    //
...
//*****************************************************************************
static void
TFTPRecv(void *arg, struct udp_pcb *upcb, struct pbuf *p, struct ip_addr *addr, u16_t port)
{
  uint8_t *pui8Data;
  bool bGetRequest;
  tTFTPMode eMode;
...
//*****************************************************************************
void
TFTPServerInit(tTFTPRequest pfnRequest)
{
  void *pcb;
