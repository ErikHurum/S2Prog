...
#define VOL_LIMIT 0.1	 // Max deviation accepted
...
class TankData
{
	...
	float	Level1[MAX_ENTRIES],
			Level2[MAX_ENTRIES],
			Level3[MAX_ENTRIES],
			Level4[MAX_ENTRIES],
			FSM[MAX_ENTRIES];     //Take out next time! RBMARK

	float	Draft[MAX_ENTRIES],
			Sounding[MAX_ENTRIES],
			Ullage[MAX_ENTRIES],
			Volume[MAX_ENTRIES],
			SoundingVol[MAX_ENTRIES],
			LCG[MAX_ENTRIES],
			TCG[MAX_ENTRIES],
			VCG[MAX_ENTRIES],
			Swl[MAX_ENTRIES],
	...
	TankData():Entries1(0),Entries2(0),Entries3(0),Entries4(0),Entries5(0),TankType(CARGO),IsOnline(false)
	{
	}
};
