...
BEGIN_MESSAGE_MAP(COutputWndGR, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
END_MESSAGE_MAP()

int COutputWndGR::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndEchoCurve.Create(IDD_ECHOCURVE_GR, &m_wndTabs) ||
		!m_wndEchoData.Create(IDD_ECHODATA_GR, &m_wndTabs) ||
		!m_wndSimulate.Create(IDD_SIMULATE_GR, &m_wndTabs)||
		!m_wndOutputMapping.Create(IDD_OUTPUTMAPPING_GR, &m_wndTabs) ||
		!m_wndTrend.Create(IDD_TREND_GR, &m_wndTabs))
	{
		TRACE0("未能创建输出窗口\n");
		return -1;      // 未能创建
	}
...
void COutputWndGR::OnSize(UINT nType, int cx, int cy)
{
...
BOOL COutputWndGR::CanBeClosed() const
{
	return false;
}
