...
IMPLEMENT_DYNAMIC(CAdvanced2DlgGR, COperationDlg)

CAdvanced2DlgGR::CAdvanced2DlgGR()
{
...
BEGIN_MESSAGE_MAP(CAdvanced2DlgGR, COperationDlg)
	ON_BN_CLICKED(IDC_BUTTON_DISTADJ, &CAdvanced2DlgGR::OnBnClickedButtonDistadj)
	ON_BN_CLICKED(IDC_BUTTON_VELOADJ, &CAdvanced2DlgGR::OnBnClickedButtonVeloadj)
	ON_BN_CLICKED(IDC_CHECK_ENABLEDIA, &CAdvanced2DlgGR::OnBnClickedCheckEnabledia)
END_MESSAGE_MAP()

void CAdvanced2DlgGR::LoadData()
{
	CProgress progress;
	CString str;
	UINT8 int8Data;
	...
	theApp.m_pClient->LoadData(0x551, int16Data);
...
// CAdvanced2DlgGR 消息处理程序



void CAdvanced2DlgGR::DoDataExchange(CDataExchange* pDX)
{
...
void CAdvanced2DlgGR::OnBnClickedButtonDistadj()
{
	double dbData = 0;
...
void CAdvanced2DlgGR::OnBnClickedButtonVeloadj()
{
	double dbData = 0;
	if (GetNumMessageBox(IDS_MSG_INPUTNULLHIGHT, dbData) != IDOK)
	{
...
void CAdvanced2DlgGR::OnBnClickedCheckEnabledia()
{
