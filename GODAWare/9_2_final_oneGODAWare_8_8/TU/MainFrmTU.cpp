...
// MainFrmTU.cpp : CClientUITU 类的实现
//
...
IMPLEMENT_DYNAMIC(CClientUITU, CClientUI)

BEGIN_MESSAGE_MAP(CClientUITU, CClientUI)
	ON_WM_CREATE()
	ON_WM_SETFOCUS()
	ON_COMMAND_RANGE(ID_VIEW_APPLOOK_WIN_2000, ID_VIEW_APPLOOK_OFF_2007_AQUA, &CClientUITU::OnApplicationLook)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_APPLOOK_WIN_2000, ID_VIEW_APPLOOK_OFF_2007_AQUA, &CClientUITU::OnUpdateApplicationLook)
	ON_COMMAND(ID_BASIC_LEVEL, &CClientUITU::OnBasicLevel)
	ON_COMMAND(ID_BASIC_APPLICATION, &CClientUITU::OnBasicApplication)
	ON_COMMAND(ID_BASIC_DAMPING, &CClientUITU::OnBasicDamping)
	ON_COMMAND(ID_BASIC_LINERIZATION, &CClientUITU::OnBasicLinerization)
	ON_COMMAND(ID_BASIC_OTHER, &CClientUITU::OnBasicOther)
	ON_COMMAND(ID_DISPLAY_DISPLAY, &CClientUITU::OnDisplayDisplay)
	ON_COMMAND(ID_DIAGNOSIS_DIAGNOSIS, &CClientUITU::OnDiagnosisDiagnosis)
	ON_COMMAND(ID_SERVICE_RESET, &CClientUITU::OnServiceReset)
	ON_COMMAND(ID_SERVICE_CURRENTOUTPUT, &CClientUITU::OnServiceCurrentoutput)
	ON_COMMAND(ID_SERVICE_PIN, &CClientUITU::OnServicePin)
	ON_COMMAND(ID_SERVICE_OTHER, &CClientUITU::OnServiceOther)
	ON_COMMAND(ID_SERVICE_FALSEECHO, &CClientUITU::OnServiceFalseecho)
	ON_COMMAND(ID_ADVANCED_ADVANVCED1, &CClientUITU::OnAdvancedAdvanvced1)
	ON_COMMAND(ID_ADVANCED_ADVANVCED2, &CClientUITU::OnAdvancedAdvanvced2)
	ON_COMMAND(ID_DEVICEOP_LOAD, &CClientUITU::OnDeviceopLoad)
	ON_COMMAND(ID_DEVICEOP_SAVE, &CClientUITU::OnDeviceopSave)
	ON_COMMAND(ID_INFO_INFO, &CClientUITU::OnInfo)
	ON_COMMAND(ID_DEVICEOP_REFRESH, &CClientUITU::OnRefresh)
	ON_WM_TIMER()
END_MESSAGE_MAP()

// CClientUITU 构造/析构

CClientUITU::CClientUITU()
...
int CClientUITU::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL CClientUITU::PreCreateWindow(CREATESTRUCT& cs)
{
	if( !CFrameWndEx::PreCreateWindow(cs) )
		return FALSE;
	// TODO: 在此处通过修改
	//  CREATESTRUCT cs 来修改窗口类或样式
	m_strTitle.LoadString(IDS_TUTITLE);
	cs.dwExStyle &= ~WS_EX_CLIENTEDGE;
	cs.lpszClass = AfxRegisterWndClass(0);
	return TRUE;
}

void CClientUITU::InitializeRibbon()
{
	BOOL bNameValid;
...
*/
	...
	//lstQATCmds.AddTail(ID_FILE_TUINT_DIRECT);
...
BOOL CClientUITU::CreateDockingWindows()
{
	BOOL bNameValid;

	// 创建输出窗口
	CString strOutputWndTU;
	bNameValid = strOutputWndTU.LoadString(IDS_OUTPUT_WND);
	ASSERT(bNameValid);
	if (!m_wndCurve.Create(strOutputWndTU, this, CRect(0, 0, 200, 200),TRUE, ID_WND_OUTPUT, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_BOTTOM | CBRS_FLOAT_MULTI))
	{
		TRACE0("未能创建输出窗口\n");
		return FALSE; // 未能创建
	}
...
void CClientUITU::SetDockingWindowIcons(BOOL /*bHiColorIcons*/)
{
	//HICON hPropertiesBarIcon = (HICON) ::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(bHiColorIcons ? IDI_TUOPERTIES_WND_HC : IDI_TUOPERTIES_WND), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), 0);
...
#ifdef _DEBUG
void CClientUITU::AssertValid() const
{
	CFrameWndEx::AssertValid();
}

void CClientUITU::Dump(CDumpContext& dc) const
{
	CFrameWndEx::Dump(dc);
}
#endif //_DEBUG
...
void CClientUITU::OnSetFocus(CWnd* /*pOldWnd*/)
{
	// 将焦点前移到视图窗口
	m_wndView.SetFocus();
}

BOOL CClientUITU::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
	// 让视图第一次尝试该命令
...
void CClientUITU::OnApplicationLook(UINT id)
{
	CWaitCursor wait;
...
void CClientUITU::OnUpdateApplicationLook(CCmdUI* pCmdUI)
{
	pCmdUI->SetRadio(theApp.m_nAppLook == pCmdUI->m_nID);
}
void CClientUITU::OnBasicLevel()
{
	m_wndView.Show(IDD_LEVEL_ADJUSTMENT_TU);
}

void CClientUITU::OnBasicApplication()
{
	m_wndView.Show(IDD_APPLICATION_TU);
}

void CClientUITU::OnBasicDamping()
{
	m_wndView.Show(IDD_DAMPING_TU);
}

void CClientUITU::OnBasicLinerization()
{
	m_wndView.Show(IDD_LINEARIZATION_TU);
}

void CClientUITU::OnBasicOther()
{
	m_wndView.Show(IDD_OTHERSETTINGS_TU);
}

void CClientUITU::OnDisplayDisplay()
{
	m_wndView.Show(IDD_DISPLAY_TU);
}

void CClientUITU::OnDiagnosisDiagnosis()
{
	m_wndView.Show(IDD_DIAGNOSIS_TU);
}

void CClientUITU::OnServiceCurrentoutput()
{
	m_wndView.Show(IDD_CURRENTOUTPUT_TU);
}

void CClientUITU::OnServiceReset()
{
	m_wndView.Show(IDD_RESET_TU);
}

void CClientUITU::OnServicePin()
{
	m_wndView.Show(IDD_PIN_TU);
}

void CClientUITU::OnServiceOther()
{
	m_wndView.Show(IDD_SERVICEOTHER_TU);
}

void CClientUITU::OnServiceFalseecho()
{
	m_wndView.Show(IDD_FALSEECHO_TU);
}

void CClientUITU::OnAdvancedAdvanvced1()
{
	m_wndView.Show(IDD_ADVANCEDSETUP1_TU);
}

void CClientUITU::OnAdvancedAdvanvced2()
{
	m_wndView.Show(IDD_ADVANCEDSETUP2_TU);
}

void CClientUITU::OnInfo()
{
	m_wndView.Show(IDD_INFO);
}

void CClientUITU::OnDeviceopLoad()
{
	m_wndView.Load();
}

void CClientUITU::OnDeviceopSave()
{
	m_wndView.Save();
}

void CClientUITU::OnRefresh()
{
	theApp.m_pClient->RefreshId();
}

void CClientUITU::OnTimer(UINT_PTR nIDEvent)
{
...
void CClientUITU::SetSendBytes( long )
{

}

void CClientUITU::SetRecvBytes( long )
{

}

void CClientUITU::SetErrBytes( long )
{
