...
BEGIN_MESSAGE_MAP(COutputWndTU, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
END_MESSAGE_MAP()

int COutputWndTU::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndEchoCurve.Create(IDD_ECHOCURVE_TU, &m_wndTabs) ||
		!m_wndEchoData.Create(IDD_ECHODATA_TU, &m_wndTabs) ||
		!m_wndSimulate.Create(IDD_SIMULATE_TU, &m_wndTabs)||
		!m_wndOutputMapping.Create(IDD_OUTPUTMAPPING_TU, &m_wndTabs) ||
		!m_wndTrend.Create(IDD_TREND_TU, &m_wndTabs))
	{
		TRACE0("未能创建输出窗口\n");
		return -1;      // 未能创建
	}
...
void COutputWndTU::OnSize(UINT nType, int cx, int cy)
{
...
BOOL COutputWndTU::CanBeClosed() const
{
	return false;
}
