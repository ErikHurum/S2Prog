...
// MainFrmSU.cpp : CClientUISU 类的实现
//
...
IMPLEMENT_DYNAMIC(CClientUISU, CClientUI)

BEGIN_MESSAGE_MAP(CClientUISU, CClientUI)
	ON_WM_CREATE()
	ON_WM_SETFOCUS()
	ON_COMMAND_RANGE(ID_VIEW_APPLOOK_WIN_2000, ID_VIEW_APPLOOK_OFF_2007_AQUA, &CClientUISU::OnApplicationLook)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_APPLOOK_WIN_2000, ID_VIEW_APPLOOK_OFF_2007_AQUA, &CClientUISU::OnUpdateApplicationLook)
	ON_COMMAND(ID_BASIC_LEVEL, &CClientUISU::OnBasicLevel)
	ON_COMMAND(ID_BASIC_APPLICATION, &CClientUISU::OnBasicApplication)
	ON_COMMAND(ID_BASIC_DAMPING, &CClientUISU::OnBasicDamping)
	ON_COMMAND(ID_BASIC_LINERIZATION, &CClientUISU::OnBasicLinerization)
	ON_COMMAND(ID_BASIC_OTHER, &CClientUISU::OnBasicOther)
	ON_COMMAND(ID_DISPLAY_DISPLAY, &CClientUISU::OnDisplayDisplay)
	ON_COMMAND(ID_DIAGNOSIS_DIAGNOSIS, &CClientUISU::OnDiagnosisDiagnosis)
	ON_COMMAND(ID_SERVICE_RESET, &CClientUISU::OnServiceReset)
	ON_COMMAND(ID_SERVICE_CURRENTOUTPUT, &CClientUISU::OnServiceCurrentoutput)
	ON_COMMAND(ID_SERVICE_PIN, &CClientUISU::OnServicePin)
	ON_COMMAND(ID_SERVICE_OTHER, &CClientUISU::OnServiceOther)
	ON_COMMAND(ID_SERVICE_FALSEECHO, &CClientUISU::OnServiceFalseecho)
	ON_COMMAND(ID_ADVANCED_ADVANVCED1, &CClientUISU::OnAdvancedAdvanvced1)
	ON_COMMAND(ID_ADVANCED_ADVANVCED2, &CClientUISU::OnAdvancedAdvanvced2)
	ON_COMMAND(ID_DEVICEOP_LOAD, &CClientUISU::OnDeviceopLoad)
	ON_COMMAND(ID_DEVICEOP_SAVE, &CClientUISU::OnDeviceopSave)
	ON_COMMAND(ID_INFO_INFO, &CClientUISU::OnInfo)
	ON_COMMAND(ID_DEVICEOP_REFRESH, &CClientUISU::OnRefresh)
	ON_WM_TIMER()
END_MESSAGE_MAP()

// CClientUISU 构造/析构

CClientUISU::CClientUISU()
...
int CClientUISU::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL CClientUISU::PreCreateWindow(CREATESTRUCT& cs)
{
	if( !CFrameWndEx::PreCreateWindow(cs) )
		return FALSE;
	// TODO: 在此处通过修改
	//  CREATESTRUCT cs 来修改窗口类或样式
	m_strTitle.LoadString(IDS_SUTITLE);
	cs.dwExStyle &= ~WS_EX_CLIENTEDGE;
	cs.lpszClass = AfxRegisterWndClass(0);
	return TRUE;
}

void CClientUISU::InitializeRibbon()
{
	BOOL bNameValid;
...
*/
	...
	//// 1.7 点位号
	//bNameValid = strTemp.LoadString(IDS_POINTNUMBER);
	//ASSERT(bNameValid);
	//CMFCRibbonPanel* pPanelPointNumber = pCategoryDeviceSettings->AddPanel(strTemp, m_PanelImages.ExtractIcon(27));

	//// 1.7.1 点位号
	//CMFCRibbonComboBox* pComboPointNum = new CMFCRibbonComboBox(UINT(-1), TRUE, 80);
	//pComboPointNum->AddItem(_T("0"));
	//pComboPointNum->AddItem(_T("1"));
	//pComboPointNum->AddItem(_T("2"));
	//pComboPointNum->AddItem(_T("3"));
	//pComboPointNum->SelectItem(0);
	//pPanelPointNumber->Add(pComboPointNum);
	...
	//lstQATCmds.AddTail(ID_FILE_SUINT_DIRECT);
...
BOOL CClientUISU::CreateDockingWindows()
{
	BOOL bNameValid;

	// 创建输出窗口
	CString strOutputWndSU;
	bNameValid = strOutputWndSU.LoadString(IDS_OUTPUT_WND);
	ASSERT(bNameValid);
	if (!m_wndCurve.Create(strOutputWndSU, this, CRect(0, 0, 200, 200),TRUE, ID_WND_OUTPUT, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_BOTTOM | CBRS_FLOAT_MULTI))
	{
		TRACE0("未能创建输出窗口\n");
		return FALSE; // 未能创建
	}
...
void CClientUISU::SetDockingWindowIcons(BOOL /*bHiColorIcons*/)
{
	//HICON hPropertiesBarIcon = (HICON) ::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(bHiColorIcons ? IDI_SUOPERTIES_WND_HC : IDI_SUOPERTIES_WND), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), 0);
...
#ifdef _DEBUG
void CClientUISU::AssertValid() const
{
	CFrameWndEx::AssertValid();
}

void CClientUISU::Dump(CDumpContext& dc) const
{
	CFrameWndEx::Dump(dc);
}
#endif //_DEBUG
...
void CClientUISU::OnSetFocus(CWnd* /*pOldWnd*/)
{
	// 将焦点前移到视图窗口
	m_wndView.SetFocus();
}

BOOL CClientUISU::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
	// 让视图第一次尝试该命令
...
void CClientUISU::OnApplicationLook(UINT id)
{
	CWaitCursor wait;
...
void CClientUISU::OnUpdateApplicationLook(CCmdUI* pCmdUI)
{
	pCmdUI->SetRadio(theApp.m_nAppLook == pCmdUI->m_nID);
}
void CClientUISU::OnBasicLevel()
{
	m_wndView.Show(IDD_LEVEL_ADJUSTMENT_SU);
}

void CClientUISU::OnBasicApplication()
{
	m_wndView.Show(IDD_APPLICATION_SU);
}

void CClientUISU::OnBasicDamping()
{
	m_wndView.Show(IDD_DAMPING_SU);
}

void CClientUISU::OnBasicLinerization()
{
	m_wndView.Show(IDD_LINEARIZATION_SU);
}

void CClientUISU::OnBasicOther()
{
	m_wndView.Show(IDD_OTHERSETTINGS_SU);
}

void CClientUISU::OnDisplayDisplay()
{
	m_wndView.Show(IDD_DISPLAY_SU);
}

void CClientUISU::OnDiagnosisDiagnosis()
{
	m_wndView.Show(IDD_DIAGNOSIS_SU);
}

void CClientUISU::OnServiceCurrentoutput()
{
	m_wndView.Show(IDD_CURRENTOUTPUT_SU);
}

void CClientUISU::OnServiceReset()
{
	m_wndView.Show(IDD_RESET_SU);
}

void CClientUISU::OnServicePin()
{
	m_wndView.Show(IDD_PIN_SU);
}

void CClientUISU::OnServiceOther()
{
	m_wndView.Show(IDD_SERVICEOTHER_SU);
}

void CClientUISU::OnServiceFalseecho()
{
	m_wndView.Show(IDD_FALSEECHO_SU);
}

void CClientUISU::OnAdvancedAdvanvced1()
{
	m_wndView.Show(IDD_ADVANCEDSETUP1_SU);
}

void CClientUISU::OnAdvancedAdvanvced2()
{
	m_wndView.Show(IDD_ADVANCEDSETUP2_SU);
}

void CClientUISU::OnInfo()
{
	m_wndView.Show(IDD_INFO);
}

void CClientUISU::OnDeviceopLoad()
{
	m_wndView.Load();
}

void CClientUISU::OnDeviceopSave()
{
	m_wndView.Save();
}

void CClientUISU::OnRefresh()
{
	theApp.m_pClient->RefreshId();
}

void CClientUISU::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (theApp.m_pClient->IsOnline())
	{
		...
		for (int i = 1; i <= 3; i++)
		{
// 			theApp.m_pClient->SetDeviceNum(i);
// 			
// 			float nullHight;
// 			theApp.m_pClient->LoadData(0x990, nullHight);
// 			theApp.m_pClient->SetVarInvalid(0x990);

			SetNullHight(gNullHight[i-1], i);
...
void CClientUISU::SetSendBytes( long )
{

}

void CClientUISU::SetRecvBytes( long )
{

}

void CClientUISU::SetErrBytes( long )
{
