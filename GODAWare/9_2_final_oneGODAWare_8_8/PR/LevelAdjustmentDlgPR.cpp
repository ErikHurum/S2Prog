...
IMPLEMENT_DYNAMIC(CLevelAdjustmentDlgPR, COperationDlg)

CLevelAdjustmentDlgPR::CLevelAdjustmentDlgPR()
{
...
void CLevelAdjustmentDlgPR::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CLevelAdjustmentDlgPR, COperationDlg)
END_MESSAGE_MAP()


void CLevelAdjustmentDlgPR::LoadData()
{
	CProgress progress;
	float dbData;
	CString str;

	theApp.m_pClient->LoadData(0x6, dbData);
...
void CLevelAdjustmentDlgPR::SaveData()
{
	CProgress progress;
	double dbData;
	CString str;
	...
	dbData = _tstof(str);
	theApp.m_pClient->SaveData(0x6, float(dbData));
