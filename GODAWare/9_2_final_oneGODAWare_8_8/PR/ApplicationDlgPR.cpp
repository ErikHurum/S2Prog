...
IMPLEMENT_DYNAMIC(CApplicationDlgPR, COperationDlg)

CApplicationDlgPR::CApplicationDlgPR()
: m_nMultiEcho(0)
{
...
BEGIN_MESSAGE_MAP(CApplicationDlgPR, COperationDlg)
	ON_CBN_SELCHANGE(IDC_COMBO_MEDIATYPE, &CApplicationDlgPR::OnCbnSelchangeComboMediatype)
	ON_BN_CLICKED(IDC_CHECK_LOWDK, &CApplicationDlgPR::OnBnClickedCheckLowdk)
END_MESSAGE_MAP()



// CApplicationDlgPR 消息处理程序



void CApplicationDlgPR::DoDataExchange(CDataExchange* pDX)
{
...
void CApplicationDlgPR::LoadData()
{
	CProgress progress;
	CString str;
	UINT8 int8Data;
...
void CApplicationDlgPR::SaveData()
{
	CProgress progress;
	CString str;
	int intData;
...
void CApplicationDlgPR::OnCbnSelchangeComboMediatype()
{
...
void CApplicationDlgPR::OnBnClickedCheckLowdk()
{
	if (m_lowDK.GetCheck())
	{
		m_nullHight.SetReadOnly(false);
