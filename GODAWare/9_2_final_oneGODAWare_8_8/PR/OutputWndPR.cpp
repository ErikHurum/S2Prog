...
BEGIN_MESSAGE_MAP(COutputWndPR, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
END_MESSAGE_MAP()

int COutputWndPR::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndEchoCurve.Create(IDD_ECHOCURVE_PR, &m_wndTabs) ||
		!m_wndEchoData.Create(IDD_ECHODATA_PR, &m_wndTabs) ||
		!m_wndSimulate.Create(IDD_SIMULATE_PR, &m_wndTabs)||
		!m_wndOutputMapping.Create(IDD_OUTPUTMAPPING_PR, &m_wndTabs) ||
		!m_wndTrend.Create(IDD_TREND_PR, &m_wndTabs))
	{
		TRACE0("未能创建输出窗口\n");
		return -1;      // 未能创建
	}
...
void COutputWndPR::OnSize(UINT nType, int cx, int cy)
{
...
BOOL COutputWndPR::CanBeClosed() const
{
	return false;
}
