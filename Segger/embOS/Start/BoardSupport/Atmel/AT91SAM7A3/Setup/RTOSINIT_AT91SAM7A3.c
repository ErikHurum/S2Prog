...
----------------------------------------------------------------------
File    : RTOSInit_AT91SAM7A3.c
          for ATMEL AT91SAM7A3 CPUs
...
*       Currently the standard code supports UART0 to UART2,
*       but disables the UART per default, because the eval kit has
*       no UART connector
*/
#ifndef   OS_UART
  #define OS_UART (-1)
...
#define _USART2_BASE_ADDR  (0xFFFC8000)
...
*/
  ...
  #elif (OS_UART == 2)
    #define OS_UART_BASE_ADDR  (_USART2_BASE_ADDR)
    #define OS_UART_ID         (_US2IRQ_ID)
    #define OS_UART_RX_PIN     (9)
    #define OS_UART_TX_PIN     (10)
  #endif
