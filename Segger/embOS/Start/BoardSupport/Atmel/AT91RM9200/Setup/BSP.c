...
#define _LED0_BIT      (0) /* USER LED1 on AT91RM9200-EK board, active low */
#define _LED1_BIT      (1) /* USER LED2 on AT91RM9200-EK board, active low */
...
void BSP_SetLED(int Index) {
  _LEDPORT_CODR = (1 << Index);         /* Switch on LED 0  */
}

void BSP_ClrLED(int Index) {
  _LEDPORT_SODR = (1 << Index);         /* Switch off LED 0 */
}

void BSP_ToggleLED(int Index) {
  if (_LEDPORT_STATE & (1 << Index)) {  /* LED is switched off */
    BSP_SetLED(Index);
  } else {
    BSP_ClrLED(Index);
  }
