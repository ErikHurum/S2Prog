...
#define _USART0_BASE_ADDR  (0xFFF8C000)
#define _USART1_BASE_ADDR  (0xFFF90000)
#define _USART2_BASE_ADDR  (0xFFF94000)

/*      Debug unit */
#define _DBGU_BASE_ADDR    (0xFFFFEE00)
...
#define _PIOD_BASE_ADDR    (0xfffff800)
#define _PIOE_BASE_ADDR    (0xfffffA00)
...
#define _PMC_SCER     (*(volatile OS_U32*) (_PMC_BASE_ADDR + 0x10))  /* System clock enable register */
...
#define _PIOD_ID        (4)
#define _PIOE_ID        (4)
#define _US0IRQ_ID      (7)   /* USART Channel 0 interrupt */
#define _US1IRQ_ID      (8)   /* USART Channel 1 interrupt */
#define _US2IRQ_ID      (9)   /* USART Channel 2 interrupt */
...
#define _MATRIX_BASE_ADDR   (0xFFFFEC00)                                // MATRIX Base Address

#define _MATRIX_MCFG   (*(volatile OS_U32*) (_MATRIX_BASE_ADDR + 0x000)) // MATRIX Master configuration register
#define _MATRIX_EBICSA (*(volatile OS_U32*) (_MATRIX_BASE_ADDR + 0x120)) // MATRIX EBI Chip Select Assignment register

/*      SDRAM controller */
#define _SDRAMC_BASE_ADDR  (0xFFFFE200)   // SDRAMC Base Address
...
#define _SMC_BASE_ADDR   (0xFFFFE400)                   // SMC  Base Address
...
*
* NOTES: (1) Settings used here are for AT91SAM9263-EK eval board
...
static void _InitClocks(OS_U32 PLLA_Val, OS_U32 PLLB_Val, OS_U32 MCKR_Val) {
  ...
  //
  _PMC_MOR = ((64/8) << 8) + 0x01; // Startup time = 64 slow clocks, Main oscillator enable
...
static void _InitSDRAM(void) {
  ...
  if ((_MATRIX_EBICSA & (1 << 1)) == 0) {
    ...
    *(volatile int*) (SDRAM_BASE_ADDR) = 0x0;       // Write to SDRAM
    //
...
*/
    ...
    #define OS_PIO_UART_ID        (_PIOA_ID)
    #define OS_UART_RX_PIN        (26)
    #define OS_UART_TX_PIN        (27)
  #elif (OS_UART == 1)
    #define _PIO_US_BASE_ADDR     (_PIOD_BASE_ADDR)
    #define _OS_UART_BASE_ADDR    (_USART1_BASE_ADDR)
    #define OS_UART_ID            (_US1IRQ_ID)
    #define OS_PIO_UART_ID        (_PIOD_ID)
    #define OS_UART_RX_PIN        (1)
    #define OS_UART_TX_PIN        (0)
  #elif (OS_UART == 2)
    #define _PIO_US_BASE_ADDR     (_PIOD_BASE_ADDR)
    #define _OS_UART_BASE_ADDR    (_USART2_BASE_ADDR)
    #define OS_UART_ID            (_US2IRQ_ID)
    #define OS_PIO_UART_ID        (_PIOD_ID)
    #define OS_UART_RX_PIN        (2)
    #define OS_UART_TX_PIN        (3)
  #endif
...
void OS_COM_Init(void) {             /* Initialize UART, enable UART interrupts */
  _PMC_PCER  = (1 << OS_UART_ID)     /* Enable peripheral clock for selected USART */
             | (1 << OS_PIO_UART_ID) /* Enable the PIO port where UART pins are located */
             ;
...
OS_INTERWORK int __low_level_init(void) {
  ...
  OS_ARM_MMU_AddTTEntries(&_TranslationTable[0], OS_ARM_CACHEMODE_C_B,     0x200, 0x200, 0x020);    // SDRAM, first 32MB, cacheable, bufferable
  OS_ARM_MMU_AddTTEntries(&_TranslationTable[0], OS_ARM_CACHEMODE_NC_NB,   0x220, 0x220, 0x020);    // SDRAM, second 32MB, NON cacheable, NON bufferable
