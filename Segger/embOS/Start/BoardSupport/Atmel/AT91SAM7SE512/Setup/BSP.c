...
#define _LED0_BIT      (1)  /* LED1 on SAM7SE-EK board, active low */
#define _LED1_BIT      (2)  /* LED2 on SAM7SE-EK board, active low */
...
void BSP_SetLED(int Index) {
  _LEDPORT_CODR = (1 << (Index + 1));         /* Switch on LED 0  */
}

void BSP_ClrLED(int Index) {
  _LEDPORT_SODR = (1 << (Index + 1));         /* Switch off LED 0 */
}

void BSP_ToggleLED(int Index) {
  if (_LEDPORT_STATE & (1 << (Index + 1))) {  /* LED is switched off */
    BSP_SetLED(Index);
  } else {
    BSP_ClrLED(Index);
  }
