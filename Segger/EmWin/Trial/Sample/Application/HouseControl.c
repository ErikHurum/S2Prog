...
static OBJECT _Garage         = {0, 0, { 40, 732,  90, 777}};
...
static void _DrawElevator(int x, int y, int w, int h, int Level) {
  ...
  GUI_DrawRect(x + 1, y + 79, x + w - 1, y + h - 1);
  /* Draw level number */
...
static void _DrawFrame(int x, int y, int w, int h, int Light) {
  ...
  GUI_SetColor(Color << 16 | Color << 8 | Color);
  GUI_SetPenSize(1);
  GUI_DrawLine(x + 1, y + 1, x + w - 2, y + h - 2);
  GUI_DrawLine(x + w - 2, y + 1, x + 1, y + h - 2);
}
...
static void _DrawSmallElevatorDoor(int Level, int x0, int y0, int Light) {
  ...
  GUI_SetColor(Color << 16);
  GUI_DrawRect(x0 + 14, y0 + 63, x0 + 23, y0 +  77);
...
static void _DrawLevel(int n, int y) {
  if (n != 5) {
    _DrawFrame(35, y, 130, 130, _Level[n].Light.ActV);
    _DrawElevator(55, y + 2, 30, 126, n);
    if (n != 0) {
  ...
  case 0:
    ...
    _DrawDoor(115, y + 82, 30, 46);
    _DrawGarage(40, y + 82, 50, 46, _Garage.ActV);
    break;
  case 1:
...
static void _DrawHouseMap(int x0, int y0) {
  ...
  GUI_SetColor(GUI_BLACK);
  GUI_DrawRect(x0 + 31, y0 + 219, x0 + 41, y0 + 233);
  ...
  GUI_SetColor(GUI_BLACK);
  GUI_DrawRect(x0 + 9, y0 + 219, x0 + 25, y0 + 233);
  /* Draw Jalousie 1 */
...
static void _NEC_Demo(void) {
  ...
  _Logo.XSize   = ((I32)_LogoBitmap.XSize * LOGOMULX) / 1000;
  _Logo.YSize   = ((I32)_LogoBitmap.YSize * LOGOMULY) / 1000;
