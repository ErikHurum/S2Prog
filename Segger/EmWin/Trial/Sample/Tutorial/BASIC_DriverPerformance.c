...
*       _InitXbppDDP
*/
static void _InitXbppDDP(PARAM_XBPP * pParam) {
  U32 MaxColor;
  GUI_COLOR Color;
  int x, y, BitsPerPixel, BytesPerPixel;
  ...
  pParam->Bm.XSize        = XSIZE_XBPP;
  pParam->Bm.YSize        = YSIZE_XBPP;
  pParam->Bm.BytesPerLine = XSIZE_XBPP;
  pParam->Bm.BitsPerPixel = BytesPerPixel << 3;
  pParam->Bm.pData        = (U8 *)pParam->aPixels;
  pParam->Bm.pPal         = NULL;
  pParam->Bm.pMethods     = NULL;
  if (BitsPerPixel > 8) {
    pParam->Bm.BytesPerLine <<= 1;
  }
  Color = 0;
  switch (BytesPerPixel) {
  case 1: {
      U8 * pData;
      pData = (U8 *)pParam->aPixels;
...
static void _ExecXbppDDP(void * p) {
  PARAM_XBPP * pParam = (PARAM_XBPP *)p;
  GUI_DrawBitmap(&pParam->Bm, 0, 0);
  GUI_DrawBitmap(&pParam->Bm, 1, 1);
}
...
*       _Measure
*/
static float _Measure(void (* pOutput)(void * p), void * p) {
  int t0;
  int Period = LCDTEST_MEASURE_PERIOD;
  int Cnt  = 0;
  int t;
  (*pOutput)(p);  // Eliminate one time effects like color conversion
  t0 = GUI_GetTime();
...
static void _LCDBench(void) {
  ...
  FixedPalette = LCD_GetFixedPaletteEx(0);
  _InitXbppDDP(&Param);
  /*
  * Say hi !
  */
  ...
  */
  aPixelrateBitmap[4]= _Measure(&_ExecXbppDDP, (void *)&Param) * (float)2 * XSIZE_XBPP * YSIZE_XBPP;
  ...
  */
  ...
  GUI_DispString(" colors, Fixed palette: ");
  GUI_DispDecMin(FixedPalette);
  GUI_DispString("\n\nMIRROR_X ");
