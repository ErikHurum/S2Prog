...
#if (GUI_WINSUPPORT) && (GUI_SUPPORT_MEMDEV) && (GUI_BYTESPERPIXEL == 4)
...
void GUI_MEMDEV_DrawPerspectiveX(GUI_MEMDEV_Handle hMem, int x, int y, int h0, int h1, int xSize, int dy);
...
static int _GetImages(GUI_MEMDEV_Handle * phMem, GUI_MEMDEV_Handle * phMemNarrow, int MaxItems) {
  ...
  for (; i < MaxItems; i++) {
    *(phMem + i) = GUI_MEMDEV_CreateFixed(0, 0, 150, 150, GUI_MEMDEV_NOTRANS, GUI_MEMDEV_APILIST_32, &LCD_API_ColorConv_888);
    GUI_MEMDEV_Select(*(phMem + i));
...
void GUIDEMO_ImageFlow(void) {
  ...
  if (_GetImages(ahMem, ahMemNarrow, GUI_COUNTOF(ahMem)) == 0) {
    ...
    GUI_DispStringHCenterAt("emWin - ImageFlow Demo", 160,   6);
    GUI_DispStringHCenterAt("emWin - ImageFlow Demo", 160, 246);

    TimeEnd = GUI_GetTime() + 17000;
    Index = 0;
    do {
