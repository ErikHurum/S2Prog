...
static int _xPos, _yPos, _xAdd, _yAdd;
...
static void _cbWin(WM_MESSAGE * pMsg) {
  ...
  case APP_TIMER:
    ...
    if        (_xAdd < 0) {
      if (_xPos == (xSizeWin - xSizeBmp)) {
        _xAdd = 0;
        _yAdd = -2;
      }
    } else if (_yAdd < 0) {
      if (_yPos == (ySizeWin - ySizeBmp)) {
        _yAdd = 0;
        _xAdd = 2;
      }
    } else if (_xAdd > 0) {
    ...
    if (xSizeWin < xSizeBmp) {
      _xPos += _xAdd;
    }
    if (ySizeWin < ySizeBmp) {
      _yPos += _yAdd;
    }
    WM_InvalidateWindow(hWin);
    break;
  case WM_PAINT:
    GUI_DrawBitmap(&_bmMap_400x320, _xPos, _yPos);
    break;
  }
}
...
static void _TransparentDialog(void) {
  WM_HWIN hWin, hFrame, hClient, hSlider0, hSlider1;
  int xSize, ySize, TimeNow, TimeNext, TimeStart, TimeUsed, Value;
  ...
  //
  hWin = WM_CreateWindow(0, 0, xSize, ySize, WM_CF_SHOW, _cbWin, 0);
  //
  // Create dialog
  //
  ...
  do {
    ...
    if (TimeNow >= TimeNext) {
      TimeNext += PERIOD;
      WM_SendMessageNoPara(hWin, APP_TIMER);
  ...
  } while (((GUI_GetTime() - TimeStart) < DURATION) && (GUIDEMO_CheckCancel() == 0));
  //
  // Free memory
  //
  ...
  TEXT_SetDefaultFont(&GUI_Font13_1);
