...
U8                     WM_IsActive;
U16                    WM__CreateFlags;
WM_HWIN                WM__ahCapture[GUI_NUM_LAYERS];
WM_HWIN                WM__ahWinFocus[GUI_NUM_LAYERS];
...
WM_HWIN WM_CreateWindowAsChild( int x0, int y0, int width, int height
  ...
  if ((hWin = (WM_HWIN) GUI_ALLOC_AllocZero(NumExtraBytes + sizeof(WM_Obj))) == 0) {
    ...
    #if WM_SUPPORT_NOTIFY_VIS_CHANGED
      if (hParent) {
        WM__SendMsgNoData(hParent, WM_NOTIFY_VIS_CHANGED);             /* Notify window that visibility may have changed */
      }
    #endif
...
void WM_DeleteWindow (WM_HWIN hWin) {
  ...
  /* Make sure that focus is set to an existing window */
    if (WM__ahWinFocus[WM__TOUCHED_LAYER] == hWin) {
      WM__ahWinFocus[WM__TOUCHED_LAYER] = 0;
    }
    if (WM__ahCapture[WM__TOUCHED_LAYER] == hWin) {
  ...
  /* Select a valid window */
    ...
    #else
      WM_SelectWindow(WM__ahDesktopWin[WM__TOUCHED_LAYER]);
    #endif
  } else {
...
*
*  Purpose:
*    Returns the layer of the top level window
*/
#if GUI_NUM_LAYERS > 1
int WM__GetTopLevelLayer(WM_HWIN  hWin) {
  ...
  Layer = _DesktopHandle2Index(hTop);
  return Layer;
}
#endif
...
*
*  Purpose:
*    Selects the layer of the top level window
*/
#if GUI_NUM_LAYERS > 1
void WM__SelectTopLevelLayer(WM_HWIN  hWin) {
  int Layer;
  Layer = WM__GetTopLevelLayer(hWin);
  if (Layer >= 0) {
...
static void _Paint1(WM_HWIN hWin, WM_Obj* pWin) {
  ...
  if ((pWin->cb != NULL)  && (Status & WM_SF_ISVIS)) {
    ...
    if (Status & WM_SF_LATE_CLIP) {
      /* Late clipping: Only one WM_PAINT message is send. We iterate over visible rectangles in the drawing routines */
      Msg.hWin   = hWin;
      Msg.MsgId  = WM_PAINT;
      ...
      /* Early clipping: One WM_PAINT message is send for every visible rectangle; Iteration is done at this level. */
      /* Drawing routines do not iterate */
      ...
      if ((pChild->Status & (WM_SF_HASTRANS | WM_SF_ISVIS))   /* Transparent & visible ? */
        ...
        if (GUI_RectsIntersect(&pChild->Rect, &pWin->InvalidRect)) {
          ...
          if(_Paint1Trans(hChild, pChild)) {
            /* Within the paint event the application is allowed to deal with memory devices.
        ...
        WM_SelectWindow(hWin);
          ...
          if (pWin->Status & WM_SF_MEMDEV) {
            ...
            if (pWin->hParent == 0) {
              ...
              if ((pWin->Status & WM_SF_INVALID_POSITION) && (pWin->hMem)) {
                //
                // Contents of window is valid, so only the memory device needs to be drawn
                //
                GUI_MEMDEV_CopyToLCDAt(pWin->hMem, pWin->Rect.x0, pWin->Rect.y0);
              } else {
                //
                // Draw and fill static memory device
                //
                GUI_MEMDEV_DrawStatic(&pWin->Rect, _cbPaintMemDev, &Info, &pWin->hMem, Flags);
              }
              //
              // Validate position
              //
            ...
            #endif
            pWin   = WM_H2P(hWin); /* Needs strictly to be recalculated after creating a memory device!!! */
            *ppWin = pWin;
          } else
        #endif
        {
...
static void _DrawNext(void) {
  int UpdateRem = 1;
  WM_HWIN iWin = (_NextDrawWin == WM_HWIN_NULL) ? WM__FirstWin : _NextDrawWin;
  ...
  _NextDrawWin = iWin;   /* Remember the window */
...
void WM_Init(void) {
  int i;//, xSize, ySize;
	...
	if (!_IsInited) {
	  _NextDrawWin = WM__FirstWin = WM_HWIN_NULL;
	  ...
	  */
    #if GUI_NUM_LAYERS == 1
      //xSize = LCD_GetVXSizeEx(0);
      //ySize = LCD_GetVYSizeEx(0);
      //WM__ahDesktopWin[0] = WM_CreateWindow(0, 0, xSize, ySize, WM_CF_SHOW, cbBackWin, 0);
    ...
    #else
    {
      ...
      for (i = 0; i < GUI_NUM_LAYERS; i++) {
        if (GUI_DEVICE__GetpDriver(i)) {
          //xSize = LCD_GetVXSizeEx(i);
          //ySize = LCD_GetVYSizeEx(i);
          //WM__ahDesktopWin[i] = WM_CreateWindowAsChild(0, 0, xSize, ySize, WM_UNATTACHED, WM_CF_SHOW, cbBackWin, 0);
    ...
    for (i = 0; i < GUI_NUM_LAYERS; i++) {
      WM__AddCriticalHandle(&WM__aCHWinModal[i]);
      WM__AddCriticalHandle(&WM__aCHWinLast[i]);
      #if GUI_SUPPORT_MOUSE
        WM__AddCriticalHandle(&WM__aCHWinMouseOver[i]);
      #endif
    }
