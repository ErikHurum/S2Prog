...
**********************************************************************
*/
static GUI_PID_STATE _aStateLast[GUI_NUM_USERS];
...
WM_CRITICAL_HANDLE  WM__aCHWinModal[GUI_NUM_USERS];
WM_CRITICAL_HANDLE  WM__aCHWinLast[GUI_NUM_USERS];
#if GUI_SUPPORT_MOUSE
  WM_CRITICAL_HANDLE  WM__aCHWinMouseOver[GUI_NUM_USERS];
#endif
...
int WM_HandlePID(void) {
  ...
  WM__AddCriticalHandle(&CHWin);
    ...
    if (StateNew.Layer < GUI_NUM_USERS) {
      WM__TouchedLayer = StateNew.Layer;
    }
  #endif
  GUI_GetLayerPosEx(WM__TOUCHED_LAYER, &Point);
  StateNew.x -= Point.x;
  StateNew.y -= Point.y;
  if (((_aStateLast[WM__TOUCHED_LAYER].x != StateNew.x) || 
      ...
      if (StateNew.Layer >= GUI_NUM_CURSORS) {
        StateNew.Layer = 0;
      }
    ...
    if (WM__aCHWinMouseOver[WM__TOUCHED_LAYER].hWin && (WM__aCHWinMouseOver[WM__TOUCHED_LAYER].hWin != CHWin.hWin)) {
    ...
    if (WM__IsInModalArea(CHWin.hWin)) {
      ...
      if (_aStateLast[WM__TOUCHED_LAYER].Pressed | StateNew.Pressed) {    /* Only if pressed or just released */
         ...
         * b) PID is moved out
         */
        if (WM__aCHWinLast[WM__TOUCHED_LAYER].hWin != CHWin.hWin) {
