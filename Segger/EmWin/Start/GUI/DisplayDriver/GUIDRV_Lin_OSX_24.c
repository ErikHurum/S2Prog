...
static void _DrawBitLine32BPP(GUI_DEVICE * pDevice, int x, int y, U32 const GUI_UNI_PTR * p, int xsize) {
  ...
  //
  Off = XY2OFF32(pContext->vxSizePhys, xPhys, yPhys);
  Add = (pContext->vySize  + pContext->vySize + pContext->vySize) >> 2;
  switch (xPhys & 3) {
  case 0:
    while (xsize--) {
      PixelIndex = *p++;
...
void GUIDRV_Lin_OSX_24_C(void);   // Avoid empty object files
void GUIDRV_Lin_OSX_24_C(void) {}
