...
static void _DrawBitLine8BPP_Swap(GUI_DEVICE * pDevice, unsigned x, unsigned y, U8 const GUI_UNI_PTR * pData, int ySize, const LCD_PIXELINDEX * pTrans, int BytesPerLine, U8 Pos) {
  ...
  if (pTrans == NULL) {
    return; // Because it makes no sense to use 8bpp bitmaps without palette with a 32bpp configuration
  }
  pContext = (DRIVER_CONTEXT *)pDevice->u.pContext;
  y0       = pContext->vxSize - x - 1;
  x0       = y;
...
*       Draw Bitmap 32 BPP
*/
static void _DrawBitLine32BPP_Swap(GUI_DEVICE * pDevice, int x, int y, U32 const GUI_UNI_PTR * pData, int ySize, int BytesPerLine, U8 Pos) {
  DRIVER_CONTEXT * pContext;
  U32 Off;
  U32 * pDest;
  int WordsPerLine;
...
static void _DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
  ...
  case 32:
    for (i = Diff; i < xSize; i++) {
      _DrawBitLine32BPP_Swap(pDevice, x0 + i, y0, (U32 *)(pData + (i << 2)), ySize, BytesPerLine, i);
    }
    break;
  }
}
...
void GUIDRV_Lin_OSY_32_C(void);   // Avoid empty object files
void GUIDRV_Lin_OSY_32_C(void) {}
