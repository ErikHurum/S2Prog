...
#define XY2OFF16(vxSizePhys, x, y)    ((U32)y * (vxSizePhys >> 2) + ((U32)x >> 2))
#define XY2OFF32(vxSizePhys, x, y)    ((U32)y * (vxSizePhys >> 3) + ((U32)x >> 3))
...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
  ...
  Shift    = ((x & 1) << 2) + ((((x & 3) >> 1) ^ LCD_ENDIAN_BIG) << 3);
  Data    &= ~(0xF << Shift);
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  ...
  Data     = READ_MEM16(pContext->VRAMAddr, Off);
  Shift    = ((x & 1) << 2) + ((((x & 3) >> 1) ^ LCD_ENDIAN_BIG) << 3);
  PixelIndex = (Data & (0xF << Shift)) >> Shift;
  return PixelIndex;
}
...
static void _DrawBitLine1BPP(GUI_DEVICE * pDevice, unsigned x, unsigned y, U8 const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  if (Mode == 0) {
    ...
    //
    if (Index0 == Index1) {
      ColorIndexOld = LCD_COLORINDEX;  // Save foreground color
  ...
  if ((GUI_Context.DrawMode & LCD_DRAWMODE_TRANS) == 0) {
    ...
    if (pTrans) {
      ...
      if (NumPixel_0) {
        ...
        for (i = NumPixel_0; (i < 8) && xsize; i++, xsize--) {
          Shift = 4 * (1 - CurrentPixel);
          Pixel = (*p & (0xF << Shift)) >> Shift;
          Index = *(pTrans + Pixel);
          ColorMask |= Index << (4 * i);
          if (++CurrentPixel == 2) {
            CurrentPixel = 0;
...
void GUIDRV_Lin_4_C(void);   // Avoid empty object files
void GUIDRV_Lin_4_C(void) {}
