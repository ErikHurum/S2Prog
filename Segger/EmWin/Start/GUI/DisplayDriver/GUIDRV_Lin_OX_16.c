...
static void _DrawHLine  (GUI_DEVICE * pDevice, int x0, int y,  int x1) {
  ...
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
    ...
    //
    temp = x0;
    x0 = (pContext->vxSize - 1 - x1);
    x1 = (pContext->vxSize - 1 - temp);
    //
    // Draw...
    //
...
static void _DrawBitLine1BPP(GUI_DEVICE * pDevice, unsigned x, unsigned y, U8 const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  case LCD_DRAWMODE_TRANS:
    do {
      if (Pixels & 1) {
        WRITE_MEM16(pContext->VRAMAddr, Off, Index1);
      }
      Off--;
      PixelCnt--;
      Pixels >>= 1;
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * p, int xsize) {
  ...
  pDest = OFF2PTR16(pContext->VRAMAddr, Off);
  while (xsize >= 8) {
    WRITE_MEM16P(pDest - 0, *(p + 0));
    WRITE_MEM16P(pDest - 1, *(p + 1));
    WRITE_MEM16P(pDest - 2, *(p + 2));
    WRITE_MEM16P(pDest - 3, *(p + 3));
    WRITE_MEM16P(pDest - 4, *(p + 4));
    WRITE_MEM16P(pDest - 5, *(p + 5));
    WRITE_MEM16P(pDest - 6, *(p + 6));
    WRITE_MEM16P(pDest - 7, *(p + 7));
    xsize -= 8;
    p     += 8;
    pDest -= 8;
  }
...
static void _ReadRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1, LCD_PIXELINDEX * pBuffer) {
  ...
  pContext = (DRIVER_CONTEXT *)pDevice->u.pContext;
  x0 = (pContext->vxSize - 1 - (x0));
  x1 = (pContext->vxSize - 1 - (x1));
  p = (U16 *)pBuffer;
  x = x0;
  do {
    Off = XY2OFF16(pContext->vxSizePhys, x0, y0);
    NumPixelsPerLine = x0 - x1 + 1;
    do {
      *p++ = READ_MEM16(pContext->VRAMAddr, Off);
      Off--;
...
void GUIDRV_Lin_OX_16_C(void);   // Avoid empty object files
void GUIDRV_Lin_OX_16_C(void) {}
