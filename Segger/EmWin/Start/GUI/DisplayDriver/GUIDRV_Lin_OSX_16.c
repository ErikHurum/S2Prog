...
#define XY2OFF32(vxSizePhys, x, y)    ((U32)y * (vxSizePhys >> 1) + ((U32)x >> 1))
#define XY2OFF16(vxSizePhys, x, y)    ((U32)y *  vxSizePhys       +       x      )

#define LOG2PHYS_X(vxSize, vySize, x, y) vySize - 1 - (y)
...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
  ...
  //
  pContext = (DRIVER_CONTEXT *)pDevice->u.pContext;
  Off      = XY2OFF16(pContext->vxSizePhys, xPhys, yPhys);
  WRITE_MEM16(pContext->VRAMAddr, Off, PixelIndex);
}
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  ...
  //
  Off        = XY2OFF16(pContext->vxSizePhys, xPhys, yPhys);
  PixelIndex = READ_MEM16(pContext->VRAMAddr, Off);
  return PixelIndex;
}
...
static void _DrawVLine  (GUI_DEVICE * pDevice, int x, int y0,  int y1) {
  ...
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
    ...
    pContext  = (DRIVER_CONTEXT *)pDevice->u.pContext;
    x0        = pContext->vySize - y1 - 1;
    x1        = pContext->vySize - y0 - 1;
    y         = x;
...
static void _DrawBitLine1BPP_Swap(GUI_DEVICE * pDevice, unsigned x, unsigned y, U8 const GUI_UNI_PTR * pData, int ySize, const LCD_PIXELINDEX * pTrans, int BytesPerLine, U8 Pos) {
  ...
  pContext  = (DRIVER_CONTEXT *)pDevice->u.pContext;
  y0        = x;
  x0        = pContext->vySize - y - 1;
  IndexMask = pDevice->pColorConvAPI->pfGetIndexMask();
  Off       = XY2OFF16(pContext->vxSizePhys, x0, y0);
  pDest     = OFF2PTR16(pContext->VRAMAddr, Off);
  Index0    = *(pTrans + 0);
  Index1    = *(pTrans + 1);
  Mask      = 0x80 >> (Pos & 0x7);
  ...
  case LCD_DRAWMODE_XOR:
    do {
      if (*pData & Mask) {
        Index = READ_MEM16(pContext->VRAMAddr, Off);
        Index ^= IndexMask;
        WRITE_MEM16(pContext->VRAMAddr, Off, Index);
      }
      pData += BytesPerLine;
      Off--;
    } while (--ySize);
...
*       Draw Bitmap 8 BPP
*/
static void _DrawBitLine8BPP_Swap(GUI_DEVICE * pDevice, unsigned x, unsigned y, U8 const GUI_UNI_PTR * pData, int ySize, const LCD_PIXELINDEX * pTrans, int BytesPerLine, U8 Pos) {
  DRIVER_CONTEXT * pContext;
...
*       Draw Bitmap 16 BPP
*/
static void _DrawBitLine16BPP_Swap(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * pData, int ySize, int BytesPerLine, U8 Pos) {
  DRIVER_CONTEXT * pContext;
  U32 Off;
  U16 * pDest;
  int WordsPerLine;
  ...
  y            = pContext->vySize - y - 1;
  Off          = XY2OFF16(pContext->vxSizePhys, y, x);
  pDest        = OFF2PTR16(pContext->VRAMAddr, Off);
  while (ySize >= 4) {
    do {
...
static void _DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
  ...
  case 8:
    for (i = 0; i < xSize; i++) {
      _DrawBitLine8BPP_Swap(pDevice, x0 + i, y0, pData + i, ySize, pTrans, BytesPerLine, i);
    }
    break;
  case 16:
    for (i = Diff; i < xSize; i++) {
      _DrawBitLine16BPP_Swap(pDevice, x0 + i, y0, (U16 *)(pData + (i << 1)), ySize, BytesPerLine, i);
    }
    break;
  }
}
...
/*********************************************************************
*
*       Functions available by _GetDevFunc()
*
**********************************************************************
*/
/*********************************************************************
...
*       _ControlCache
*/
static U8 _ControlCache(GUI_DEVICE * pDevice, U8 Cmd) {
  GUI_USE_PARA(pDevice);
  GUI_USE_PARA(Cmd);
  return 0;
}
...
*       _GetDevFunc
*/
static void (* _GetDevFunc(GUI_DEVICE * pDevice, int Index))(void) {
  switch (Index) {
  ...
  case LCD_DEVFUNC_CONTROLCACHE:
    return (void (*)(void))_ControlCache;
  case LCD_DEVFUNC_ON:
...
void GUIDRV_Lin_OSX_16_C(void);   // Avoid empty object files
void GUIDRV_Lin_OSX_16_C(void) {}
