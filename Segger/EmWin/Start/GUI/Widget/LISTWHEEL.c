...
*   Draws the widget
*/
static void _OnPaint(LISTWHEEL_Handle hObj, WM_MESSAGE * pMsg) {
  ...
  if (pfOwnerDraw) {
    ...
    // Draw background
    //
    ItemInfo.Cmd       = WIDGET_DRAW_BACKGROUND;
    ItemInfo.hWin      = hObj;
    ItemInfo.x0        = 0;
    ItemInfo.y0        = 0;
    ...
    // Draw overlay
    //
    ItemInfo.Cmd       = WIDGET_DRAW_OVERLAY;
    ItemInfo.hWin      = hObj;
    ItemInfo.x0        = 0;
    ItemInfo.y0        = 0;
    ItemInfo.x1        = RectInside.x1;
...
static void _OnReleased(LISTWHEEL_Handle hObj, int Notification) {
  ...
  if (Velocity && (hTimer == 0)) {
    ...
    //
    hTimer = WM_CreateTimer(hObj, 0, TIMER_PERIOD, 0);
  } else if (Velocity == 0) {
    //
...
*/
static void _OnPressed(LISTWHEEL_Handle hObj, int x, int y) {
  LISTWHEEL_OBJ * pObj;
  ...
  if (WM_HasCaptured(pMsg->hWin)) {
    if (pMsg->Data.p) {
      ...
      if (pState->Pressed && (pState->y >= 0) && (pState->y <= Rect.y1)) {
        _OnPressed(hObj, pState->x, pState->y);
      } else {
        if (pState->Pressed) {
...
*/
static void _OnTimer(LISTWHEEL_Handle hObj, WM_MESSAGE * pMsg) {
  LISTWHEEL_OBJ * pObj;
  WM_HMEM hTimer;
...
void LISTWHEEL_MoveToPos(LISTWHEEL_Handle hObj, unsigned int Index) {
  LISTWHEEL_OBJ * pObj;
  WM_HMEM hTimer;
  int Pos, SnapPosition, Destination, ySizeData;
