...
*/
int WIDGET__GetWindowSizeX(WM_HWIN hObj) {
  WIDGET * pWidget;
  int r;
  pWidget = WIDGET_LOCK(hObj);
  if (pWidget->State & WIDGET_STATE_VERTICAL) {
    r = WM_GetWindowSizeY(hObj);
  } else {
    r = WM_GetWindowSizeX(hObj);
  }
...
int WIDGET_HandleActive(WM_HWIN hObj, WM_MESSAGE * pMsg) {
  int Diff, Notification, r;
  WIDGET * pWidget;
  r = 1;                              /* Message NOT handled */
  if (hObj) {
    pWidget = WIDGET_LOCK(hObj);
    switch (pMsg->MsgId) {
