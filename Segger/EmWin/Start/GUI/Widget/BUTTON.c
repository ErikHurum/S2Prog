...
/* Draw the actual button (background and text) */  
  ...
  if (State & BUTTON_STATE_FOCUS) {
    ...
    GUI_DrawFocusRect(&rClient, pObj->Widget.pEffect->EffectSize > 2 ? pObj->Widget.pEffect->EffectSize : 2);
  }
...
*       _OnTouch
*/
static void _OnTouch(BUTTON_Handle hObj, WM_MESSAGE * pMsg) {
...
BUTTON_Handle BUTTON_CreateEx(int x0, int y0, int xsize, int ysize, WM_HWIN hParent, int WinFlags, int ExFlags, int Id) {
  ...
  if (hObj) {
    BUTTON_Obj * pObj = (BUTTON_Obj *)GUI_LOCK_H(hObj); /* Don't use use WIDGET_H2P because WIDGET_INIT_ID() has not be called at this point */
