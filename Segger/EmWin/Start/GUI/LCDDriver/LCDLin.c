...
----------------------------------------------------------------------
File        : LCDLin.c
Purpose     : Driver for any display controller with linear 
              video memory in 1, 2, 4, 8 and 16 bits/pixel mode.
...
*/
  ...
  #define SwapBytes SwapBytes_5
#else
  LCD_DISPLAY_INDEX > 5 not supported!
#endif
...
#elif LCD_DISPLAY_INDEX == 5
  #define SIM_WriteMem8  SIM_WriteMem8_5
  #define SIM_WriteReg8  SIM_WriteReg8_5
  #define SIM_ReadMem8   SIM_ReadMem8_5
  #define SIM_ReadReg8   SIM_ReadReg8_5
  #define SIM_WriteMem16 SIM_WriteMem16_5
  #define SIM_WriteReg16 SIM_WriteReg16_5
  #define SIM_ReadMem16  SIM_ReadMem16_5
  #define SIM_ReadReg16  SIM_ReadReg16_5
#endif
...
*/
    ...
    #elif LCD_DISPLAY_INDEX == 5     /* 6. display in a multi-display configuration */
      #define LCD_READ_MEM(Off)       LCD_READ_MEM_5(Off)
      #define LCD_WRITE_MEM(Off,Data) LCD_WRITE_MEM_5(Off,Data)
      #define LCD_READ_REG(Off)       LCD_READ_REG_5(Off)
      #define LCD_WRITE_REG(Off,Data) LCD_WRITE_REG_5(Off,Data)
    #else
...
/*********************************************************************
*
*       Static code: BitBlt access for S1D13513
*
**********************************************************************
*/
#if LCD_USE_BITBLT              \
...
*       _WaitForComplete
*/
static void _WaitForComplete(void) {
  U16 Data;
  do {
    READ_REG((0x1800 >> 1), Data);
  } while (Data & 1);
}

/*********************************************************************
*
*       _WriteFormatAndOffset
*/
static void _WriteFormatAndOffset(void) {
  #if   (LCD_BITSPERPIXEL ==  8)
    WRITE_REG((0x1804 >> 1), 0x00);
    WRITE_REG((0x1824 >> 1), (LCD_VXSIZE << 0));
  #elif (LCD_BITSPERPIXEL == 16)
    WRITE_REG((0x1804 >> 1), 0x04);
    WRITE_REG((0x1824 >> 1), (LCD_VXSIZE << 1));
  #elif (LCD_BITSPERPIXEL == 32)
    WRITE_REG((0x1804 >> 1), 0x08);
    WRITE_REG((0x1824 >> 1), (LCD_VXSIZE << 2));
  #else
    #error Not supported!
  #endif
}
...
static void _FillRectBB(int x0, int y0, int x1, int y1) {
  LCD_ENABLE_REG_ACCESS(); {
    WRITE_REG((0x1802 >> 1), 0x01);           /* Reset BitBLT      */
    _WriteFormatAndOffset();
    WRITE_REG((0x1808 >> 1), 0x03);           /* Solid fill        */
    WRITE_REG((0x181C >> 1), x0);             /* xPos              */
    WRITE_REG((0x181E >> 1), y0);             /* yPos              */
    WRITE_REG((0x1826 >> 1), (x1 - x0 + 1));  /* Width             */
    WRITE_REG((0x1828 >> 1), (y1 - y0 + 1));  /* Height            */
    WRITE_REG((0x1854 >> 1), COLOR & 0xFFFF); /* Color low  word   */
    WRITE_REG((0x1856 >> 1), COLOR >> 16);    /* Color high word   */
    WRITE_REG((0x1800 >> 1), 1);              /* Start operation   */
    _WaitForComplete();                       /* Wait for complete */
  } LCD_ENABLE_MEM_ACCESS();
}
...
static void _DrawBitmap1BPPBB(int x, int y, U8 const * p, int Diff, int xsize, int ysize, int BytesPerLine, const LCD_PIXELINDEX * pTrans) {
  ...
  int WordsPerLine;
  x           += Diff;
  WordsPerLine = (BytesPerLine + 1) >> 1;
  Format       = (GUI_Context.DrawMode & LCD_DRAWMODE_TRANS) ? 0x0030 : 0x0020;
  LCD_ENABLE_REG_ACCESS(); {
    WRITE_REG((0x1802 >> 1), 0x01);            /* Reset BitBLT                */
    _WriteFormatAndOffset();
    WRITE_REG((0x1808 >> 1), Format);          /* Color expansion mode        */
    WRITE_REG((0x181C >> 1), x);               /* xPos                        */
    WRITE_REG((0x181E >> 1), y);               /* yPos                        */
    WRITE_REG((0x1826 >> 1), xsize);           /* Width                       */
    WRITE_REG((0x1828 >> 1), ysize);           /* Height                      */
    WRITE_REG((0x1860 >> 1), Diff);            /* Starting position           */
    WRITE_REG((0x1862 >> 1), 0x00);            /* Color expansion format 1bpp */
    WRITE_REG((0x1900 >> 1), (*(pTrans + 0))); /* Background color            */
    WRITE_REG((0x1902 >> 1), (*(pTrans + 1))); /* Foreground color            */
    WRITE_REG((0x1850 >> 1), (*(pTrans + 0))); /* Transparent color           */
    WRITE_REG((0x1800 >> 1), 1);               /* Start operation             */
    /* Write data */
    do {
      int i, RemBytes;
      i = WordsPerLine;
      RemBytes = BytesPerLine;
      pSrc = (U8 *)p;
      do {
        U16 Data;
        /* Check if buffer is able to recept data */
        do {
          READ_REG((0x1890 >> 1), Data);
        } while (Data & 0x2);
        /* Get data */
        if (RemBytes > 1) {
          Data = ((*pSrc) << 8) | (*(pSrc + 1));
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 16 bit bus, using BITBLT
*/
#if (LCD_USE_BITBLT)           \
    && (!LCD_SWAP_XY)          \
    && (LCD_BUSWIDTH==16)      \
    && (!defined (LCD_LUT_COM))   \
    && (!defined (LCD_LUT_SEG))   \
    && ((LCD_CONTROLLER == 1304) || (LCD_CONTROLLER == 1305) || (LCD_CONTROLLER == 1356) || (LCD_CONTROLLER == 13806))
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 16 bit bus, 16bpp, LCD_SWAP_XY == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (LCD_BUSWIDTH==16)      \
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (LCD_BUSWIDTH==16)      \
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 16 bit bus, 4bpp, LCD_SWAP_XY == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (LCD_BUSWIDTH==16)      \
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 16 bit bus, 1bpp, LCD_SWAP_XY == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (LCD_BUSWIDTH==16)      \
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 8 bit bus, 8bpp, LCD_SWAP_XY == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (LCD_BUSWIDTH==8)      \
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 8 bit bus, 4bpp, LCD_SWAP_XY == 0, LCD_MIRROR_Y == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (!LCD_MIRROR_Y)         \
...
/*********************************************************************
*
*       LCD_L0_DrawHLine, optimized for 8 bit bus, 1bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0, LCD_MIRROR_Y == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (!LCD_MIRROR_Y)         \
...
/*********************************************************************
*
*       LCD_L0_DrawVLine, optimized for 16 bit bus, using BITBLT, LCD_SWAP_XY == 0
*/
#if (LCD_USE_BITBLT)           \
    && (!LCD_SWAP_XY)          \
    && (LCD_BUSWIDTH==16)      \
...
/*********************************************************************
*
*       LCD_L0_DrawVLine, optimized for 8 bit bus, 4bpp, LCD_SWAP_XY == 1, LCD_MIRROR_Y == 1
*/
#elif (LCD_OPTIMIZE)             \
      && (LCD_SWAP_XY)          \
      && (LCD_MIRROR_Y)         \
...
/*********************************************************************
*
*       LCD_L0_DrawVLine, optimized for 8 bit bus, 2bpp, LCD_SWAP_XY == 1, LCD_MIRROR_X == 1, LCD_MIRROR_Y == 0
*/
#elif (LCD_OPTIMIZE)              \
      && (LCD_SWAP_XY)            \
      && (LCD_MIRROR_X)           \
...
/*********************************************************************
*
*       LCD_L0_DrawVLine, optimized for 16 bit bus, 4bpp, LCD_SWAP_XY == 1
*/
#elif (LCD_OPTIMIZE)               \
      && (LCD_SWAP_XY)             \
      && (!defined (LCD_LUT_COM))  \
...
/*********************************************************************
*
*       LCD_L0_DrawVLine, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY == 0, LCD_MIRROR_Y == 0
*/
#elif (LCD_OPTIMIZE)              \
      && (!LCD_SWAP_XY)          \
      && (!LCD_MIRROR_Y)          \
...
/*********************************************************************
*
*       LCD_L0_DrawVLine, optimized for 16 bit bus, 16bpp, LCD_SWAP_XY == 1
*/
#elif (LCD_OPTIMIZE)               \
      && (LCD_SWAP_XY)             \
      && (!defined (LCD_LUT_COM))  \
...
void LCD_L0_DrawVLine(int x, int y0,  int y1) {
  ...
  Off = XY2OFF(y0, x);
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
    LCD_PIXELINDEX Index, Mask;
    Mask = LCD_L0_GetIndexMask();
    do {
      READ_MEM(Off, Index);
      Index ^= Mask;
      WRITE_MEM(Off, Index);
...
/*********************************************************************
*
*       LCD_L0_FillRect, optimized for 16 bit bus, Using BITBLT, LCD_SWAP_XY == 0
*/
#if (LCD_USE_BITBLT)           \
    && (!LCD_SWAP_XY)          \
    && (LCD_BUSWIDTH==16)      \
...
/*********************************************************************
*
*       LCD_L0_FillRect, using (optimized) LCD_L0_DrawHLine/LCD_L0_DrawVLine
*/
void LCD_L0_FillRect(int x0, int y0, int x1, int y1) {
  #ifdef LCD_FILL_RECT
...
/*********************************************************************
*
*       DrawBitLine1BPP, optimized for 8 bit bus, 1bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0, LCD_MIRROR_Y == 0
*/
#if (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (!LCD_MIRROR_Y)         \
...
/*********************************************************************
*
*       DrawBitLine1BPP, optimized for 16 bit bus, 1bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0, LCD_MIRROR_Y == 0
*/
#elif (LCD_OPTIMIZE)                \
      && (!LCD_SWAP_XY)           \
      && (!LCD_MIRROR_Y)          \
...
/*********************************************************************
*
*       DrawBitLine1BPP, optimized for 16 bit bus, 2bpp, LCD_SWAP_XY == 1, LCD_MIRROR_X == 1, LCD_MIRROR_Y == 0
*/
#elif (LCD_OPTIMIZE)            \
    && (LCD_SWAP_XY)            \
    && (LCD_MIRROR_X)           \
...
static void DrawBitLine1BPP_Swap(int x, int y, U8 const GUI_UNI_PTR * pData, int ysize, const LCD_PIXELINDEX * pTrans, int BytesPerLine, U8 Pos) {
...
/*********************************************************************
*
*       DrawBitLine1BPP, optimized for 16 bit bus, 4bpp, LCD_SWAP_XY == 1
*/
#elif (LCD_OPTIMIZE)               \
    && (LCD_SWAP_XY)             \
    && (!defined (LCD_LUT_COM))  \
...
/*********************************************************************
*
*       DrawBitLine1BPP, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (!LCD_MIRROR_X)         \
...
/*********************************************************************
*
*       DrawBitLine1BPP, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 1
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (LCD_MIRROR_X)         \
...
/*********************************************************************
*
*       DrawBitLine1BPP, optimized for 16 bit bus, 4bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0
*/
#elif (LCD_OPTIMIZE)             \
      && (!LCD_SWAP_XY)          \
      && (!LCD_MIRROR_X)         \
...
/*********************************************************************
*
*       DrawBitLine1BPP_Swap, optimized for 16 bit bus, 16bpp, LCD_SWAP_XY == 1
*/
#elif (LCD_OPTIMIZE)               \
    && (LCD_SWAP_XY)             \
    && (!defined (LCD_LUT_COM))  \
...
/*********************************************************************
*
*       DrawBitLine4BPP, optimized for 16 bit bus, 4bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0
*/
...
/*********************************************************************
*
*       DrawBitLine4BPP_Swap, optimized for 16 bit bus, 4bpp, LCD_SWAP_XY == 1
*/
#elif (LCD_OPTIMIZE)               \
    && (LCD_SWAP_XY)             \
    && (!defined (LCD_LUT_COM))  \
...
/*********************************************************************
*
*       DrawBitLine4BPP, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0
*/
#elif (LCD_OPTIMIZE)               \
...
/*********************************************************************
*
*       DrawBitLine8BPP, optimized for 16 bit bus, 4bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0
*/
#if (LCD_OPTIMIZE)               \
...
/*********************************************************************
*
*       DrawBitLine8BPP, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 0
*/
#elif (LCD_OPTIMIZE)               \
    && (LCD_MAX_LOG_COLORS > 16) \
    && (LCD_BITSPERPIXEL == 8)   \
...
static void  DrawBitLine8BPP(int x, int y, U8 const GUI_UNI_PTR *p, int xsize, const LCD_PIXELINDEX*pTrans) {
  ...
  if ((GUI_Context.DrawMode & LCD_DRAWMODE_TRANS)==0) {
    ...
    /* Handle bitmaps without palette */
        ...
        GUI_MEMCPY((U16*)(LCD_DATAADR+x+((U32)y)*LCD_VXSIZE), p,xsize);
      #else
        if (x&1) {
          pixel = *p++;
          SETPIXEL(x+0, y, pixel);
...
/*********************************************************************
*
*       DrawBitLine8BPP, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY == 0, LCD_MIRROR_X == 1
*/
#elif (LCD_OPTIMIZE)               \
    && (LCD_MAX_LOG_COLORS > 16) \
    && (LCD_BITSPERPIXEL == 8)   \
...
/*********************************************************************
*
*       DrawBitLine16BPP, optimized for 16 bit bus, 16bpp, LCD_SWAP_XY == 0
*/
#if (LCD_BITSPERPIXEL > 8)
#if (LCD_OPTIMIZE)               \
    && (LCD_MAX_LOG_COLORS > 16) \
...
*/
    ...
    && (LCD_SWAP_XY)            \
    && (!defined (LCD_LUT_SEG))
static void DrawBitLine16BPP_Swap(int x, int y, U16 const GUI_UNI_PTR * pData, int ySize, int BytesPerLine, U8 Pos) {
  tOff Off;
  int WordsPerLine;
...
/*********************************************************************
*
*       DrawBitLine16BPP, not optimized
*/
#else
  ...
  */
  ...
  case 1:
    ...
    case 16:
          ...
          && (LCD_BUSWIDTH==16)
        for (i = 0; i < xsize; i++) {
          DrawBitLine16BPP_Swap(x0 + i, y0, (U16 *)(pData + (i << 1)), ysize, BytesPerLine, i);
        }
      #else
        for (i=0; i<ysize; i++) {
...
/*********************************************************************
*
*       LCD_L0_ReadRect, optimized for 16 bit bus, 8bpp, LCD_SWAP_XY = 0, LCD_MIRROR_X = 1, LCD_MIRROR_Y = 1
*/
#elif (LCD_OPTIMIZE)            \
    && (!LCD_SWAP_XY)           \
    && (LCD_MIRROR_X)           \
...
void LCDLin_c(void);
void LCDLin_c(void) { } /* avoid empty object files */
