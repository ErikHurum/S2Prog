...
*/
static int _GetPrevPointDiffy(const GUI_POINT * paPoint, int i,const int NumPoints, const int y0) {
  int j, y1;
...
void GL_FillPolygon  (const GUI_POINT*paPoint, int NumPoints, int xOff, int yOff) {
  int i, y, i1, y0, y1, yMin, yMax, yPrev, xIntersect;
  ...
  for (y = yMin; y <= yMax; y++) {
    _Init();
    //
    // find next intersection and count line
    //
    for (i = 0; i<NumPoints; i++) {
      i1 = (i < (NumPoints - 1)) ? i + 1 : 0;
      y0 = (paPoint + i )->y;
      y1 = (paPoint + i1)->y;
      //
      ...
      if (y0 == y) {
        if (y1 == y) {  // Add the entire line
          ...
          yPrev = _GetPrevPointDiffy(paPoint, i, NumPoints, y);
          if (yPrev != y) {
...
void GUI_FillPolygon(const GUI_POINT* pPoints, int NumPoints, int x0, int y0) {
  GUI_DEVICE * pDevice;
  void (* pfFillPolygon)(GUI_DEVICE *, const GUI_POINT *, int, int, int);
  ...
  pfFillPolygon = (void (*)(GUI_DEVICE *, const GUI_POINT *, int, int, int))pDevice->pDeviceAPI->pfGetDevFunc(pDevice, LCD_DEVFUNC_FILLPOLY);
  if (pfFillPolygon) {
    pfFillPolygon(pDevice, pPoints, NumPoints, x0, y0);
  } else {
