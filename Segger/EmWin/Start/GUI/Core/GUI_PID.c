...
**********************************************************************
*/
static GUI_PID_STATE _aState[GUI_PID_BUFFER_SIZE]; // Buffer
...
static GUI_PID_STATE * _DecPointer(GUI_PID_STATE * p, GUI_PID_STATE * pRef) {
  ...
  if (pNew == _aState) {
    ...
    //
    pNew = _aState + GUI_PID_BUFFER_SIZE - 1;
  } else {
    //
    // Decrement pointer
    //
...
static GUI_PID_STATE * _IncPointer(GUI_PID_STATE * p, GUI_PID_STATE * pRef) {
  GUI_PID_STATE * pNew;
  pNew = p;
  if (pNew == (_aState + GUI_PID_BUFFER_SIZE - 1)) {
    //
