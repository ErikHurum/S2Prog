...
static void  _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * p, int xSize) {
  ...
  pDevice->pNext->pDeviceAPI->pfDrawBitmap(pDevice, x, y, xSize, 1, BitsPerPixel, 0, (U8 *)_pIndex, 0, 0);
}
...
static void (* _GetDevFunc(GUI_DEVICE * pDevice, int Index))(void) {
  pDevice = pDevice->pNext;
  return pDevice->pDeviceAPI->pfGetDevFunc(pDevice, Index);
}
...
unsigned GUI_SetAlpha(U8 Alpha) {
  ...
  OldAlpha = _aAlpha[GUI_Context.apDevice[GUI_Context.SelLayer]->LayerIndex];
  if (Alpha == 0) {
    if (GUI_Context.apDevice[GUI_Context.SelLayer]->pDeviceAPI->DeviceClassIndex == DEVICE_CLASS_ALPHA) {
      //
      // Delete device
      //
      ...
      _aAlpha[GUI_Context.apDevice[GUI_Context.SelLayer]->LayerIndex] = Alpha;
    }
  } else {
    if (_InitAlpha() == 0) {
      //
      ...
      if (GUI_Context.apDevice[GUI_Context.SelLayer]->pDeviceAPI->DeviceClassIndex != DEVICE_CLASS_ALPHA) {
        ...
        // Create new device
        //
        GUI_DEVICE_CreateAndLink(&_Alpha_API, NULL, 0, GUI_Context.SelLayer);
      }
      //
      // Set alpha value
      //
