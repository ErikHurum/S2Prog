...
void LCD_SetPixelAA(int x, int y, U8 Intens) {
  ...
  RETURN_IF_X_OUT();
  if (Intens >= 15) {
    LCDDEV_L0_SetPixelIndex(GUI_Context.apDevice[GUI_Context.SelLayer], x, y, LCD_COLORINDEX);
  } else {
...
void LCD_SetPixelAA_NoTrans(int x, int y, U8 Intens) {
  ...
  RETURN_IF_X_OUT();
  if (Intens == 0) {
    LCDDEV_L0_SetPixelIndex(GUI_Context.apDevice[GUI_Context.SelLayer], x, y, LCD_BKCOLORINDEX);
  } else if (Intens == 15) {
    LCDDEV_L0_SetPixelIndex(GUI_Context.apDevice[GUI_Context.SelLayer], x, y, LCD_COLORINDEX);
  } else {
    ...
    Color      = LCD_Index2Color(LCD_COLORINDEX);
    ColorBk    = LCD_Index2Color(LCD_BKCOLORINDEX);
