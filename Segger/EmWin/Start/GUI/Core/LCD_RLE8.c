...
static void _DrawBitmap_RLE8(int x0,int y0,int xsize, int ysize, const U8 GUI_UNI_PTR * pPixel, const LCD_LOGPALETTE GUI_UNI_PTR * pLogPal, int xMag, int yMag) {
  ...
  aColorIndex[0] = *LCD_pBkColorIndex;
  aColorIndex[1] = *LCD_pColorIndex;
 ...
 /* Check if we can limit the number of lines due to clipping) */
  ...
  for (; y < ysize; ) {
    ...
    if (Cmd) {
      ...
      Cache.x = xi;
      Cache.y = y;
      LCD_COLORINDEX = pTrans ? *(pTrans+Data) : Data;
      while (Cmd) {
        int xi1 = xi+Cmd;
        if (xi1>=xsize)
          xi1 = xsize;
              ...
              if (y + y0>= GUI_Context.ClipRect.y0)
                if (x >= GUI_Context.ClipRect.x0)
                  if (x <= GUI_Context.ClipRect.x1)
                    LCDDEV_L0_SetPixelIndex(GUI_Context.apDevice[GUI_Context.SelLayer], x, y + y0, PixelIndex);
            #else
  ...
  LCD_BKCOLORINDEX = aColorIndex[0];
  LCD_COLORINDEX = aColorIndex[1];
}
