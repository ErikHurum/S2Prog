...
int GUI_SIF__GetNumCharAreas(const GUI_FONT GUI_UNI_PTR * pFont) {
  U16 NumCharAreas;
  const U8 * pData;
  pData = (const U8 *)pFont->p.pFontData + 14;            /* Skip unused data */
...
const U8 * GUI_SIF__GetpCharInfo(const GUI_FONT GUI_UNI_PTR * pFont, U16P c, unsigned SizeOfCharInfo) {
  ...
  int NumCharAreas;
  NumCharAreas = GUI_SIF__GetNumCharAreas(pFont);         /* Get # of char areas */
  pCharArea = (const U8 *)pFont->p.pFontData 
              + sizeof(GUI_SI_FONT);                      /* Set char area pointer to first char area */
  pCharInfo = pCharArea
              + (U32)sizeof(GUI_SIF_CHAR_AREA) * NumCharAreas; /* Set char info pointer to first character info */
  do {
    ...
    if ((c >= CharArea.First) && (c <= CharArea.Last)) {
      pCharInfo += (U32)SizeOfCharInfo * (c - CharArea.First);
      return pCharInfo;
    }
    pCharInfo += (U32)SizeOfCharInfo * (CharArea.Last - CharArea.First + 1);
  } while(--NumCharAreas);
  return 0;
}
